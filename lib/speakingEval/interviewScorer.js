...
export async function scoreInterview({ question, transcript }) {
  ...
  try {
    ...
    // 120s — short-answer interview scoring is lighter than writing eval but
    // still needs headroom for proxy/load spikes.
    const raw = await callAI(systemPrompt, userPrompt, 1500, 120000, 0.3);
