...
function mapBuildSentence(raw) {
  ...
  // runtime（runtimeModel.validateRuntimeQuestion）要求词块互不相同 —— 重复词块会被
  // prepareQuestions 判成「题库数据异常」整题丢掉，这里提前抛出，好让批次题数与实际可做题数一致。
  if (new Set(chunks).size !== chunks.length) {
    throw new Error("真题造句适配失败：词块重复（runtime 要求 bank 内词块互不相同）");
  }
...
export function mapRealLCRSetsToPicker(sets) {
  return (Array.isArray(sets) ? sets : [])
    ...
    .map((s, i) => ({
      ...compactCard(s, i),
      title: `第 ${i + 1} 套 · ${s.items.length} 题`,
