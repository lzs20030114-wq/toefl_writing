...
// 只会渲染出生词 —— 唯一稳定且对用户有意义的分类还是「来源分档 + 考试日期」（readingTag）。

export function mapRealLCRToPicker(items) {
  return (Array.isArray(items) ? items : []).map((it) => ({
    id: it.id,
    tag: readingTag(it),
    title: truncate(it.speaker, 70) || "(真题应答)",
    subtitle: readingSubtitle(it, it.audio_url ? "含真题录音" : "文本朗读"),
  }));
}
...
export function mapRealLCToPicker(items) {
  return (Array.isArray(items) ? items : []).map((it) => ({
    id: it.id,
    tag: readingTag(it),
    title: truncate(it.situation || it.conversation?.[0]?.text, 70) || "(真题对话)",
    subtitle: readingSubtitle(it, `${it.questions.length} 题`),
  }));
}
...
export function mapRealLAToPicker(items) {
  return (Array.isArray(items) ? items : []).map((it) => ({
    id: it.id,
    tag: readingTag(it),
    title: truncate(it.situation || firstSentence(it.announcement), 70) || "(真题通知)",
    subtitle: readingSubtitle(it, `${it.questions.length} 题`),
  }));
}
...
export function mapRealLATToPicker(items) {
  return (Array.isArray(items) ? items : []).map((it) => ({
    id: it.id,
    tag: readingTag(it),
    title: truncate(it.topic || firstSentence(it.transcript), 70) || "(真题讲座)",
    subtitle: readingSubtitle(it, `${it.questions.length} 题`),
  }));
}
...
export function mapRealRepeatToPicker(sets) {
  return (Array.isArray(sets) ? sets : []).map((s) => ({
    id: s.id,
    tag: readingTag(s),
    title: truncate(firstSentence(s.scenario) || s.sentences?.[0]?.sentence, 70) || "(真题跟读)",
    subtitle: readingSubtitle(s, `${s.sentences.length} 句`),
  }));
}
...
export function mapRealInterviewToPicker(sets) {
  return (Array.isArray(sets) ? sets : []).map((s) => ({
    id: s.id,
    tag: readingTag(s),
    title: truncate(firstSentence(s.intro) || s.questions?.[0]?.question, 70) || "(真题访谈)",
    subtitle: readingSubtitle(s, `${s.questions.length} 题`),
  }));
}
