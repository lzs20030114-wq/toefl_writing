...
/* ── 按考试场次分组（真题专区二期前端：/real-bank/sets） ──────────────── */
//
// 机经用户的心智模型是「哪一场考试」（1.21 A 卷 / 3.10 / 5.10…），不是「哪种题型」。
// 54 套卷铺量后按题型的扁平列表会有几百条，找不着；这里把同一 source（卷名）的题归成一场，
// 每场带各题型的题目清单，供场次总览 / 场次详情页消费。
//
// 只有 build_bank 产物（阅读三题型）带卷名；写作三题型是冻结语料，source 是网址或 TPO 编号，
// 不属于任何一场考试，所以不进场次。将来 build_bank 产出写作 / 口语 / 听力真题时，
// 只要 item 带同样的 source/date 字段，往 REAL_SET_SECTIONS 加一行就会自动进场次页。

/**
 * 场次里可出现的题型（顺序 = 详情页分组顺序 = 考试科目顺序）。
 *   key    — 题型键，也是 /real-bank?type= 的取值
 *   subject — 科目（阅读 / 写作 / 听力 / 口语），总览页按科目汇总
 *   label  — 中文题型名；unit — 计数单位（篇 / 题）
 */
export const REAL_SET_SECTIONS = [
  { key: "ctw", subject: "reading", subjectLabel: "阅读", label: "阅读填词", etsName: "Complete the Words", unit: "篇" },
  { key: "rdl", subject: "reading", subjectLabel: "阅读", label: "日常阅读", etsName: "Read in Daily Life", unit: "篇" },
  { key: "ap", subject: "reading", subjectLabel: "阅读", label: "学术阅读", etsName: "Academic Passage", unit: "篇" },
];

/** 卷名 → 场次 id。卷名本身就是唯一键（build_bank 跨卷去重按内容哈希，同名卷只会入库一份），
 *  只做去空白，URL 里由 encodeURIComponent 处理中文。 */
export function realSetId(source) {
  return String(source || "").replace(/\s+/g, "").trim();
}

/** 卷名里的日期展示：「1.21新托福真题A卷」→ { month: "1", day: "21", variant: "A卷" }。
 *  解析失败（卷名格式外）时回落到 date 字段；再不行就只显示卷名。 */
export function parseRealSetName(source, date) {
  const s = String(source || "").trim();
  const m = s.match(/^(\d{1,2})\.(\d{1,2})(?:新托福真题|托福真题|真题)?\s*([A-Z]卷)?/);
  if (m) return { month: m[1], day: m[2], variant: m[3] || "" };
  const d = String(date || "").match(/^(\d{4})-(\d{2})-(\d{2})$/);
  if (d) return { month: String(Number(d[2])), day: String(Number(d[3])), variant: "" };
  return { month: "", day: "", variant: "" };
}

function sectionItems(sectionKey) {
  if (sectionKey === "ctw") return getRealCTWItems().map((it) => ({ raw: it, picker: mapRealCTWToPicker([it])[0] }));
  if (sectionKey === "rdl") return getRealRDLItems().map((it) => ({ raw: it, picker: mapRealRDLToPicker([it])[0] }));
  if (sectionKey === "ap") return getRealAPItems().map((it) => ({ raw: it, picker: mapRealAPToPicker([it])[0] }));
  return [];
}

/**
 * 全部场次，按考试日期倒序（最近一场在最前，与「回忆版更贴近当下考情」同一逻辑）。
 * 每场形状：
 *   { id, source, date, tier, flags: [{code,severity,detail}],
 *     sections: [{ key, label, unit, items: [{ id, title, subtitle, type }] }],   // 只含有题的题型
 *     total }                                                                   // 本场题目总数
 * flags 是本场所有题 source_flags 的并集（按 code 去重）—— 场次页只提示一次，不逐题重复。
 */
export function getRealExamSets() {
  const bySet = new Map();
  for (const section of REAL_SET_SECTIONS) {
    for (const { raw, picker } of sectionItems(section.key)) {
      if (!raw?.source) continue;
      const id = realSetId(raw.source);
      if (!id) continue;
      if (!bySet.has(id)) {
        bySet.set(id, {
          id,
          source: String(raw.source).trim(),
          date: String(raw.date || "").trim(),
          tier: raw.tier || REAL_READING_TIER,
          flags: [],
          sections: new Map(),
          total: 0,
        });
      }
      const set = bySet.get(id);
      if (!set.date && raw.date) set.date = String(raw.date).trim();
      for (const f of raw.source_flags || []) {
        if (!set.flags.some((x) => x.code === f.code)) set.flags.push({ ...f });
      }
      if (!set.sections.has(section.key)) {
        set.sections.set(section.key, { key: section.key, label: section.label, unit: section.unit, subject: section.subject, items: [] });
      }
      set.sections.get(section.key).items.push({
        id: picker.id,
        title: picker.title,
  ...
  return [...bySet.values()]
    .map((s) => ({ ...s, sections: REAL_SET_SECTIONS.map((sec) => s.sections.get(sec.key)).filter(Boolean) }))
    .sort((a, b) => (b.date || "").localeCompare(a.date || "") || a.source.localeCompare(b.source, "zh"));
}

/** 单场（找不到返回 null）。 */
export function getRealExamSet(setId) {
  const id = realSetId(setId);
  if (!id) return null;
  return getRealExamSets().find((s) => s.id === id) || null;
}

/** 已知源料缺陷的用户可读一句话（只取 warn 及以上；blocking 题本就不该入库，若出现同样提示）。 */
export function realSourceFlagNote(flags) {
  const list = Array.isArray(flags) ? flags : [];
  if (list.length === 0) return "";
  const detail = list.map((f) => String(f?.detail || "").trim()).filter(Boolean)[0];
  return detail ? `已知来源提示：${detail}` : "该场次来源有已知瑕疵，请留意";
}
