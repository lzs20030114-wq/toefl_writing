...
//
// 写作回忆版（data/realBank/writing/{bs,email,discussion}.json）同样是 build_bank.mjs 的产物
// （商家「重排版」docx → 结构化），来源分档一律 recalled；与上面四份手写语料合流后按 id 去重。
//
...
import RB_WRITING_BS from "../data/realBank/writing/bs.json";
import RB_WRITING_EMAIL from "../data/realBank/writing/email.json";
import RB_WRITING_DISCUSSION from "../data/realBank/writing/discussion.json";
...
import RB_LISTENING_ALIASES from "../data/realBank/listening/id-aliases.json";
import RB_SPEAKING_ALIASES from "../data/realBank/speaking/id-aliases.json";
import { materialImageSrc } from "./reading/materialImage";
import { isSentenceSelection, normalizeSentenceSelection } from "./reading/sentenceSelection";
import { splitBlankToken } from "./reading/ctwToken";
import { normalizeSentenceTimings } from "./listening/sentenceTimings";
...
export const REAL_TIER_NOTE =
  ...
  + "带「双票复核」标的是回忆重排版，答案经两家模型复核一致后才收录。";

/** 源料缺陷（vendor_reformatted）在选题卡上的徽章文案。 */
export const REAL_SOURCE_FLAG_BADGE = "双票复核";

/**
 * 跨场重出的题在选题卡上的徽章文案。措辞不写「往期」：库里留下的那条不一定考得更早
 * （留的是先入库的那条），说「往期」会把先后说反。
 */
export const REAL_RECYCLED_BADGE = "跨场重复";
...
function compactCard(item, index, subtitleFallback) {
  ...
  // 源料缺陷标记走徽章位而不是 subtitle —— 答题页刻意与常规练习逐像素同款、不再挂来源条，
  // 所以这件事必须在选题时就看得到；而 subtitle 是 nowrap+ellipsis 的窄行（实测追加就被截掉）。
  // 徽章位与「ETS官方」互斥：官方题是 ETS PDF 原题，不可能同时是商家重排版（库里实测 0 条重叠）。
  const flagged = !!realSourceFlagNote(item);
  // 跨场重出的题也得在选题时看得到（同一段材料会在两场考试里出现）——「诚实标注」同一个理由。
  // 写在**标题**而不是徽章位：徽章位已经被「ETS官方 / 双票复核」占着，而回收题继承保留方的
  // 源料标记、多半正好带着双票复核（实测 LC 5 条、复述 1 条全被挤掉，一个都标不上）；
  // subtitle 也不行 —— 那是 nowrap+ellipsis 的窄行，追加会被截掉（见上面那段注释）。
  const recycled = !!(item?.recycled_of || item?.__anyRecycled);
  return {
    id: item.id,
    no: index + 1,
    title: `第 ${index + 1} 套${recycled ? ` · ${REAL_RECYCLED_BADGE}` : ""}`,
    subtitle: date ? `${date} · ${realTierLabel(item?.tier)}` : (subtitleFallback || realTierLabel(item?.tier)),
    ...(item?.tier === "official"
      ? { badge: REAL_TIER_LABELS.official }
      : flagged && { badge: REAL_SOURCE_FLAG_BADGE }),
  };
}
...
/**
 * 学术讨论真题（realBank 回忆版 + recalled_supplement 回忆版 + 参考版）。
 * 回忆版在前 —— 是 2026 真实考试题，更贴近当下考情，放在列表更显眼的位置；
 * realBank 那批带考试日期（卡片第二行会显示），所以排在最前。
 */
export function getRealDiscussionPrompts() {
  const realBank = bankItems(RB_WRITING_DISCUSSION).map(mapDiscussion);
  ...
  return dedupeById(
    [...realBank, ...recalled, ...reference].filter(
...
/** 来源分档的展示优先级：ETS 官方 > 回忆版 > 参考版（数字越小越靠前）。 */
const TIER_ORDER = { official: 0, recalled: 1, legacy: 2 };

/**
 * 邮件真题：ETS 官方（tpo1 / tpo2）在最前，其次 realBank 回忆版，最后参考版。
 * 官方逐字原题是这个题型里最硬的货，不能被后入库的回忆版挤到第 15 套去。
 * 排序稳定（Array.prototype.sort 在 ES2019 起保证稳定），同档内保持入库顺序，
 * 所以「第 N 套」的序号只在新题入库时向后追加，不会回头漂。
 * goals 不足 3 条的整条作废。
 */
export function getRealEmailPrompts() {
  const realBank = bankItems(RB_WRITING_EMAIL).map(mapEmail);
  const reference = (Array.isArray(EM_TPO_REFERENCE) ? EM_TPO_REFERENCE : []).map(mapEmail);
  return dedupeById(
    [...realBank, ...reference].filter(
      (p) => p.id && p.scenario && p.direction && p.goals.length >= 3
    )
  ).sort((a, b) => (TIER_ORDER[a.tier] ?? 2) - (TIER_ORDER[b.tier] ?? 2));
}

/* ── 造句真题（20 条 ETS 官方 + realBank 回忆版） ───────────────── */
//
...
// 用户口径「造句要按套算」：少于这个题数的批次（多为源卷零星回忆、凑不成一套）不进真题专区，
// 避免前端渲染成 1~3 题的碎卡。官方两批各 10 题、多数回忆版卷子 ≥5 题，不受影响。
export const REAL_BS_MIN_BATCH = 5;
...
function mapBuildSentence(raw) {
  ...
  return {
    ...
    source_flags: readingSourceFlags(raw),
    real: true,
    // 跨卷重复还回来的那一份：如实标明同一道题最早出自哪一场（前端据此在卡片上说明）
    ...(raw?.__recycledOf ? { recycled_of: realId(raw.__recycledOf), recycled_from: String(raw.__recycledFrom || "").trim() } : {}),
  };
}

/**
 * 一批 = 一份卷子。官方 20 题按 source_label（Full-Length Practice Test 1 / 2）分批，
 * 回忆版按 source（一场考试一卷）分批 —— 分组键不同，但都保持源文件里的出现顺序，
 * 所以批次序号（→ groupId）不会因为新题入库而回头漂。
 * 单题适配失败（deriveBsPrefilled 对不齐）只丢那一题，绝不让整页崩。
 */
function groupBsBatches(rawItems, groupKey) {
  const order = [];
  const byKey = new Map();
  (Array.isArray(rawItems) ? rawItems : []).forEach((raw) => {
    let q;
    try {
      q = mapBuildSentence(raw);
    } catch (err) {
      console.warn(`[realBank] 造句真题跳过：${raw?.id} —— ${err?.message || err}`);
      return;
    }
    const key = String(groupKey(raw) || "").trim() || "ETS 官方真题";
    if (!byKey.has(key)) { byKey.set(key, []); order.push(key); }
    byKey.get(key).push(q);
  });
  return order
    .map((key) => ({ label: String(byKey.get(key)[0].source_label || key), questions: byKey.get(key) }))
    .filter((b) => b.questions.length > 0);
}

/**
 * 跨卷重复的造句：ETS 真实地循环出题（实测 4.1 有 9 道与 3.21 重复），库里同一道题只留先入库的
 * 那一条，后面又考它的卷靠 writing/id-aliases.json 的别名把这道题还回自己那一批
 * （账本怎么来的见 scripts/realbank/bs_aliases.js）。不还的话那几卷在专区里缺题、
 * 整卷都是重复题的卷（4.1 / 5.6 / rf0902）连卡片都不出现 —— 但题其实一直在库里。
 *
 * 还回来的那一份是**同一道题的另一次出现**：id 用那一卷自己的（做题记录按卷分开），
 * 内容取保留的那条，另带 recycled_of / recycled_from 如实标明它与哪一场的哪道题是同一道。
 * 追加在原生题之后 —— groupBsBatches 按首次出现排批次顺序，追加在后才不会挪动老批次的序号
 * （序号 = 用户的「第 N 套」与已练标记）。
 */
function bsRecycledRaws(items) {
  const byId = new Map(items.map((it) => [String(it?.id), it]));
  const out = [];
  for (const a of RB_WRITING_ALIASES?.aliases || []) {
    if (!a || a.from_type !== "bs" || !a.from || !a.to || !a.from_source) continue;
    const kept = byId.get(String(a.to));
    if (!kept || byId.has(String(a.from))) continue;   // 保留方不在库里 / 这一卷自己就有这道题
    const date = String(a.from_date || kept.date || "").trim();
    out.push({
      ...kept,
      id: a.from,
      source: String(a.from_source).trim(),
      date,
      source_label: `${date} 真题造句`,
      __recycledOf: kept.id,
      __recycledFrom: String(kept.source || "").trim(),
    });
  }
  return out;
}
...
/**
 * 造句真题批次：官方 2 批各 10 题在前（"real-bs-set-1" / "real-bs-set-2" 序号**永远不动**，
 * 否则老用户的「已练」标记会错位），realBank 回忆版按考试套次接在后面（set-3 起）。
 * 每题带 `__sourceGroupId` —— useBuildSentenceSession 完成后按该字段写
 * DONE_STORAGE_KEYS.BUILD_SENTENCE_GP，picker 因此零改动就有「已练」标记。
 */
export function getRealBSBatches() {
  const official = groupBsBatches(BS_TPO_OFFICIAL, (raw) => raw?.source_label);
  const bsItems = bankItems(RB_WRITING_BS);
  const recalled = groupBsBatches([...bsItems, ...bsRecycledRaws(bsItems)], (raw) => raw?.source || raw?.source_label);
  const seen = new Set();
  return [...official, ...recalled].map((b, i) => {
    const groupId = `${REAL_BS_GROUP_PREFIX}${i + 1}`;
    const questions = b.questions
      .filter((q) => !seen.has(q.id) && seen.add(q.id))
      .map((q) => ({ ...q, __sourceGroupId: groupId }));
    return {
      id: groupId,
      label: b.label,
      tier: questions[0]?.tier || "legacy",
      date: questions[0]?.date || "",
      questions,
    };
  }).filter((b) => b.questions.length >= REAL_BS_MIN_BATCH);
}

/** 扁平的全部造句真题（已带 __sourceGroupId）。 */
...
/**
 * 邮件题：tag = 来源分档（回忆版 / TPO 两类，低基数）；
 * 回忆版带考试日期（compactCard 第二行显示），TPO 那批没有日期 → 第二行退回来源分档。
 */
export function mapRealEmailToPicker(items) {
  return (Array.isArray(items) ? items : []).map((p, i) => ({
    ...compactCard(p, i, realTierLabel(p.tier)),
    tag: p.tier === "recalled" ? REAL_TIER_LABELS.recalled : "TPO",
  }));
}

/**
 * 造句题：一批一张卡，id = __sourceGroupId，与已练 key 同源。
 * tag = 来源分档（官方 / 回忆版两类，低基数，筛选栏干净）；徽章只给 ETS 官方。
 */
export function mapRealBSToPicker(batches) {
  return (Array.isArray(batches) ? batches : []).map((b, i) => {
    // 卡片上把「这套里有几道与别的场次是同一道题」说清楚：不说的话，按卷刷的用户会在另一卷里
    // 撞见一模一样的题，以为是题库灌水。真实考试就是这么循环出题的，标出来即可。
    // 措辞刻意不写「往期」：库里留下的那条不一定考得更早（留的是先入库的那条），说「往期」会把先后说反。
    const recycled = b.questions.filter((q) => q.recycled_of).length;
    return {
      id: b.id,
      no: i + 1,
      tag: realTierLabel(b.tier),
      title: `第 ${i + 1} 套 · ${b.questions.length} 题`,
      subtitle: recycled > 0 ? `${b.label} · 含 ${recycled} 题与其他场次重复` : b.label,
      ...(b.tier === "official" && { badge: REAL_TIER_LABELS.official }),
    };
  });
}
 ...
 *     token 可以粘着标点（"(like" / "region—not"），比的是切掉前标点之后的部分（lib/reading/ctwToken.js）。
 */
function mapRealCTW(raw) {
  ...
  for (const b of srcBlanks) {
    ...
    const { lead } = splitBlankToken(tokens[position], originalWord);
    if (!tokens[position].slice(lead.length).toLowerCase().startsWith(fragment.toLowerCase())) return null;
...
/**
 * RDL / AP 共用的选择题体检：RDLTask 对四选一题硬编码渲染 A/B/C/D 四个键，
 * 少一个选项或答案键不在 A-D 里，正确答案就渲染不出来 —— 用户怎么点都错。
 * 任一道四选一题不合格 = 整条材料作废（同一屏的其它题也一起丢，宁可少题不许出死题）。
 *
 * 选句题（sentence_selection，契约与体检都在 lib/reading/sentenceSelection.js）另走一套：
 * 选项是 paragraphs[paragraph_index] 那一段的 S1..Sn 各句、答案是 S 键；paragraph_index 缺失 / 越界拒收，
 * 最后一道闸是「RDLTask 能在 material 正文里把每一句按顺序圈出来」。它不合格**只丢这一道题**：
 * 它的毛病只可能是「下标 / 句子和正文对不上」，不代表同屏四选一题的选项也坏了；丢完一道题都不剩才整条作废。
 *
 * @param material 组件实际拿到的正文与段落数组 { text | passage, paragraphs? }（选句题定位用）
 */
function mapMcqQuestions(rawQuestions, material) {
  const src = Array.isArray(rawQuestions) ? rawQuestions : [];
  if (src.length === 0) return null;
  const out = [];
  for (const q of src) {
    if (isSentenceSelection(q)) {
      const selection = normalizeSentenceSelection(q, material);
...
/**
 * 段落数组：逐项 trim、**不滤空**。选句题的 paragraph_index 是源数组的下标，滤掉一个空段
 * 后面每段的下标都错一位 —— 映射前后必须是同一套下标（lib/reading/sentenceSelection.js 同口径）。
 */
function mapParagraphs(rawParagraphs) {
  return (Array.isArray(rawParagraphs) ? rawParagraphs : []).map((p) => String(p ?? "").trim());
}

function mapRealRDL(raw) {
  const id = readingId(raw?.id);
  const text = String(raw?.text || raw?.passage || "").trim();
  // 选句题契约上只出在 AP；真出现在 RDL 里时：源条目带 paragraphs 就按它的下标定位（从学术阅读归位过来的
  // 条目可能带着），不带就按空行切 text。带了才透传，老 RDL 条目的形状不变。
  const paragraphs = mapParagraphs(raw?.paragraphs);
  const hasParagraphs = paragraphs.some(Boolean);
  const questions = mapMcqQuestions(raw?.questions, { text, ...(hasParagraphs ? { paragraphs } : {}) });
...
function mapRealAP(raw) {
  ...
  const paragraphs = mapParagraphs(raw?.paragraphs);
  // 选句题按「页面喂给 RDLTask 的同一份正文与段落数组」体检：app/real-bank 把 passage 转成 text，
  // paragraphs 原样透传 —— 这里必须用映射后（trim 过、下标不变）的这两样，不能用 raw 的。
  const questions = mapMcqQuestions(raw?.questions, { passage, paragraphs });
...
function listeningProvenance(raw) {
  return {
    ...
    date: String(raw?.date || "").trim(),
    source_flags: readingSourceFlags(raw),
    // 跨卷重出还回来的那一份：如实标明它与哪一场的哪道是同一道（四个听力题型 + 两个口语题型共用这一处）
    ...(raw?.__recycledOf
      ? { recycled_of: readingId(raw.__recycledOf), recycled_from: String(raw.__recycledFrom || "").trim() }
      : {}),
  };
}

/**
 * 跨卷重出的听力 / 口语条目：按别名账本（`<科目>/id-aliases.json`，build_bank 落）
 * 把保留方那条克隆回它原本所在的卷。
 *
 * 机经里同一段材料会在多场考试重出，库里只留一条 —— 不还回去，那几场在专区里就少题
 * （整套都是重复的 rf0902 复述 / 面试连卡片都不出现）。与造句那套同一个做法：
 * id 用那一卷自己的（做题记录按场次分开、子条目 id 一并改前缀免得跨套撞车），
 * 内容取保留的那条，带 recycled_of / recycled_from 如实标注。
 */
function withRecycled(items, ledger, types) {
  const list = Array.isArray(items) ? items : [];
  const byId = new Map(list.map((it) => [String(it?.id), it]));
  const out = [...list];
  for (const a of ledger?.aliases || []) {
    if (!a || !types.includes(a.from_type) || !a.from || !a.to || !a.from_source) continue;
    const kept = byId.get(String(a.to));
    if (!kept || byId.has(String(a.from))) continue;
    const from = String(a.from);
    const to = String(a.to);
    const swapId = (v) => (typeof v === "string" && v.startsWith(to) ? from + v.slice(to.length) : v);
    const relabel = (arr) => (Array.isArray(arr)
      ? arr.map((x) => (x && typeof x === "object" && x.id ? { ...x, id: swapId(x.id) } : x))
      : arr);
    out.push({
      ...kept,
      id: from,
      ...(Array.isArray(kept.sentences) ? { sentences: relabel(kept.sentences) } : {}),
      ...(Array.isArray(kept.questions) ? { questions: relabel(kept.questions) } : {}),
      source: String(a.from_source).trim(),
      date: String(a.from_date || kept.date || "").trim(),
      __recycledOf: to,
      __recycledFrom: String(kept.source || "").trim(),
    });
  }
...
/**
 * 句级时间戳（docs/listening-sentence-timings.md）：只在有真实 audio_url 时透传，
 * 且整份通过 normalizeSentenceTimings 体检——半份错位的时间戳比没有更糟（点哪句放哪句全串）。
 * 没有 / 不合格就不带这个键，播放器按「没有逐句功能」处理。
 */
function sentenceTimingsOf(raw) {
  if (!audioUrl(raw)) return {};
  const timings = normalizeSentenceTimings(raw?.sentence_timings);
  return timings ? { sentence_timings: timings } : {};
}
...
export function getRealLCRItems() {
  return withRecycled(bankItems(RB_LISTENING_LCR), RB_LISTENING_ALIASES, ["lcr"]).map(mapRealLCR).filter(Boolean);
}
export function getRealLCItems() {
  return withRecycled(bankItems(RB_LISTENING_LC), RB_LISTENING_ALIASES, ["lc"]).map(mapRealLC).filter(Boolean);
}
export function getRealLAItems() {
  return withRecycled(bankItems(RB_LISTENING_LA), RB_LISTENING_ALIASES, ["la"]).map(mapRealLA).filter(Boolean);
}
export function getRealLATItems() {
  return withRecycled(bankItems(RB_LISTENING_LAT), RB_LISTENING_ALIASES, ["lat"]).map(mapRealLAT).filter(Boolean);
}

/** id 尾巴 `_<module>_<题号>`（real_lcr_121b_2_03 / real_lcr_rf0610_1_12）→ [module, 题号]；认不出排到最后。 */
function lcrPosition(id) {
  const m = /_(\d+)_(\d+)$/.exec(String(id || ""));
  return m ? [Number(m[1]), Number(m[2])] : [Infinity, Infinity];
}

/**
 * LCR 真题按「考试日期」打包成套：同一天考出来的应答题 = 一套（同日 A/B/C 卷也并进同一套），
 * 整套交给 LCRTask 的 batchItems 连做、做完一次出分 —— 单句应答一题只有十几秒，一题一套没法练。
 * 套内顺序 = 卷次 → Module → 题号，与原卷作答顺序一致。
 * 套 id 用日期拼、不用序号：以后补进更早日期的卷，老套的 id 不会整体错位。
 */
export function getRealLCRSets() {
  const groups = new Map();
  for (const it of getRealLCRItems()) {
    const key = it.date || "undated";
    if (!groups.has(key)) groups.set(key, []);
    groups.get(key).push(it);
  }
  return [...groups.entries()]
    .sort(([a], [b]) => (a === "undated") - (b === "undated") || a.localeCompare(b))
    .map(([key, list]) => {
      const items = [...list].sort((a, b) => {
        const [ma, qa] = lcrPosition(a.id);
        const [mb, qb] = lcrPosition(b.id);
        return a.source.localeCompare(b.source) || ma - mb || qa - qb || a.id.localeCompare(b.id);
      });
      // 源料缺陷按 code 去重合并：套里任何一卷是重排版，选题卡就该亮「答案经双票复核」。
      const flags = new Map();
      items.forEach((it) => it.source_flags.forEach((f) => flags.has(f.code) || flags.set(f.code, f)));
      return {
        id: `real-lcr-set-${key}`,
        real: true,
        tier: REAL_LISTENING_TIER,
        date: items[0].date,
        sources: [...new Set(items.map((it) => it.source).filter(Boolean))],
        source_flags: [...flags.values()],
        items,
      };
    });
}
...
/**
 * 复述题的场景插图（可选字段，抠图脚本写回题库）：
 *   scene_image:     { url, w, h, source_page }                  —— 无高亮底图
 *   sentence_frames: [ { sentence_id, n, url, w, h } ]           —— 逐句高亮帧
 *
 * **对齐一律靠 sentence_id**（题库里该句的 id），n 只是真题题号、仅留档不参与匹配：
 * 句子 id 的 `_s<k>` 后缀**不等于**真题题号（3.15 / 4.20 / 5.23 三套录入时丢了靠前的句子、
 * 剩下的又连号重排），拿后缀去对题号会整体错位、把高亮挂到别的句子上。
 * 形状不对（不是对象 / url 不是非空字符串 / sentence_id 不是非空字符串）一律当没有 ——
 * 宁可不显示图，也不能把半个对象喂给 <img> 渲染出破图。
 *
 * URL 一律改写成同源 /api/img/…（materialImageSrc）：桶是同一个 real_bank_images，
 * 国内直连 supabase.co 常常不通（阅读材料原图踩过同一个坑）。
 */
function sceneImageSize(raw) {
  const w = Number(raw?.w);
  const h = Number(raw?.h);
  return {
...
function mapSceneImage(raw) {
  if (!raw || typeof raw !== "object" || Array.isArray(raw)) return null;
  const url = typeof raw.url === "string" ? raw.url.trim() : "";
  if (!url) return null;
  const src = materialImageSrc(url);
  if (!src) return null;
  const page = String(raw.source_page || "").trim();
  return { url: src, ...sceneImageSize(raw), ...(page ? { source_page: page } : {}) };
}

function mapSentenceFrames(raw) {
  if (!Array.isArray(raw)) return [];
  const out = [];
  const seen = new Set();
  for (const f of raw) {
    if (!f || typeof f !== "object" || Array.isArray(f)) continue;
    // sentence_id 是唯一的对齐键：必须是非空字符串，重复的只认第一条。
    const sentenceId = typeof f.sentence_id === "string" ? f.sentence_id.trim() : "";
    ...
    if (!src) continue;
    seen.add(sentenceId);
    // n 仅留档（真题题号）：不是正整数就整个省掉，绝不写 NaN / 字符串进去。
    const n = f.n;
    const keepN = typeof n === "number" && Number.isInteger(n) && n > 0;
    out.push({ sentence_id: sentenceId, ...(keepN ? { n } : {}), url: src, ...sceneImageSize(f) });
  }
  // 顺序原样保留（写库脚本按句子顺序写）；前端按 sentence_id 查，顺序只影响预加载。
  return out;
}

/** Repeat：逐句 {id, sentence, audio_url}；句子文本是 STT 打分的参照，缺了整套作废。 */
export function mapRealRepeatSet(raw) {
  const id = readingId(raw?.id);
  ...
  const sceneImage = mapSceneImage(raw?.scene_image);
  const sentenceFrames = mapSentenceFrames(raw?.sentence_frames);
  return {
    id,
    scenario: String(raw?.scenario || "").trim(),
    speaker_role: String(raw?.speaker_role || "").trim(),
    topic: String(raw?.topic || "").trim(),
    sentences,
    // 没图的套（绝大多数）不带这两个键 —— 前端据此一个节点都不渲染。
    ...(sceneImage ? { scene_image: sceneImage } : {}),
    ...(sentenceFrames.length ? { sentence_frames: sentenceFrames } : {}),
    ...listeningProvenance(raw),
  };
}
...
export function getRealRepeatSets() {
  return withRecycled(bankItems(RB_SPEAKING_REPEAT), RB_SPEAKING_ALIASES, ["repeat"]).map(mapRealRepeatSet).filter(Boolean);
}
export function getRealInterviewSets() {
  return withRecycled(bankItems(RB_SPEAKING_INTERVIEW), RB_SPEAKING_ALIASES, ["interview"]).map(mapRealInterviewSet).filter(Boolean);
}
...
/**
 * LCR 是按考试日期打包的整套（getRealLCRSets），一张卡 = 一天。
 * 日期已经在卡片第二行，tag 再放日期只会撑出一排「(1)」的筛选 pill —— 所以 tag 放来源分档
 * （全库都是回忆版 = 只有一类，TopicPicker 自动不渲染筛选栏）；标题带题数，与造句整卷卡同款。
 */
export function mapRealLCRSetsToPicker(sets) {
  return (Array.isArray(sets) ? sets : [])
    .filter((s) => Array.isArray(s?.items) && s.items.length > 0)
    .map((s, i) => ({
      // 整套卡：套里只要有一道是跨场重出的，卡片就该标出来（__anyRecycled 只给 compactCard 看）
      ...compactCard({ ...s, __anyRecycled: s.items.some((it) => it.recycled_of) }, i),
      title: `第 ${i + 1} 套 · ${s.items.length} 题${s.items.some((it) => it.recycled_of) ? ` · 含${REAL_RECYCLED_BADGE}` : ""}`,
      tag: realTierLabel(s.tier),
    }));
}
