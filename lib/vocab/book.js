...
export function introducedToday(cards, now = new Date()) {
  ...
  return activeCards(cards).filter((c) => {
    return [c.introducedAt, c.listeningState?.introducedAt].some((at) => {
      if (!at) return false;
      const d = new Date(at);
...
export function bookStats(cards, now = new Date(), limits = DEFAULT_LIMITS, mode = null) {
  ...
  const newQuota = Math.max(0, lim.newPerDay - introducedToday(all, now));
  const freshAll = all.map((c) => reviewCard(c)).filter((c) => c.state === STATE.NEW && (!mode || c.reviewMode === mode))
    .sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt)).slice(0, newQuota);
...
export function buildQueue(cards, now = new Date(), limits = DEFAULT_LIMITS, rand = Math.random, mode = null) {
  const lim = { ...DEFAULT_LIMITS, ...limits };
  const all = activeCards(cards).map((c) => reviewCard(c));
  const live = mode ? all.filter((c) => c.reviewMode === mode) : all;
  ...
  const quota = Math.max(0, lim.newPerDay - introducedToday(cards, now));
  const fresh = all
    .filter((c) => c.state === STATE.NEW && (!mode || c.reviewMode === mode))
 ...
 *   recall    默认产出卡（中→英）：给中文释义 + 挖空的原句，让你拼出英文词
 *
...
export function clozeSentence(card) {
  const re = card?.word ? wordRe(card.word) : null;
  if (!re) return null;
  // 产出卡填写的是词条本身；句中即使是 divides，答案仍是 divide。
