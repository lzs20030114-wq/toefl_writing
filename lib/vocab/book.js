...
export const CARD_FIELDS = [
  "word", "display", "phonetic", "def", "tag", "sentence", "source", "productive",
...
export function normalizeCard(raw, now = new Date()) {
  ...
  return {
    ...
    // 旧卡片没有这个字段，也按「需要会写」处理；只有手动剔除才存 false。
    productive: raw.productive !== false,
 ...
 * 这张卡该怎么问。
 *
 *   recall    默认产出卡（中→英）：给中文释义，让你拼出英文词
 *   recognize 手动剔除会写要求后，改走认词卡（英→中）
 *
 * 一个词仍然只有一张卡，不因会写要求而额外排第二张。
 */
export function cardDirection(card) {
  return card?.productive === false ? "recognize" : "recall";
}
