...
import { normalizeCard, mergeCards, activeCards, needsDictFill, DEFAULT_LIMITS, MAX_CONTEXTS, reviewCard, validReviewMode } from "./book";
 ...
 */
export function gradeCard(word, rating, now = new Date(), params, durationMs = null, mode = null) {
  const w = String(word || "").trim().toLowerCase();
  const book = loadBook();
  const idx = book.findIndex((c) => c.word === w && !c.deletedAt);
  if (idx < 0) return null;
  ...
  writeBook(next);
  appendReviewLog(projected, srs, rating, now, durationMs, selected);
  scheduleSync();
...
export async function syncVocabCloud({ force = false } = {}) {
  ...
  try {
    ...
    const push = merged.filter((c) => {
      ...
      if (!r) return true;
      return JSON.stringify(c) !== JSON.stringify(normalizeCard(r));
    });
    if (push.length > 0) {
      // 分批推，避免单个请求体过大被网关拦掉。
