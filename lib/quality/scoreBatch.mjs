...
// The per-bank validators are CommonJS (lib/**/*Validator.js). Load them through
// createRequire so the (ESM) quality scorer can reuse the SAME calibrated-dimension
// checks the merge pipeline already enforces — instead of re-measuring only length.
...
// ── Validator-driven quality (the calibrated dimensions, reused) ─────────
// Runs each bank's real generation-time validator on every staging item and
// scores how well the batch hits the calibration we designed (ETS flavor, answer
// position, turn balance, profile, etc.) — not just length. An item with a hard
// error scores 0 (it would be rejected at merge); warnings dock it a little.
// Returns null for banks that have no validator (discussion/email) so the caller
// can fall back to the legacy length checks for those.
function normValidator(v) {
  v = v || {};
  return { errors: Array.isArray(v.errors) ? v.errors : [], warnings: Array.isArray(v.warnings) ? v.warnings : [] };
}
function runValidator(bankKey, item) {
  try {
    switch (bankKey) {
      case "bs": {
        const v = _require("../questionBank/buildSentenceSchema.js").validateQuestion(item);
        return { errors: [...(v.fatal || []), ...(v.format || [])], warnings: [...(v.content || [])] };
      }
      case "reading-ap": return normValidator(_require("../readingGen/apValidator.js").validateAPItem(item));
      case "reading-rdl-short":
      case "reading-rdl-long": return normValidator(_require("../readingGen/rdlValidator.js").validateRDLItem(item));
      case "reading-ctw": {
        const { processPassage } = _require("../readingGen/cTestBlanker.js");
        const { validateCTWItem } = _require("../readingGen/ctwValidator.js");
        const { item: blanked, error } = processPassage(item, item.id || "ctw_score");
        if (error) return { errors: ["blank: " + error], warnings: [] };
        return normValidator(validateCTWItem(blanked));
      }
      case "listening-lat": return normValidator(_require("../listeningGen/latValidator.js").validateLAT(item));
      case "listening-lc": return normValidator(_require("../listeningGen/lcValidator.js").validateLC(item));
      case "listening-la": return normValidator(_require("../listeningGen/laValidator.js").validateLA(item));
      case "listening-lcr": return normValidator(_require("../listeningGen/lcrValidator.js").validateLCR(item));
      case "speaking-repeat": return normValidator(_require("../speakingGen/speakingValidator.js").validateRepeatSet(item));
      default: return null; // discussion / email have no dedicated validator
    }
  } catch (e) {
    return { errors: ["validator threw: " + (e && e.message || e)], warnings: [] };
  }
}
// Short label for an error/warning message: the part before the first ":" or "("
// (e.g. "correct_is_longest: 3/5 …" → "correct_is_longest"), for aggregation.
function issueKey(msg) {
  return String(msg).split(/[:(]/)[0].trim().slice(0, 28);
}
export function validatorQuality(items, bankKey) {
  const N = items.length;
  if (N === 0) return { score: 0, breakdown: ["无 item"] };
  // Probe the first item: if this bank has no validator, signal fallback.
  if (runValidator(bankKey, items[0]) === null) return null;

  let sum = 0, passCount = 0, failCount = 0, warnedCount = 0;
  const errAgg = {}, warnAgg = {};
  for (const it of items) {
    const r = runValidator(bankKey, it) || { errors: [], warnings: [] };
    const e = r.errors.length, w = r.warnings.length;
    let s;
    // Hard error → 0 (this item would be rejected at merge). Otherwise full credit
    // minus a gentle dock per warning (soft calibration drift — the item still ships).
    if (e > 0) { s = 0; failCount += 1; }
    else { s = Math.max(0.5, 1 - 0.10 * w); passCount += 1; if (w) warnedCount += 1; }
    sum += s;
    for (const m of r.errors) errAgg[issueKey(m)] = (errAgg[issueKey(m)] || 0) + 1;
    for (const m of r.warnings) warnAgg[issueKey(m)] = (warnAgg[issueKey(m)] || 0) + 1;
  }
  const top = (agg) => Object.entries(agg).sort((a, b) => b[1] - a[1]).slice(0, 2).map(([k, v]) => `${k}×${v}`).join(", ");
  const breakdown = [`通过 ${passCount}/${N}`];
  if (failCount) breakdown.push(`拒 ${failCount}(${top(errAgg)})`);
  if (warnedCount) breakdown.push(`警告 ${top(warnAgg)}`);
  // score = rich (warnings included) for display. passRate = errors-only fraction
  // (0-100) — the robust signal the gate and the email's "需要注意" key on, so soft
  // warnings inform the number without falsely forcing an R2 retry.
  return { score: Math.round((sum / N) * 100), passRate: Math.round((passCount / N) * 100), breakdown };
}
...
const BANKS = [
  ...
  { key: "listening-lc",        div: (i) => openingDiversity(i, lcText),              qual: (i) => rangeQuality(i, lcText, 68, 105, "conv words") },
...
export function scoreBatch(rootDir, sessionId, results) {
  ...
  for (const bank of BANKS) {
    ...
    // Prefer validator-driven quality (the calibrated dimensions); fall back to the
    // legacy length checks only for banks without a validator (discussion/email).
    const quality = validatorQuality(items, bank.key) || bank.qual(items);
