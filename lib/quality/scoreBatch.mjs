...
const STAGING_TMPL = {
  ...
  "speaking-interview": "data/speaking/staging/interview-$SESSION.json", // wired 2026-07-09 (§7 P1-11 方案A)
};
...
function readingQuality(items, wordMin, wordMax, paraRange = null) {
  const N = items.length;
  if (N === 0) return { score: 0, breakdown: ["无 item"] };
  let wcOK = 0, paraOK = 0, paraN = 0;
  for (const it of items) {
    const wcNum = readingWordCount(it);
    if (wcNum >= wordMin && wcNum <= wordMax) wcOK += 1;
    if (paraRange) {
      // Paragraph structure lives in METADATA (paragraph_count / paragraphs[]), which only
      // generated items carry — real-anchor items store a flat passage with none. Judging
      // metadata-less items as "0 paragraphs" failed 64/64 real AP anchors (R2 finding ④,
      // 2026-07-09). Graceful degrade: items without the metadata stay OUT of this
      // component's denominator; if no item has it, score on word count alone.
      const hasMeta = it.paragraph_count != null || Array.isArray(it.paragraphs);
      if (hasMeta) {
        paraN += 1;
  ...
  if (paraRange && paraN > 0) {
    const score = (wcOK / N + paraOK / paraN) / 2 * 100;
    return { score: Math.round(score), breakdown: [`词数 ${wcOK}/${N}`, `段落数 ${paraOK}/${paraN}`] };
  }
...
function interviewQuality(sets) {
  // Structural mirror of validateInterviewSet's hard checks (4 questions, all
  // interrogative, INTERVIEW_WORD_RANGES-ish lengths, an intro line) — kept
  // dependency-free like the other quality fns here.
  const N = sets.length || 1;
  let clean = 0;
  for (const s of sets) {
    const qs = Array.isArray(s.questions) ? s.questions : [];
    const four = qs.length === 4;
    const allQ = qs.every((q) => /\?/.test(q.question || ""));
    // validator ranges are 25-50 by position; allow slack at the edges (20-60)
    const lenOK = qs.filter((q) => { const w = wc(q.question || ""); return w >= 20 && w <= 60; }).length >= qs.length - 1;
    const hasIntro = typeof s.intro === "string" && wc(s.intro) >= 8;
    if (four && allQ && lenOK && hasIntro) clean += 1;
  }
  return { score: Math.round(clean / N * 100), breakdown: [`clean sets ${clean}/${N} (4 Qs, interrogative, lengths, intro)`] };
}
...
const BANKS = [
  ...
  // RECALIBRATED 2026-07-09 to anchor p5–p95 (R2 ruler validation: the 05-31 bands mis-killed
  // real anchors — rdl-long 80-150 covered only 22/53 of the RDL sample corpus (median 163!),
  // ctw 60-95 covered 57/75, ap 160-210 covered 54/64. Measured: rdl-long p5=88 p95=218,
  // ctw p5=48 p95=86, ap p5=125 p95=206. rdl-short (38-62) already covered its anchors 4/4.
  { key: "reading-ap",          div: readingDiversity,   qual: (i) => readingQuality(i, 120, 210, [2, 4]) },
  { key: "reading-ctw",         div: readingDiversity,   qual: (i) => readingQuality(i, 45, 95) },
  { key: "reading-rdl-short",   div: rdlDiversity,       qual: (i) => readingQuality(i, 38, 62) },
  { key: "reading-rdl-long",    div: rdlDiversity,       qual: (i) => readingQuality(i, 80, 220) },
  ...
  { key: "speaking-interview",  div: (i) => openingDiversity(i, (x) => (x.questions?.[0]?.question || x.topic)),    qual: interviewQuality },
];
