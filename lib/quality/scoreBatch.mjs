...
function emailQuality(items) {
  const N = items.length;
  if (N === 0) return { score: 0, breakdown: ["无 item"] };
  let lenOK = 0, goalsOK = 0, dirOK = 0, recipOK = 0, subjOK = 0;
  for (const it of items) {
    ...
    if (goals.length === 3) {
      ...
      // 2026-07-11: 真题 ~1/10 允许重复动词,只拒"全同",不再强制三动词全不同
      if (verbs.size >= 2 && !verbs.has("tell")) goalsOK += 1;
    }
    if (/^Write an email/i.test(String(it.direction || "").trim())) dirOK += 1;
    // 2026-07-11 范式轴:收件人两式(Title+Surname / first name,真题仅此两式)+
    // 主题行 2-5 词名词组(真题均值 4.1,never 8+)
    const to = String(it.to || "");
    if (/^(Mr|Ms|Mrs|Dr|Prof(essor)?)\.?\s+[A-Z][a-zA-Z'-]+$/.test(to) || /^[A-Z][a-z]+$/.test(to)) recipOK += 1;
    const sw = wc(it.subject);
    if (sw >= 2 && sw <= 5) subjOK += 1;
  }
  const score = (lenOK + goalsOK + dirOK + recipOK + subjOK) / (N * 5) * 100;
  return {
    score: Math.round(score),
    breakdown: [`场景词数 ${lenOK}/${N}`, `动词规范 ${goalsOK}/${N}`, `格式 ${dirOK}/${N}`, `收件人两式 ${recipOK}/${N}`, `主题行2-5词 ${subjOK}/${N}`],
  };
}
...
// LA 范式质量 (2026-07-10 校准): 长度 55-115 + 零定式短语(真题 78 篇 0 命中,精度 1.0
// 的合成指纹检测) + salutation 开场不过半(真题 ~20%,旧库 75% 是主要机器味来源)。
const LA_STOCK_PHRASES = /this is a (friendly )?reminder that|light refreshments|i'?m pleased to announce|we'?re (excited|thrilled) to announce/i;
const LA_SALUTATION = /^(attention|good (morning|afternoon|evening)|hello|hi\b|greetings|welcome)/i;
function laQuality(items) {
  const N = items.length || 1;
  let clean = 0, salut = 0;
  for (const it of items) {
    const t = (it.announcement || it.transcript || "").trim();
    const w = wc(t);
    if (LA_SALUTATION.test(t)) salut += 1;
    if (w >= 55 && w <= 115 && !LA_STOCK_PHRASES.test(t)) clean += 1;
  }
  const salutOK = salut <= Math.ceil(N / 2);
  const score = Math.round((clean / N) * (salutOK ? 100 : 70));
  return { score, breakdown: [`clean ${clean}/${N} (55-115w, no stock phrase)`, `salutation openers ${salut}/${N}${salutOK ? "" : " <- >50%, 扣30"}`] };
}
...
// AP 范式质量 (2026-07-10 校准): 基础长度/题数检查 + 末句 However 检测。
// 真题末句含 However 仅 1/64 (1.6%),生成库曾 45.8% — 模板坍缩指纹,精度 1.0。
// 单批(通常3条)最多允许 1 条 However 收尾。
function apQuality(items) {
  const base = readingQuality(items, 160, 210, [2, 4]);
  const N = items.length || 1;
  const lastSent = (t) => { const s = String(t || "").trim().split(/(?<=[.!?])\s+/); return s[s.length - 1] || ""; };
  const howeverEnders = items.filter((i) => /\bhowever\b/i.test(lastSent(i.passage))).length;
  const ok = howeverEnders <= Math.ceil(N / 3);
  const score = Math.round(base.score * (ok ? 1 : 0.7));
  return { score, breakdown: [...base.breakdown, `however-ending ${howeverEnders}/${N}${ok ? "" : " <- >1/3, 扣30"}`] };
}
...
const BANKS = [
  ...
  { key: "reading-ap",          div: readingDiversity,   qual: apQuality },
  ...
  { key: "listening-la",        div: (i) => openingDiversity(i, (x) => x.announcement), qual: laQuality },
