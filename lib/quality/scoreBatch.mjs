...
function bsDiversity(items) {
  ...
  const personPrefilledCount = items.filter((it) => {
    const pf = Array.isArray(it.prefilled) ? it.prefilled : [];
    return pf.length > 0 && isPersonPrefilled(pf[0]);
  }).length;
  ...
  else if (personFrac >= 0.10) personScore = 15;     // healthy TPO-like band
  else personScore = 10;                              // too low — mild penalty

  // 7 axes: 15+15+15+15+12+13+15 = 100
  const total = nameScore + scenarioScore + typeScore + openerScore + pfSpreadScore + pfUniformityScore + personScore;
