...
// v2 (2026-06-16): bumped when the BS bank was replaced with the gate-clean corpus.
// The new corpus uses set_id 1-20, which collide with the old bank's 1-29 done-progress;
// versioning the key resets "which sets are done" (history is preserved separately via
// per-session snapshots) so users aren't shown brand-new same-numbered sets as completed.
const BS_DONE_KEY = "toefl-bs-done-sets-v2";
export const DONE_STORAGE_KEYS = {
  ...
  // AP was long served by a hardcoded fallback string in app/reading/page.js;
  // the value here MUST stay identical so existing progress isn't orphaned.
  READING_AP: "toefl-reading-ap-done",
  LISTENING_LCR: "toefl-listening-lcr-done",
  // Per-subtype done keys — the practice picker used to hardcode LCR for all four
  // listening subtypes (la/lc/lat practice was a dead end + shared one progress set).
  // LCR keeps its existing key (no migration); la/lc/lat get their own.
  LISTENING_LA: "toefl-listening-la-done",
  LISTENING_LC: "toefl-listening-lc-done",
  LISTENING_LAT: "toefl-listening-lat-done",
 ...
 * - Returns the questions in that set (size varies — typically 9–10)
 */
export function selectBSQuestions(options = {}) {
  ...
  if (!Array.isArray(chosen.questions) || chosen.questions.length < 1 || chosen.questions.length > 20) {
    throw new Error(`Build sentence set ${chosen.set_id} has an invalid question count (${chosen.questions?.length}).`);
  }
