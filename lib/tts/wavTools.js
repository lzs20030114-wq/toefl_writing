 ...
 * Also reports where every segment landed in the output — `segments[i]` is the
 * i-th input's { start, end } in SECONDS from the head of the stitched WAV (the
 * gap after it excluded). That is what lets a sentence-per-call render keep
 * sentence timings (renderListening → item.sentence_timings) for free: the
 * offsets are exact sample counts, no alignment needed.
 * @param {Buffer[]} wavBuffers
 * @param {{ gapMs?: number, normalizeSegments?: boolean }} [opts]
 * @returns {{ wav: Buffer, sampleRate: number, channels: number, segments: Array<{ start: number, end: number }> }}
 */
function concatWavSegmentsTimed(wavBuffers, opts = {}) {
  ...
  const perSec = sampleRate * channels; // interleaved samples per second
  const segments = [];
  let pos = 0;
  for (let i = 0; i < segs.length; i++) {
    const start = pos;
    all.set(segs[i], pos); pos += segs[i].length;
    segments.push({ start: start / perSec, end: pos / perSec });
    if (i < segs.length - 1) { all.set(gap, pos); pos += gap.length; }
  }
  return { wav: buildWav(all, sampleRate, channels), sampleRate, channels, segments };
}

/** Same as concatWavSegmentsTimed but returns only the stitched WAV. */
function concatWavSegments(wavBuffers, opts = {}) {
  return concatWavSegmentsTimed(wavBuffers, opts).wav;
}
...
// Sentence splitting for TTS: one call per sentence (see renderListening). A period is a boundary
// only when it really ends a sentence — "seven a.m.", "Dr. Lee", "U.S.", "e.g.", "$4.50", "www.x.com"
// used to be chopped into fragments, each rendered separately with a gap ("seven a … m").
// Rules, in order: no boundary when the period sits inside a token (next char is a letter/digit with
// no whitespace); no boundary after a single-letter initial ("J. Smith"); title-style abbreviations
// (Dr./Mr./e.g./…) never end a sentence; a.m./p.m./etc./U.S. end one only when followed by
// whitespace + a capital/quote/digit. Closing quotes/brackets right after the terminator stay with
// the sentence they close.
const NEVER_ENDS = new Set(["dr", "mr", "mrs", "ms", "prof", "st", "mt", "no", "vs", "e.g", "i.e", "fig", "vol", "dept", "approx"]);
const MAY_END = new Set(["a.m", "p.m", "etc", "u.s", "u.k", "jr", "sr", "inc", "ltd", "co", "ft", "ave"]);
const CLOSERS_RE = /^[”"'’)\]]+/;
function splitSentences(text) {
  ...
  while ((m = re.exec(s))) {
    ...
    if (m[0] === ".") {
      if (/^[A-Za-z0-9]/.test(after)) continue;                       // a.m / U.S / 4.50 / www.example
      const lastWord = (before.match(/[A-Za-z]+(?:\.[A-Za-z]+)*$/) || [""])[0].toLowerCase();
      if (lastWord.length === 1 && /^\s+[A-Z]/.test(after)) continue; // initial: "J. Smith"
      if (NEVER_ENDS.has(lastWord)) continue;
      if (MAY_END.has(lastWord) && !/^\s+[“"'‘(\[]?[A-Z0-9]/.test(after)) continue; // "7 a.m. tomorrow"
    }
    const closers = after.match(CLOSERS_RE);
    if (closers) end += closers[0].length;
...
module.exports = { parseWav, normalize, silence, buildWav, concatWavSegments, concatWavSegmentsTimed, splitSentences };
