...
// ── Safe voice palette ──
// The voices gpt-4o-mini-tts actually accepts. marin/cedar are realtime-only and the
// speech API 400-rejects them, so they are NOT here (and have been purged from the
// presets below). The gendered lists hold only ear-confirmed voices; the 4
// unknown-gender voices stay OUT of the gender-locked path until classified.
const SAFE_VOICES = {
  female: ["nova", "shimmer", "coral"],
  male: ["echo", "onyx", "ash"],
  neutral: ["sage"],
  unknown: ["alloy", "ballad", "fable", "verse"],
};
const ALL_SAFE_VOICES = Object.freeze([
  ...SAFE_VOICES.female,
  ...SAFE_VOICES.male,
  ...SAFE_VOICES.neutral,
  ...SAFE_VOICES.unknown,
 ...
 * @param {object} opts
 * @param {string} [opts.voice="nova"] — voice ID (must be a SAFE_VOICES entry)
...
async function generateSpeech(text, opts = {}) {
  ...
  if (preset && VOICE_PRESETS[preset]) {
    ...
    voice = voiceOverride || SAFE_VOICES.female[0]; // 'nova' — safe default (marin is 400-rejected)
...
module.exports = { generateSpeech, generateConversation, estimateCost, VOICE_PRESETS, SAFE_VOICES, ALL_SAFE_VOICES };
