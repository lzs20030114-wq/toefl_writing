 ...
 *     - en-US-SaraNeural       — young, casual, student-like
...
async function generateSpeech(text, opts = {}) {
  ...
  // Edge TTS writes to a temp file, read it back as buffer
  const tmpDir = path.join(require("os").tmpdir(), "edge-tts-" + Date.now());
  fs.mkdirSync(tmpDir, { recursive: true });
  const tmpFile = path.join(tmpDir, `audio.${format}`);

  await tts.toFile(tmpFile);
  const buffer = fs.readFileSync(tmpFile + (format === "mp3" ? ".mp3" : format === "wav" ? ".wav" : ".webm"));
