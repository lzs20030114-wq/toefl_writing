"use client";
import { normalizeWord, naiveStems, shardOf, resolveFromShard, isThinEntry, pickLemma } from "./core";
...
export async function lookupWord(raw) {
  ...
  if (direct) {
    // 命中的是 ECDICT 给屈折形单收的「薄条目」（varying → `[计] 改变`，没音标没词性）。
    // 这种条目挡在原形前面，学生看到的就只剩一句看不懂的专业释义；找得到原形就换成原形，
    // 和「查 studies 显示 study」是同一条约定。找不到原形才退回薄条目。
    const lemma = pickLemma(shard, w);
