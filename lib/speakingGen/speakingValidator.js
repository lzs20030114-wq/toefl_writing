...
const REPEAT_WORD_RANGES = {
  easy:   { min: 4, max: 7 },
  medium: { min: 8, max: 12 },
  // hard max 18 (2026-07-09): real sentences top out at 17-18 words, and the
  // nightly quality monitor already scores 3-18 as the in-range band.
  hard:   { min: 13, max: 18 },
};
...
function validateRepeatSentenceProfile(sentence) {
  ...
  // RECALIBRATED 2026-05-31 — two synthetic tells that are 0/351 in the real bank:
  // (1) the Repeat task NEVER asks the test-taker to repeat a question (0 question marks).
  if (/\?/.test(text)) {
    warnings.push("question_mark: Repeat sentences are statements/imperatives — real items have 0 question marks");
  }
  // (2) no punitive-warning trope ("late returns will result in suspension of privileges").
  if (/\b(will result in|suspension|privileges|incur|penalt|violation)\b/i.test(text)) {
    warnings.push("punitive_warning: real Repeat items have no threat/consequence sentences — use neutral logistics or soft encouragement");
  }
...
function scoreRepeatFlavor(set) {
  ...
  // 2. Length-mix & finish (weight: 0.20) — RECALIBRATED 2026-05-31 to realExam2026.
  // The OLD scorer rewarded a rigid 2/3/2 staircase (easy/easy start + medium 3-5 +
  // hard/hard 6-7). Real is MEDIUM-DOMINANT, usually ONE hard sentence, with a "bumpy
  // middle, long finish" (the LAST sentence is the longest in 92% of real sets, but the
  // middle is NOT monotonic). Reward that instead.
  const diffs = sentences.map(s => s.difficulty);
  const lenArr = sentences.map(s => wc(s.sentence || ""));
  let progScore = 0;
  if (diffs.length >= 5) {
    const mediumCount = diffs.filter(d => d === "medium").length;
    const hardCount = diffs.filter(d => d === "hard").length;
    const lastIsLongest = lenArr.length > 1 && lenArr[lenArr.length - 1] >= Math.max(...lenArr);
    if (mediumCount >= Math.ceil(diffs.length * 0.45)) progScore += 0.4; // medium-dominant (~50%)
    if (hardCount >= 1 && hardCount <= 2) progScore += 0.3;              // usually 1, at most 2
    if (lastIsLongest) progScore += 0.3;                                // long finish (real 92%)
  }
  ...
  // 6. Natural spoken register (weight: 0.10) — RECALIBRATED 2026-05-31.
  // The OLD scorer rewarded contractions + direct address linearly (maximize both). Real
  // items barely contract (~1.7%) and use you/your in only ~37% of sentences. So: do NOT
  // reward contractions, and reward an address rate NEAR 0.37 (over-addressing is a tell).
  ...
  // triangular: 1.0 at addrRate=0.37, declining to 0 by distance 0.45
  scores.natural_spoken_register = Math.max(0, 1 - Math.abs(addrRate - 0.37) / 0.45);
...
function validateRepeatSet(set) {
  ...
  // Difficulty distribution check — RECALIBRATED 2026-07-09: real sets are
  // medium-dominant with usually ONE hard sentence (dominant signatures 2/4/1,
  // 3/3/1, 1/4/2, 1/5/1); a rigid 2/3/2 is only ~6% of real sets. The old check
  // warned unless the count was exactly 2/3/2, flagging every well-formed set.
  const diffs = sentences.map(s => s.difficulty);
  const easyCount = diffs.filter(d => d === "easy").length;
  const medCount = diffs.filter(d => d === "medium").length;
  const hardCount = diffs.filter(d => d === "hard").length;
  if (sentences.length === 7 && (medCount < 3 || hardCount > 2)) {
    warnings.push(`difficulty_distribution: got easy=${easyCount} medium=${medCount} hard=${hardCount} (real sets are medium-dominant: medium>=3, hard<=2)`);
  }
...
/**
 * @param {object} q
 * @param {{realExam?: boolean}} [opts] — realExam 把词数下限从 10 放到 5：10 这条线是照着
 *   生成库「一道自足的面试题该有多长」定的，真题不是那样。data/realExam2026 的 91 条原卷
 *   面试句里 21 条不到 5 词、29 条在 5~9 词，"What is your opinion and why?"（6 词）、
 *   "Do you agree or disagree with this viewpoint? Why?"（9 词）都是完整题面。
 *   更短的（"Why?" / "What was it?"）不靠放宽下限救 —— recall_speaking 的 mergeShortGroups
 *   会把它们并进上一题，那本来就是同一道题的追问。
 */
function validateInterviewQuestionSchema(q, opts = {}) {
  const errors = [];
  ...
  const words = wc(text);
  const minWords = opts.realExam ? 5 : 10;
  if (words < minWords || words > 60) {
    errors.push(`word_count: ${words} (absolute range ${minWords}-60)`);
  }
 ...
 * @returns {{ valid: boolean, errors: string[], warnings: string[], flavor: object|null }}
 */
function validateInterviewSet(set, opts = {}) {
  const errors = [];
  const warnings = [];
  ...
  for (let i = 0; i < questions.length; i++) {
    const schemaErrors = validateInterviewQuestionSchema(questions[i], opts);
    if (schemaErrors.length > 0) {
