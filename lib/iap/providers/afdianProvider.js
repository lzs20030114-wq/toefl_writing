  ...
  async verifyWebhook({ headers, rawBody }) {
    ...
    try {
      ...
      if (result.ec === 200) {
        ...
        console.error(`[afdian-webhook] Afdian API returned ec=${result.ec}, rejecting webhook (fail-closed)`);
      ...
      console.error(`[afdian-webhook] Order verification failed (${e.message}), rejecting (fail-closed)`);
