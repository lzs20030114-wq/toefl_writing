...
export const afdianProvider = {
  ...
  async verifyWebhook({ headers, rawBody }) {
    ...
    // Verify order exists on Afdian via query-order API (anti-forgery).
    // Fail-open: if API call fails (network etc.), allow the webhook through
    // so real users aren't blocked. Only reject if API confirms order not found.
    ...
    if (!token || !userId) {
      console.warn("[afdian-webhook] AFDIAN_API_TOKEN or AFDIAN_USER_ID not set, skipping order verification");
      return;
    }
    ...
    try {
      ...
      if (result.ec === 200) {
        ...
        if (orders.length === 0) {
          console.error(`[afdian-webhook] Order ${orderId} NOT FOUND on Afdian — likely forged`);
        ...
        if (Math.abs(webhookAmount - realAmount) > 0.01) {
          console.error(`[afdian-webhook] Amount mismatch for ${orderId}: webhook=${webhookAmount}, afdian=${realAmount}`);
          throw new IapError("IAP_AMOUNT_MISMATCH", "Order amount mismatch", 403);
        }
        console.log(`[afdian-webhook] Order ${orderId} verified on Afdian (¥${realAmount})`);
      } else {
        // API returned an error (e.g. bad token, rate limit) — fail-open
        console.warn(`[afdian-webhook] Afdian API returned ec=${result.ec}, allowing webhook (fail-open)`);
      }
    } catch (e) {
      if (e instanceof IapError) throw e; // re-throw our own errors (forged/mismatch)
      // Network error, timeout, etc. — fail-open so real users aren't blocked
      console.warn(`[afdian-webhook] Order verification failed (${e.message}), allowing webhook (fail-open)`);
    }
  },
