...
// Constant-time signature comparison to avoid leaking the secret via timing.
// Length check first because timingSafeEqual throws on unequal buffer lengths;
// the expected sign is a fixed-length MD5 hex so length is not itself secret.
  ...
  parseWebhookEvent(rawBody) {
    ...
    return {
      ...
      payload: {
        ...
        purchasedAt: payTime,
        days,
        // Amount the user actually paid, in yuan (same unit checkout sends:
        // price = product.priceCents / 100). The service layer reconciles this
        // against the catalog price before granting anything.
        paidAmount: payPrice,
      },
    };
  },
