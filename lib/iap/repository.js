...
async function isEventProcessedSupabase(provider, eventId) {
  const { data, error } = await supabaseAdmin
    .from("iap_webhook_events")
    .select("id")
    .eq("provider", String(provider || "unknown"))
    .eq("event_id", String(eventId || ""))
    .limit(1);
  if (error) throw new Error(error.message || "Failed to check webhook event");
  return Array.isArray(data) && data.length > 0;
}

function isEventProcessedMemory(provider, eventId) {
  const store = getMemoryStore();
  const key = `${String(provider || "unknown")}:${String(eventId || "")}`;
  return store.processedEvents.has(key);
}
...
/**
 * Read-only check: has this (provider, eventId) already been recorded as
 * processed? Used for webhook idempotency WITHOUT committing the processed
 * marker — the marker is only written after the entitlement is granted.
 */
export async function isWebhookEventProcessed(provider, eventId) {
  if (isSupabaseAdminConfigured) return isEventProcessedSupabase(provider, eventId);
  return isEventProcessedMemory(provider, eventId);
}
