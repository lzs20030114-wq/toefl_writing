 ...
 */
async function upgradeTierAfterPurchase(userCode, months) {
  ...
  expiresAt.setDate(expiresAt.getDate() + (months || 1) * 30);
...
async function processCompletedCheckout({ userCode, productId, provider, providerRef, metadata }) {
  ...
  const months = metadata?.month || (productId.includes("yearly") ? 12 : 1);
  await upgradeTierAfterPurchase(userCode, months);
