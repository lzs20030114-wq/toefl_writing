...
import { lookupUserTier } from "../userLookup";
...
async function upgradeTierAfterPurchase(userCode, days) {
  ...
  const { user, error: lookupError } = await lookupUserTier(userCode);

  // 2026-09-13: 查库失败必须抛,不能当成「没这个人」。handleWebhook 只在
  // processCompletedCheckout 成功返回之后才写 processed 标记,抛出去 → 路由按
  // status 503 回非 2xx → provider 重试,用户付的 Pro 不会丢。旧写法把 DB 抖动
  // 静默 return 掉,webhook 照样 2xx:钱收了、权益没发、provider 也不会再来。
  if (lookupError) {
    log.error("User lookup failed for tier upgrade — webhook will retry", {
      userCode,
      error: lookupError.message || String(lookupError),
    });
    throw new IapError("IAP_USER_LOOKUP_FAILED", "User lookup failed; provider should retry", 503);
  }

  if (!user) {
    // 确认查无此人:码不存在是永久状态,重试也不会变 —— 保持原有「记日志放过」语义。
...
export async function handleWebhook({ headers, rawBody }) {
  ...
  if (event.eventType === "checkout.completed") {
    ...
    // Snapshot the money actually paid (yuan: xorpay=pay_price, afdian=total_amount,
    // both provider-verified upstream). Falls back to the catalog price in effect at
    // grant time, so admin revenue stats stay correct across future price changes
    // without a hardcoded price-era table (see lib/iap/paidStats.js).
    const paidYuan = Number(event.payload?.paidAmount ?? event.payload?.totalAmount);
    const amountCents = Number.isFinite(paidYuan) && paidYuan > 0
      ? Math.round(paidYuan * 100)
      : product.priceCents;
