...
const CTW_PROFILE = Object.freeze({
  ...
  // Blank word frequency distribution (from 60 real ETS blanks)
  // 50% easy (high-freq function/daily words), 30% medium (academic), 20% hard (rare)
  blankWordFrequency: { easyPct: 0.50, mediumPct: 0.30, hardPct: 0.20 },
  // Average blank word length target (real ETS mean: 5.5 chars)
  blankAvgLength: { min: 4.0, max: 6.5, target: 5.5 },
  // Max number of blanks with ≥8 characters per item
  maxLongBlanks: 3,
...
const AP_PROFILE = Object.freeze({
  ...
  // Passage metrics — RECALIBRATED 2026-05-31 to realExam2026 (39 clean real
  // 2026改后 AP passages: mean 182.5, median 189, max 209; ~16.6 words/sentence).
  // OLD {150,400,250} was a STALE classic-TOEFL number — real is much shorter.
  passageWordCount: { min: 150, max: 210, target: 190 },
  paragraphCount: { min: 2, max: 4, target: 3 },
  sentenceCount: { min: 8, max: 14, target: 11 },

  // Readability
  fleschKincaidGrade: { min: 11, max: 18, target: 15 },
  typeTokenRatio: { min: 0.57, max: 0.73, target: 0.63 },

  // Question type distribution — RECALIBRATED 2026-05-31 to realExam2026 (hand-coded
  // from 14 OCR clusters, n=70). OLD had 0% insert_text/reference and 17%
  // paragraph_relationship (real 2.9%). insert_text is the Q5 of ~60% of clusters.
  questionTypeTargets: {
    factual_detail: 0.23,
    inference: 0.19,
    vocabulary_in_context: 0.17,
    insert_text: 0.11,
    negative_factual: 0.10,
    rhetorical_purpose: 0.10,
    reference: 0.04,
    paragraph_relationship: 0.03,
    main_idea: 0.01,
  },
...
const ETS_FLAVOR = Object.freeze({
  ...
  // Average word length: 5.63 chars (realExam2026 n=64; gen drifts heavier ~5.88)
  avgWordLength: 5.63,
  // Long words (≥7 chars): ~37.1% (realExam2026; gen drifts to ~40%)
  longWordRatio: 0.371,

  // --- Syntactic ---
  // Passive voice: ~0.10 per sentence (realExam2026, ~1-2 per passage). OLD 0.23 was
  // the stale classic-TOEFL number; the generator over-produces passives 2×.
  passivePerSentence: 0.10,
  ...
  // --- Discourse (RECALIBRATED 2026-05-31 — generator over-produces all of these
  //     2-3×, which reads as a synthetic "too-smooth academic template" tell) ---
  // Hedging: ~1.03 per passage (realExam2026), ≈0.54% of words. OLD 0.0093 (~1.8/
  // passage) over-counted; real concentrates ONE hedge in the final limitation move.
  hedgeRatio: 0.0054,
  // Transitions: real has ~1.09 CONTRAST transitions per passage (ONE strong pivot),
  // ~3-4 transitions total. OLD 8.5 was a stale long-passage number.
  transitionsPerPassage: 3.5,
  transitionProfile: {
  ...
  // Definition patterns: ~1.23 per passage (realExam2026; gen over-produces ~2.1)
  definitionPatternsPerPassage: 1.2,
  ...
  optionRules: {
    ...
    // Option length spread — RECALIBRATED 2026-05-31. Real ETS option spread
    // averages 2.6 words and only ~69% of questions fit within 3 words; the OLD
    // 1.5 over-uniformed options into a synthetic tell. Let lengths vary ~3-4 words.
    optionLengthVarianceMax: 3.5,
  },
  ...
  distractorByQuestionType: {
    ...
    // added 2026-05-31 (realExam2026): insert_text wrong slots break logical flow;
    // reference distractors are other nearby noun phrases.
    insert_text: { primary: "breaks_flow", secondary: "wrong_referent" },
    reference: { primary: "wrong_antecedent", secondary: "too_far" },
  },
  ...
  ctwBlankProfile: {
    ...
    easyBlankRatio: 0.50,
    // Average blank word length (from 60 real blanks): 5.5 chars
    avgBlankWordLength: 5.5,
    // At most 20% of blanks should be rare/domain-specific words
    maxRareWordRatio: 0.20,
  },
  ...
  passageStructure: {
    ...
    // Opening strategy — RECALIBRATED 2026-05-31 to realExam2026 (n=42 unique). Real
    // 2026改后 AP opens by DIRECTLY naming+defining the subject (~90%); the
    // received-wisdom opener ("Historically…", "While early theories…") is 1/42 (~2%).
    // OLD 0.46 received-wisdom was a stale classic-TOEFL value and is now corrected.
    openingStrategies: {
      direct_definition: 0.90,
      received_wisdom_then_revision: 0.05,
      contextual_framing: 0.05,
    },
