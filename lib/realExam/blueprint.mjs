 ...
 *   real_interview_rp0704_1_c2   → interview / rp0704 / 1 / null（chunk 2：拼盘大集按切分表拆出的第 2 套）
...
export function parseRealBankId(id) {
  ...
  if (!slug) return null;
  let chunk = null;
  if ((type === "repeat" || type === "interview") && /^c\d+$/.test(parts[parts.length - 1] || "")) chunk = Number(parts.pop().slice(1));
  ...
  if (type === "repeat" || type === "interview") return { type, slug, module: 1, q: null, seq: nums[0] ?? null, ...(chunk != null ? { chunk } : {}) };
