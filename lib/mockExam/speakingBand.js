...
/**
 * Estimated 1-6 band from a repeat-only (or interview-only) practice session:
 * mean official level (0-5) scaled onto the 0-55 raw structure, then the same
 * linear raw→band mapping the mock exam uses. Returns null for a non-finite mean.
 *
 * @param {number} mean — mean official 0-5 level across the answered items
 * @returns {number|null}
 */
export function levelMeanToBand(mean) {
  if (!Number.isFinite(mean)) return null;
  const clamped = Math.min(5, Math.max(0, mean));
  return rawToSpeakingBand((clamped / 5) * SPEAKING_RAW_MAX);
}
