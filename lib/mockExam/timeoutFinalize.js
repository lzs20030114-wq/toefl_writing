 ...
 *   entry carries `item` and `timedOut: true`. Invariant: the returned array's
 *   summed `total` equals plannedTotal(items) (assuming each completed result's
 *   total already matches its item's scorable count).
 */
export function finalizeTimedOutResults(items, completedResults, collectPartial) {
  ...
  for (let i = startIdx; i < safeItems.length; i++) {
    ...
    finalResults.push({ ...entry, item, timedOut: true });
  }
