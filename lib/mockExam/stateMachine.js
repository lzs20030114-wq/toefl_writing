...
function extractSubmittedScore(attempts, taskId) {
  ...
  // null (not 0) when the task wasn't scored — a failed AI scoring must not be
  // counted as a real 0 in the band. calculateWritingBand excludes null components.
  return a?.status === TASK_STATUS.SUBMITTED && Number.isFinite(a?.score) ? a.score : null;
}
...
function computeAggregate(blueprint, attempts) {
  ...
  for (const item of blueprint) {
    ...
    // A submitted-but-unscored task (score=null after a failed AI scoring) is
    // excluded from the composite so it can't drag percent toward 0. The weight
    // is renormalized over the tasks that were actually scored.
  ...
  // Renormalize by the weight that was actually scored, so excluding a failed
  // task rescales the remaining tasks to 100% instead of capping the percent.
  ...
  return {
    ...
    scoredCount: bandResult.scoredCount,
