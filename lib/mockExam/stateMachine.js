import { DEFAULT_TASK_MAX_SCORES, MOCK_EXAM_STATUS, TASK_STATUS } from "./contracts";
...
export function updateTaskScore(state, taskId, payload) {
  ...
  const score = clampNumber(payload?.score ?? 0, 0, safeMax);
  const next = {
    ...state,
    attempts: {
      ...state.attempts,
  ...
  if (next.status === MOCK_EXAM_STATUS.COMPLETED || next.status === MOCK_EXAM_STATUS.ABORTED) {
