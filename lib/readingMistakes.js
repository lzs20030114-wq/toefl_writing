...
//            { blank, userAnswer, fullWord, isCorrect }. `userAnswer` holds
//            only the letters the user typed (the missing portion);
//            `fullWord` is fragment+userAnswer (the whole attempted word).
...
//
// 真题 AP 的选句题（question_type: "sentence_selection"，契约见 lib/reading/sentenceSelection.js）
// 选项是第 N 段的 S1..Sn 各句、答案是 S 键：错题卡不列 S 键选项，直接给「你选的句子 / 正确句子」。

import { isSentenceSelection, sentenceOptionKeys, sentenceOptionText } from "./reading/sentenceSelection";
...
function buildCtwMistake(detail, result, index) {
  ...
  const blank = result?.blank || blanks[index];
  const stem = ctwBlankContext(blank);
  const correctWord = String(blank?.original_word || "").trim();
  // CTWTask saves `userAnswer` (only the typed missing letters) and `fullWord`
  // (fragment + userAnswer). When the user typed nothing, leave userAnswer
  // empty so the mistake card shows "(未作答)" rather than echoing the prompt.
  const userTyped = String(result?.userAnswer ?? "").trim();
  const fragment = String(blank?.displayed_fragment || "").trim();
  const userAnswer = userTyped
    ? String(result?.fullWord || (fragment + userTyped)).trim()
    : "";
  return {
    stem,
    options: null,
...
function buildSentenceSelectionMistake(q, result, index) {
  const selectedKey = result?.selected ?? null;
  const correctKey = result?.correct ?? q.correct_answer ?? null;
  return {
    kind: "sentence_selection",
    stem: String(q.stem || "").trim(),
    // options 置空 → 错题卡走「你选的句子 / 正确句子」两行（S1..Sn 键对用户没有意义）
    options: null,
    optionsKey: null,
    selected: selectedKey,
    correctKey,
    userAnswer: sentenceOptionText(q, selectedKey),
    correctAnswer: sentenceOptionText(q, correctKey),
    explanation: String(q.explanation || "").trim(),
    type: q.question_type,
    paragraph: Number.isInteger(q.paragraph) ? q.paragraph : null,
    // 该段全文（各句按序拼回）—— 给 AI 讲解当上下文用，长文章截断时第 N 段不会被截掉
    paragraphText: sentenceOptionKeys(q.options).map((k) => sentenceOptionText(q, k)).join(" "),
    _index: index,
  };
}
...
function buildMcqMistake(detail, result, index) {
  ...
  if (isSentenceSelection(q)) return buildSentenceSelectionMistake(q, result, index);
