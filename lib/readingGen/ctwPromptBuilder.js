 ...
 */
// Blank-position word-frequency mix per difficulty tier. This is the only
// knob difficulty turns: it never touches sentence 1 (the gate's frozen
// register dimensions) or passage length/structure. The measured label still
// comes from estimateDifficulty on the actual blanks — this just steers what
// the blanker will land on. "medium" is the long-standing calibrated default.
const FREQ_CONSTRAINTS = {
  easy: `- At least 8-9 out of 10 blank-position words should be COMMON words: function words (that, from, with, their, is, are) or everyday content words (people, trees, water, help, make, find, grow, used, known)
- At most 1-2 blank-position words should be longer academic words (8+ characters)`,
  medium: `- At least 7 out of 10 blank-position words should be COMMON words: function words (that, from, with, their, is, are) or everyday content words (people, trees, water, help, make, find, grow, used, known)
- At most 2-3 blank-position words should be longer academic words (8+ characters)`,
  hard: `- 5-6 out of 10 blank-position words should be COMMON words: function words (that, from, with, their, is, are) or everyday content words (people, trees, water, help, make, find, grow, used, known)
- 4-5 blank-position words should be medium-frequency B2 content words the real exam blanks (like: record, height, majority, established, vegetation, observations) — words a prepared test-taker knows but must think to complete`,
};
...
1. **Length** (realExam2026: 75 real 2026改后 CTW passages, **mean 69**, range 47-94):
   - **Aim for 72-82 words across 5 FULL sentences.** The real-exam mean is 69.
   - ⚠️ Models SYSTEMATICALLY UNDERSHOOT length: a stated "65-78" produces only ~58
     (measured on DeepSeek output 2026-05-31). So AIM HIGH — write FIVE complete
     sentences targeting ~76 words; never stop at 3-4 short ones. A 58-word passage
     is a FAILURE here even though it is technically valid.
   - Mechanical constraint: sentences 2+ combined need ≥20 words for the 10 blanks
     (blanking takes every other word from sentence 2 onward) — five full sentences
     clears this easily.
2. **First sentence**: Always kept intact (never blanked). Make it an interesting, clear
   topic sentence of **~16-17 words with several substantial (7+ letter) words** — real
   first sentences run 16.7 words / 37% long words ("Glaciers are massive, slow-moving
   bodies of ice that form in areas where snow accumulates over time…"). The bank's
   first sentences run short and plain (14.9w) — this is the remaining difficulty gap.
3. **Register**: Write like a popular-science article or an intro college textbook — NOT a journal abstract, but NOT dumbed-down either.
   - Concrete subjects are fine, but use precise academic words where natural.
   - Average word length should be ~5.3-6.0 characters (RECALIBRATED 2026-05-31:
     the real 2026改后 CTW averages 5.7 — the old 4.5-5.5 target was too easy).
4. **Vocabulary level**: B1-B2 as the base; several B2-C1 academic words per passage
   are NORMAL and expected (the old "A2-B1, max 1-2 B2" was below the real exam).
   - The real exam routinely blanks words like: characterized, advantageous,
     conversely, systematically, vegetation, eruptions, observations, established.
   - Still avoid only the truly obscure/technical (meticulously, paradigm, cellular).
...
${FREQ_CONSTRAINTS[difficulty] || FREQ_CONSTRAINTS.medium}
- Avoid putting truly obscure or domain-specific words at blank positions — put them at ODD positions (which stay intact) instead

Think of it this way: the INTERESTING vocabulary goes in the visible positions; the ${difficulty === "hard" ? "common-to-B2" : "COMMON"} vocabulary goes in the blanked positions.
