...
// ── Ending moves (RECALIBRATED 2026-07-10 to realExam2026, n=64) ──
// 真题结尾无单一范式:limitation 仅 ~20%,且末句含 However 只有 1.6%(生成库曾
// 45.8% — 最刺眼的模板坍缩)。五种收尾轮换硬指派,每种 ~20%。
const ENDING_MOVES = [
  { key: "limitation", spec: "End on ONE forward-looking limitation ('However, … faces objections' / 'Despite these efforts, challenges remain'). Use this pivot as the passage's single strong contrast." },
  { key: "application", spec: "End on a practical application or recommendation — where/how the concept is being used or could be used ('Architects now apply these principles when…'). NO 'However' in the final sentence." },
  { key: "research", spec: "End on ongoing research/discovery — what researchers are still uncovering or measuring ('Researchers are continuously uncovering…'). NO 'However' in the final sentence." },
  { key: "outlook", spec: "End on a measured optimistic outlook — what the approach promises or enables next ('…promises even more sophisticated solutions'). NO 'However' in the final sentence." },
  { key: "synthesis", spec: "End by tying the threads together in one concrete synthesis sentence, or simply stop on the last substantive detail — textbook entries often just end. NO 'However' in the final sentence, no grand summary formula." },
];
...
function buildAPPrompt(count = 3, opts = {}) {
  ...
  // Assign ending moves (硬指派轮换 — RECALIBRATED 2026-07-10)。
  // 旧规则(强制 limitation 收尾)被生成器 100% 执行,
  // 造成末句含 However 45.8% vs 真题 1.6% 的模板坍缩;真题结尾实为多亚型
  // (limitation 仅 ~20%,其余为应用/持续研究/展望/综合收束)。
  const endingDeck = [...Array(Math.ceil(count / ENDING_MOVES.length))]
    .flatMap(() => ENDING_MOVES.map((m) => m.key))
  ...
  const passageSpecs = selected.map((s, i) => {
   ...
   the Q5 prompt. The other 3 slots must be plausible-but-wrong.`
      : "";
    const ending = ENDING_MOVES.find((m) => m.key === endingDeck[i]) || ENDING_MOVES[0];
    return `${i + 1}. Topic: ${s.topic} / ${s.subtopic}
   Rhetorical pattern: ${s.rhetorical.name} — ${s.rhetorical.template}
   Ending move (REQUIRED, follow exactly): ${ending.spec}
${diffNote}${insertNote}
...
2. **Structure rules** (MANDATORY):
   ...
   - NO conclusion/summary paragraph. The ENDING MOVE is assigned PER ITEM below —
     follow it exactly. Real passages end in many ways (limitation / application /
     ongoing research / outlook / plain stop); only ~1 in 5 ends on a limitation,
     and the final sentence starts with "However" in fewer than 1 in 20 real
     passages. Do NOT default to a "However, challenges remain" closer.
...
3. **Academic style** (RECALIBRATED — real 2026改后 is SPARING, not lardered; the
   ...
   - **At most ONE** strong contrast pivot (However/Despite) per passage — place it
     where the argument genuinely turns (mid-passage is fine). Passages whose
     assigned ending move is application/research/outlook/synthesis may have ZERO
     pivots; that is natural. Real has at most one pivot, never 2-3.
...
"passage" MUST separate its paragraphs with a blank line (\n\n) and MUST contain exactly the
same paragraphs, in the same order, as the "paragraphs" array. A passage run together without
blank lines is REJECTED: the reader renders it verbatim, and three quarters of the questions
