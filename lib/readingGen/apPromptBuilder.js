...
function buildAPPrompt(count = 3, opts = {}) {
  ...
  const passageSpecs = selected.map((s, i) => {
    ...
    const qSpecs = plan.map((type, qi) =>
      type === "insert_text"
        ? `Q${qi + 1}=insert_text(answer=slot ${letterToSlot[posPool[qi]]} of the four [■])`
...
1. **Length** (RECALIBRATED 2026-05-31 to realExam2026: 39 clean real 2026改后 AP
   ...
   - Target: **150-210 words, center ~190, 3 paragraphs (~60-65 words each)**.
   - Hard ceiling **210 words** — do NOT exceed it. The old "280-360 / mean 317.5"
     was a STALE classic-TOEFL number; the 2026改后 AP passage is much shorter.
