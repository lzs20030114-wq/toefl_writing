...
// 屈折变体：严格判——两词只差一个已知屈折后缀（s/es/ed/d/ing/ies/'s）。
// 关键：不能用「共享前缀」松判，否则 the/these、the/their 会被误判成同词根变体，
// 落进判分宽容白名单 → 把「these」的空判「the」为对（换义错答）。用于分类与自动落项。
const INFLECTIONS = ["s", "es", "ed", "d", "ing", "ies", "'s", "es'"];
function isInflectionalVariant(a, b) {
  ...
  for (const suf of INFLECTIONS) {
    if (long === short + norm(suf)) return true;
  }
  // y→ies（study/studies）：去 y 加 ies
