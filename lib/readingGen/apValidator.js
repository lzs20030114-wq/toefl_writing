...
const { isParagraphLayoutSynced, paragraphBlocks, cleanParagraphs } = require("../reading/passageLayout");

const VALID_QUESTION_TYPES = new Set([
  "main_idea", "factual_detail", "negative_factual",
  "vocabulary_in_context", "inference", "rhetorical_purpose",
  "paragraph_relationship",
  // added 2026-05-31 (realExam2026): the recalibrated prompt generates these.
...
function validateAPItem(item) {
  ...
  // RECALIBRATED 2026-05-31 to realExam2026 (real 71-209, mean 182): was 120-450, a
  // stale classic-TOEFL range that admitted 322-450w passages the 2026改后 exam never has.
  if (passageWc < 110 || passageWc > 230) {
    errors.push(`passage_length: ${passageWc} words (need 110-230; real 2026改后 is 150-210)`);
  }
  ...
  // 段落版面闸（2026-09-18）。passage 与 paragraphs 是模型分别产出的两个字段，对不上时
  // 渲染层（pre-wrap 只认空行）会把整篇糊成一坨 —— 而 AP 的题干四分之三写着「paragraph N」。
  // 合库口 merge-staging 会先跑 restoreParagraphBreaks 把能补的空行补回来，走到这里还对不上的
  // 就是补不了的（段落错序 / 文字对不上），只能拒收让人看。
  if (paragraphBlocks(item.passage).length < 2 && cleanParagraphs(item.paragraphs).length < 2) {
    errors.push("paragraph_layout: passage has no paragraph breaks and no paragraphs[] to restore them from");
  } else if (!isParagraphLayoutSynced(item.passage, item.paragraphs)) {
    errors.push(
      `paragraph_layout: passage splits into ${paragraphBlocks(item.passage).length} paragraph(s) but paragraphs[] has ${cleanParagraphs(item.paragraphs).length} — the two must agree`
    );
  }
  ...
  for (let i = 0; i < item.questions.length; i++) {
    ...
    // insert_text validity (added 2026-05-31) — the sentence to insert MUST have been
    // REMOVED from the passage (real exam), and the passage must expose exactly four [■]
    // markers. Catches the common generation bug (seen in DeepSeek output) where the
    // model leaves the insert sentence in the passage → inserting it would duplicate it.
    if (q.question_type === "insert_text" && typeof q.stem === "string") {
      // capture a prefix of the quoted insert sentence (stop at the next straight quote;
      // apostrophes inside, e.g. "viewer's", naturally cap the prefix — that's fine).
      const m = q.stem.match(/['"]([^'"]{20,})/);
      if (m) {
        const norm = (s) => s.toLowerCase().replace(/[^a-z]+/g, " ").trim();
        const probe = norm(m[1]).split(" ").slice(0, 8).join(" "); // first 8 words, ALL kept
        if (probe.length > 15 && norm(item.passage).includes(probe)) {
          errors.push(`${label}: insert_text sentence is still present in the passage (it must be removed)`);
        }
      }
      const markers = (item.passage.match(/\[■\]/g) || []).length;
      if (markers !== 4) {
        errors.push(`${label}: insert_text requires exactly 4 [■] markers in the passage (found ${markers})`);
      }
    }
  }
  ...
  for (let i = 0; i < item.questions.length; i++) {
    ...
    // insert_text options are positional slot labels ("Location 1…"), not prose —
    // their length is meaningless, so exclude them from the length-balance checks.
    if (q.question_type === "insert_text") continue;
    nonInsertCount++;
    ...
    // 2026-05-29: was 12 — too tight for real TPO which has option spreads
    // of 10-15 words commonly. Relaxed to 15.
    if (maxLen - minLen > 15) {
  ...
  if (correctLongestCount >= 3) {
    errors.push(`correct_is_longest: ${correctLongestCount}/${nonInsertCount} prose questions have correct as longest option — reject`);
  } else if (correctLongestCount >= 2) {
    warnings.push(`correct_is_longest: ${correctLongestCount}/${nonInsertCount} prose questions — borderline`);
  }
