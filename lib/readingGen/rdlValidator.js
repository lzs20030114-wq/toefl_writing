...
function validateRDLItem(item) {
  ...
  // Difficulty — the pipeline measures it via rdlDifficulty.js, so a missing
  // label only warns (legacy/staging/user-bank items), but a present-and-bogus
  // value is an error: the mock-exam planner would silently treat it as "any".
  if (!item.difficulty) {
    warnings.push("missing_difficulty");
  } else if (!VALID_DIFFICULTIES.has(item.difficulty)) {
    errors.push(`difficulty: "${item.difficulty}" invalid (must be easy/medium/hard)`);
  }
...
function validateRDLBatch(items) {
  ...
  // Difficulty distribution — detect label collapse (the pre-2026-07 failure
  // mode: every generated item carried the few-shot placeholder "easy").
  ...
  if (items.length >= 5) {
    const [topTier, topCount] = Object.entries(difficultyDist)
      .sort((a, b) => b[1] - a[1])[0];
    if (topCount / items.length >= 0.9) {
      batchWarnings.push(`difficulty_collapse: ${Math.round(topCount / items.length * 100)}% of batch is "${topTier}"`);
    }
  }

  return { warnings: batchWarnings, answerDistribution: posCounts, difficultyDist };
}
