 ...
 * @param {function} [callAIOverride] — optional (prompt, maxTokens) => Promise<string> injection.
 *   Used by /api/user-bank/verify to route through a request-scoped, proxy-aware DeepSeek client
 *   instead of the built-in curl client (which was written for pipeline scripts). Omitting it
 *   preserves the original behavior exactly (merge-staging pipeline is unaffected).
 * @returns {Promise<object>} audit results
 */
async function auditRDLItem(item, callAIOverride) {
  const call = typeof callAIOverride === "function" ? callAIOverride : callAI;
  const results = [];
  ...
  try {
    const raw = await call(withPassagePrompt);
  ...
  try {
    const raw = await call(withoutPassagePrompt);
...
// buildAnswerPrompt/parseJson are additionally exported for /api/user-bank/verify's
// answer-backfill path (用户没贴答案 → 用 auditor 的独立作答产出 ai_answer 填补)。
module.exports = { auditRDLItem, auditCTWItem, buildAnswerPrompt, parseJson };
