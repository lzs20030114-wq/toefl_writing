...
// ── Difficulty tiers ──
// Operational definitions aligned with lib/readingGen/rdlDifficulty.js (the
// post-generation estimator that assigns the final label): paraphrase
// distance of correct answers, passage-word borrowing in distractors, and
// register. The tier here GUIDES generation; the estimator MEASURES the
// result — same architecture as CTW.
const DIFFICULTY_SPECS = {
  easy: "Correct answers stay close to the passage wording (simple rewording — but still NEVER copy 3+ consecutive words). Distractors are plausible but use LITTLE passage vocabulary, so careful readers can discard them. Use everyday vocabulary in the text.",
  medium: "Correct answers use synonym substitution (swap the key content words for synonyms). At least 1 distractor per question borrows 2-3 real passage terms with a CHANGED relationship. Standard campus vocabulary.",
  hard: "Correct answers SYNTHESIZE information across 2+ sentences or convert formats (e.g. \"11:00 PM to 3:00 AM on Saturday\" → \"from Saturday night until early Sunday morning\"). 2 of 3 distractors borrow real passage terms with changed relationships. Where natural, test conditional/threshold logic (deadlines, fee tiers, limits). A slightly more formal register (longer words) is welcome.",
};

// Default tier mix, tilted toward medium/hard: the pre-2026-07 bank is
// saturated with easy items (the old prompt never assigned difficulty), so
// new generation back-fills the upper tiers until the pools balance.
function assignDifficulties(count, override, { easyShare, hardShare }) {
  const difficulties = [];
  if (override) {
    for (let i = 0; i < count; i++) difficulties.push(override);
    return difficulties;
  }
  const easyCount = Math.round(count * easyShare);
  const hardCount = Math.round(count * hardShare);
...
const DIFFICULTY_BLOCK = `
## DIFFICULTY CALIBRATION (each item below has an assigned difficulty)

- easy: ${DIFFICULTY_SPECS.easy}
- medium: ${DIFFICULTY_SPECS.medium}
- hard: ${DIFFICULTY_SPECS.hard}
`;
...
const QUESTION_TYPE_TEMPLATES = {
  detail: {
    weight: 0.55,
    // 2026-07-10: 补 person-named(真题~16%)与 conditional(真题~18%)题干变体,
    // 打散模板单一化(库里 "What can be inferred about" 曾垄断 32%)。
    stem_patterns: [
      ...
      "If a reader wants to {goal}, what should they do?",
      "What should {named_person} do if {condition}? (use a person named in the text)",
      "By when must {action} be completed?",
    ],
  ...
  inference: {
    weight: 0.28,
    // 同上:降低 "What can be inferred about" 的出现频率 — 每批至多 1 题用该句式,
    // 其余用下方变体。
    stem_patterns: [
      "What can be inferred about {subject}? (use AT MOST ONCE per batch)",
      "What is implied about {aspect}?",
      "What is suggested about the {subject}?",
      "What is probably true about {subject}?",
      "Who would most likely {action}?",
      "Why does the {genre} most likely mention {detail}?",
    ],
 ...
 * @param {string[]} [opts.excludeSubjects] — subject fingerprints already generated (bank + staging); injected as a "do not write about" block
 * @param {string[]} [opts.rejectionFeedback] — reasons previous items were rejected
 * @returns {string} prompt
 */
// ── Short RDL genre specs (subset of GENRE_SPECS with short-specific guidance) ──

const SHORT_GENRE_SPECS = {
  email: {
    label: "Brief Email",
    register: "Semi-formal. 1 contraction max. Greeting (Hi/Dear) + 1-2 sentence body + sign-off.",
    scenarios: [
      "Quick email confirming a tutoring appointment (time + room)",
      "Brief email from RA about package pickup at mailroom",
      "Short email reminder about lab safety training (date + location)",
      "Email confirming research study participation with $15 compensation",
      "Quick email from career services about resume review appointment",
      "Brief advisor email about changed office hours this week",
      "Short email about picking up campus parking permit",
      "Email confirming interlibrary loan book arrival",
      "Brief email about rescheduled club meeting (new time + room)",
      "Quick email about student ID replacement ($10 fee)",
      "Email from dining services about meal plan balance refund",
      "Short email confirming campus tour guide volunteer shift",
    ],
  },
  notice: {
    label: "Short Notice",
    register: "Formal. Passive voice OK. No contractions. Include date/time/location.",
    scenarios: [
      "Elevator out of service notice (dates + alternative)",
      "Parking lot closure for repaving (dates + alternative lot)",
      "Water fountain maintenance closure (floor + duration)",
      "Library hours change for spring break week",
      "Campus recycling bin relocation notice",
      "Wi-Fi maintenance window announcement (date + time + duration)",
      "Bike rack removal notice for sidewalk repair",
      "Vending machine refund process ($2 max, contact info)",
      "Fire alarm testing schedule for residence hall",
      "Lost-and-found unclaimed items disposal deadline",
      "Campus shuttle route detour notice (dates + new stops)",
      "Building HVAC maintenance (temperature warning + dates)",
    ],
  },
  social_media: {
    label: "Social Media Post",
    register: "Informal. 1-2 exclamation marks. Casual campus voice. Brief opinion + 1 fact.",
    scenarios: [
      "Student post recommending campus café new drink ($4.50)",
      "Post about free yoga class in the quad this Saturday",
      "Student recommending quiet study spot in library basement",
      "Post about campus food truck with $3 taco special today",
      "Student sharing campus garden open hours for spring",
      "Post about free printing at computer lab until Friday",
    ],
  },
  flyer: {
    label: "Flyer / Poster",
    register: "Mixed. Attention-grabbing headline + key details. May use fragments for brevity.",
    scenarios: [
      "Campus blood drive flyer (date + location + free T-shirt)",
      "Used textbook sale poster (date + room + price range)",
      "Student theater show flyer ($5 tickets, dates, location)",
      "Volunteer signup flyer for campus cleanup day",
      "Campus photography contest flyer (deadline + prize)",
      "Free flu shot clinic flyer (date + health center location)",
    ],
  },
};

/**
 * Build a prompt to generate short RDL items (40-50 words, 2 questions).
 * These appear in the routing module and easy adaptive module.
 *
 * @param {number} count
 * @param {object} opts
 * @param {string[]} [opts.genres]
 * @param {string[]} [opts.excludeScenarios]
 * @param {string[]} [opts.excludeSubjects] — subject fingerprints already generated (bank + staging); injected as a "do not write about" block
 * @param {string[]} [opts.rejectionFeedback]
 * @param {string} [opts.difficultyOverride] — force all items to one difficulty (easy/medium/hard)
 */
function buildShortRDLPrompt(count = 5, opts = {}) {
  const { genres = [], excludeScenarios = [], excludeSubjects = [], rejectionFeedback = [], difficultyOverride = null } = opts;

  // Cross-batch anti-duplication: subject fingerprints of items already generated
  // (live bank + un-merged staging). Empty when not passed → output unchanged.
  const excludeBlock = excludeSubjects.length > 0
    ? `\n## DO NOT WRITE ABOUT ANY OF THESE ALREADY-USED SUBJECTS/SCENARIOS (already generated — pick clearly different ones):\n${excludeSubjects.slice(0, 40).map((s) => `- ${s}`).join("\n")}\nEach passage MUST be about a subject/scenario clearly different from every entry above.\n`
    : "";

  const genreKeys = Object.keys(SHORT_GENRE_SPECS);
  const selected = [];
  for (let i = 0; i < count; i++) {
    ...
    if (genres.length > 0) {
      ...
      const r = Math.random();
      if (r < 0.40) g = "email";
      else if (r < 0.75) g = "notice";
      else if (r < 0.88) g = "social_media";
      else g = "flyer";
    }
    const spec = SHORT_GENRE_SPECS[g];
    if (!spec) { selected.push({ genre: g, scenario: "Brief campus communication", spec: SHORT_GENRE_SPECS.notice }); continue; }
  ...
  // Difficulty tiers: 20% easy / 45% medium / 35% hard (see DIFFICULTY_SPECS)
  const difficulties = assignDifficulties(count, difficultyOverride, { easyShare: 0.20, hardShare: 0.35 });

  // Question type planning biased by difficulty: easy leans detail-only,
  // medium/hard keep the detail+inference core (hardness there comes from
  // paraphrase distance and distractor traps, per DIFFICULTY_SPECS).
  const questionPlans = selected.map((s, i) => {
    if (difficulties[i] === "easy") {
      return Math.random() < 0.75 ? ["detail", "detail"] : ["detail", "vocabulary_in_context"];
    }
    if (difficulties[i] === "hard") {
      return ["detail", "inference"];
    }
    const r = Math.random();
    if (r < 0.75) return ["detail", "inference"];
    if (r < 0.90) return ["detail", "detail"];
    return ["detail", "vocabulary_in_context"];
  });

  const posPool = [];
  for (let i = 0; i < count * 2; i++) posPool.push(["A", "B", "C", "D"][i % 4]);
  ...
  const itemSpecs = selected.map((s, i) => {
    ...
    const positions = posPool.splice(0, 2);
    const mustHaveMoney = (i % 3 === 0); // 33% match long format target
    return `${i + 1}. Genre: ${s.genre} (${s.spec?.label || s.genre})
   Scenario: ${s.scenario}
   Register: ${s.spec?.register || "Neutral"}
   Difficulty: ${difficulties[i]}${mustHaveMoney ? "\n   MUST INCLUDE a dollar amount (e.g., $5 fee, $10 deposit, $3.50 price)" : ""}
   Questions: Q1=${qTypes[0]}(answer=${positions[0]}), Q2=${qTypes[1]}(answer=${positions[1]})`;
  }).join("\n\n");
  ...
  return `You are a TOEFL "Read in Daily Life" question writer. Generate ${count} SHORT items (40-50 words each, 2 questions per item).

## PASSAGE REQUIREMENTS

1. **Length**: 40-50 words ONLY. These are very brief texts — 2-4 sentences max. Count carefully.
2. **Setting**: 75% campus/university contexts. Must feel like REAL campus communication.
3. **Concrete details**: Every text MUST include at least 2 of these:
   - A specific date or day ("May 15", "this Friday", "March 3-7")
   - A specific time ("9:00 AM", "2:00-4:00 PM")
   - A specific location ("Room 204", "Anderson Hall", "South Lot B")
   - A specific name ("Ms. Chen", "Dr. Park", "Facilities Office")
   - A dollar amount ("$5", "$10 deposit", "$3.50")
4. **Register**: Follow the register guidance for each genre:
   - Emails: greeting + body + sign-off, semi-formal, 1 contraction max
   - Notices: formal, no contractions, passive voice OK, structured
   - Social media: informal, 1-2 exclamation marks, opinion + fact
   - Flyers: headline + key details, fragments OK for impact
5. **Information density**: Even in 40-50 words, pack in 4-6 extractable facts (who, what, when, where, how much, conditions). This is critical for question quality.
6. **Actionability**: 90%+ of texts should tell the reader to DO something (bring, register, contact, visit, pick up).

## QUESTION REQUIREMENTS

1. **2 questions per text**, 4 options (A/B/C/D) each.
2. **Correct answer position**: MUST match assigned position below.
3. **GRAMMATICAL PARALLELISM IS MANDATORY**: All 4 options MUST start with the same grammatical structure.
   ✓ All start with "To + verb": "To register...", "To submit...", "To complete...", "To attend..."
   ✓ All nouns: "A deposit...", "A penalty...", "A refund...", "A receipt..."
   ✗ WRONG: mixing "Pay a fee" / "The library" / "Before Friday" / "To register"
4. **Option balance**: All 4 options similar length (within 3 words of each other). Correct answer must NOT be the longest.
5. **Option semantic category**: All 4 options must be the SAME type (all actions, all locations, all times, etc.). Do NOT mix categories.
6. **Distractor engineering**:
   - "plausible_generic" (~49%): sounds reasonable but not stated in text. Use moderate vocabulary overlap.
   - "uses_passage_words" (~31%): borrow 2-3 real terms, CHANGE their relationship. Example: text says "free parking until 5 PM" → distractor: "free parking is available all day" (wrong relationship).
   - "introduces_new_terms" (~20%): topically related but completely absent from passage.
7. **Correct answer paraphrase**: NEVER copy 3+ consecutive words from the passage. Use synonym substitution or synthesis.
8. **Absolute language trap**: Distractors should NOT use absolutes (all/always/never/only) unless the correct answer does too.
${DIFFICULTY_BLOCK}${excludeBlock}
## ITEMS TO GENERATE
...
Return a JSON array. Each element:
{
  "genre": "notice",
  "variant": "short",
  "text": "Brief text here (40-50 words)...",
  "format_metadata": {},
  "questions": [
    { "question_type": "detail", "stem": "According to the notice, what...", "options": {"A":"...","B":"...","C":"...","D":"..."}, "correct_answer": "B", "explanation": "The notice states..." },
    { "question_type": "inference", "stem": "What can be inferred...", "options": {"A":"...","B":"...","C":"...","D":"..."}, "correct_answer": "D", "explanation": "Based on..." }
  ],
  "difficulty": "easy|medium|hard (copy the assigned difficulty for this item)"
}
...
function buildRDLPrompt(count = 5, opts = {}) {
  ...
  // Difficulty tiers: 15% easy / 45% medium / 40% hard (see DIFFICULTY_SPECS)
  const difficulties = assignDifficulties(count, difficultyOverride, { easyShare: 0.15, hardShare: 0.40 });

  // Select question types for each item (3 questions per item), biased by
  // difficulty: easy leans detail-heavy, hard leans inference-heavy (never
  // 3 of the same type — matches the real-ETS Q-position formula below).
  const questionPlans = selected.map((s, i) => {
    if (difficulties[i] === "easy") {
      return ["detail", "detail", Math.random() < 0.5 ? "inference" : "main_idea"];
    }
    if (difficulties[i] === "hard") {
      return Math.random() < 0.5
        ? ["detail", "inference", "inference"]
        : ["main_idea", "detail", "inference"];
    }
    const types = [];
  ...
  // Assign answer positions calibrated to 152 real ETS questions:
  //   B: 46%, C: 32%, A: 17%, D: 5%
  // The previous balanced (25% each) was off — real ETS heavily favors B/C.
  // Build a weighted pool: for every 20 slots, allocate 9 B, 7 C, 3 A, 1 D.
  const posPool = [];
  const slotsTotal = count * 3;
  const slots = {
    B: Math.round(slotsTotal * 0.46),
    C: Math.round(slotsTotal * 0.32),
    A: Math.round(slotsTotal * 0.17),
    D: Math.max(1, Math.round(slotsTotal * 0.05)),
  };
  ...
  const itemSpecs = selected.map((s, i) => {
   ...
   Structure: ${s.spec.structure}
   Difficulty: ${difficulties[i]}${moneyNote}
...
1. **Length** (calibrated to 52 real ETS RDL items: mean 159 words, range 45-253):
   - Target: **140-200 words**. Acceptable range: 100-240 words.
   - Real ETS RDL averages 159 words — do NOT cut short at 100-180.
...
module.exports = { buildRDLPrompt, buildShortRDLPrompt, GENRE_SPECS, SHORT_GENRE_SPECS, QUESTION_TYPE_TEMPLATES, DIFFICULTY_SPECS };
