...
function buildShortRDLPrompt(count = 5, opts = {}) {
  ...
  const shortGenres = ["email", "notice", "text_message", "social_media", "flyer"];
  const shortScenarios = [
    "Brief campus event reminder (date + location only)",
    "Short text message about meeting up or running late",
    "Parking lot closure notice (one paragraph)",
    "Library hours change notice for holidays",
    "Quick email confirming a reservation or appointment",
    "Vending machine refund instructions",
    "Lost item notice on a bulletin board",
    "Building fire drill schedule (2-3 lines)",
    "Short social media post recommending a campus café",
    "Brief invoice or receipt for a campus service",
    "Elevator out of service notice",
    "Water fountain maintenance closure sign",
  ];

  const selected = [];
  for (let i = 0; i < count; i++) {
    const g = genres.length > 0 ? genres[i % genres.length] : shortGenres[Math.floor(Math.random() * shortGenres.length)];
    const avail = shortScenarios.filter(s => !excludeScenarios.includes(s));
    const scenario = avail[Math.floor(Math.random() * avail.length)] || shortScenarios[0];
    selected.push({ genre: g, scenario });
  }
  ...
  const itemSpecs = selected.map((s, i) => {
    ...
    const mustHaveMoney = (i % 4 === 0);
    return `${i + 1}. Genre: ${s.genre}\n   Scenario: ${s.scenario}${mustHaveMoney ? "\n   MUST INCLUDE a dollar amount" : ""}\n   Questions: Q1=detail(answer=${positions[0]}), Q2=inference(answer=${positions[1]})`;
  }).join("\n\n");
...
1. **Length**: 40-50 words ONLY. These are very brief texts — a few sentences max.
2. **Format**: Short notices, brief emails (1-2 sentences body), text messages, quick announcements.
3. **Include**: At least 1 specific detail (date, time, location, or price) per text.
4. **2 questions per text**: Q1 = detail question, Q2 = inference question.
5. **4 options (A/B/C/D)** per question, grammatically parallel.
6. **Correct answer position**: MUST match assigned position.
...
Return a JSON array. Each element:
{
  ...
  "format_metadata": {},
  "questions": [
    { "question_type": "detail", "stem": "...", "options": {"A":"...","B":"...","C":"...","D":"..."}, "correct_answer": "B", "explanation": "..." },
    { "question_type": "inference", "stem": "...", "options": {"A":"...","B":"...","C":"...","D":"..."}, "correct_answer": "D", "explanation": "..." }
  ],
  "difficulty": "easy"
}
...
module.exports = { buildRDLPrompt, buildShortRDLPrompt, GENRE_SPECS, QUESTION_TYPE_TEMPLATES };
