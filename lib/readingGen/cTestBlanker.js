...
// Closed set of words never blanked. this/that (fragment "th", len 4) and
// these/those (fragment "th", len 5) are mutually substitutable anaphors: when
// blanked they leave an identical fragment AND identical length, and both refer
// grammatically to the just-mentioned entity, so context cannot disambiguate.
// The 2026-08-02 L1 audit found this was the single dominant CTW ambiguity class
// (data/claudeGen/reports/FULL-AUDIT-2026-07-09). Skipped like 1-letter words:
// left fully intact, not counted in the alternation, so the next eligible word
// takes the blank slot instead.
const BLANK_SKIP_WORDS = new Set(["this", "that", "these", "those"]);
...
function applyBlanking(passage) {
  ...
  for (let i = 0; i < wordPool.length && blanks.length < 10; i++) {
    ...
    // Skip ambiguous demonstratives (this/that/these/those) — see BLANK_SKIP_WORDS.
    // Treated like 1-letter words: left intact, transparent to the alternation.
    if (BLANK_SKIP_WORDS.has(item.clean.toLowerCase())) continue;
...
module.exports = { applyBlanking, processPassage, computeFragment, splitSentences, BLANK_SKIP_WORDS };
