...
function validateCTWItem(item) {
  ...
  if (item.blanks && item.blanks.length === 10) {
    ...
    if (rareRatio > 0.5) {
      errors.push(`too_many_rare_blanks: ${rareCount}/10 blanks are rare words (max 50%, ETS target ≤20%)`);
    } else if (rareRatio > 0.3) {
      warnings.push(`many_rare_blanks: ${rareCount}/10 blanks are rare/domain-specific (ETS target ≤20%)`);
    }
