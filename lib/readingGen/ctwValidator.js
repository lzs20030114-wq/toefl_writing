...
const { EASY_WORDS, MEDIUM_WORDS, wordInSet } = require("./ctwDifficulty");
...
function validateCTWItem(item) {
  ...
  if (item.blanks && item.blanks.length === 10) {
    ...
    // 2026-05-29: was max 7.0 error / 6.5 warn — too tight, was rejecting
    // legitimate harder CTW. Real TPO CTW samples allow avg up to ~7.5 chars.
    // Relaxed to 8.0 error / 7.5 warn.
    ...
    if (avgBlankLen > 8) {
      errors.push(`blank_words_too_long: avg ${avgBlankLen.toFixed(1)} chars (max 8.0, ETS target 5.5)`);
    } else if (avgBlankLen > 7.5) {
      warnings.push(`blank_words_slightly_long: avg ${avgBlankLen.toFixed(1)} chars (ETS target 5.5)`);
    }

    // Rare/hard word ratio check — blanks should be mostly common words
    const rareCount = blankWords.filter(w => !wordInSet(w, EASY_WORDS) && !wordInSet(w, MEDIUM_WORDS)).length;
    const rareRatio = rareCount / 10;
    // 2026-05-31 (Lesson D — a gate must not be stricter than the real exam):
    // simulated on real 2026改后 CTW, this rule rejected ~19% / warned ~54% of REAL
    // items — it was suppressing authentic difficulty (real blanks avg 5.77 chars,
    // ~26% >=8 letters). Loosened error 0.5->0.7, warn 0.3->0.5.
    if (rareRatio > 0.7) {
      errors.push(`too_many_rare_blanks: ${rareCount}/10 blanks are rare words (max 70%)`);
    } else if (rareRatio > 0.5) {
      warnings.push(`many_rare_blanks: ${rareCount}/10 blanks are rare/domain-specific`);
    }

    // Long blank word count — max 3 blanks with ≥8 characters
    const longBlanks = blankWords.filter(w => w.length >= 8);
    if (longBlanks.length > 3) {
      warnings.push(`too_many_long_blanks: ${longBlanks.length} blanks ≥8 chars (max 3): ${longBlanks.join(", ")}`);
    }
