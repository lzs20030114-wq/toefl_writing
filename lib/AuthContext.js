...
// Custom event fired when the auth state in localStorage changes (login,
// logout, tier upgrade). Consumers (e.g. useMistakeFavorites) can subscribe
// to refresh their data without polling.
export const AUTH_CHANGED_EVENT = "toefl-auth-changed";

function emitAuthChanged() {
  if (typeof window === "undefined") return;
  try { window.dispatchEvent(new CustomEvent(AUTH_CHANGED_EVENT)); }
  catch { /* no-op */ }
}
