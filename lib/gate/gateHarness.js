...
function loadItems(cfg, file) {
  ...
  // cfg.noFlatten opts out — LA/AP items legitimately carry a questions[] (选项/题干),
  // flattening them would destroy the passage-level item (2026-07-11).
  if (!cfg.noFlatten && arr.length && arr[0] && Array.isArray(arr[0].questions)) arr = arr.flatMap((s) => s.questions);
  return arr;
}
