...
/**
 * Extract the corrected English word from a fix string.
 * The AI often outputs Chinese instructions like "将 recieve 改为 receive"
 * instead of just "receive".
 */
function extractCorrectedWord(fixText) {
  const s = String(fixText || "").trim();
  if (SINGLE_WORD_RE.test(s)) return s;
  // "将 X 改为 Y" / "改为 Y"
  const m1 = s.match(/改为\s*([A-Za-z][A-Za-z'-]*)/);
  if (m1) return m1[1];
  // "→ Y" / "-> Y"
  const m2 = s.match(/(?:→|->)\s*([A-Za-z][A-Za-z'-]*)/);
  if (m2) return m2[1];
  // "should be Y" / "change to Y" / "replace with Y"
...
function isSpellingSegment(segment) {
  ...
  if (!wrongText || !fixRaw) return false;
  if (!isSingleWord(wrongText)) return false;
  const fixText = extractCorrectedWord(fixRaw);
  if (!fixText || !isSingleWord(fixText)) return false;
  if (errorType !== "spelling" && !SPELLING_NOTE_RE.test(noteText)) return false;
...
export function extractPostWritingPracticeItems(histOrSessions, now = new Date()) {
  ...
  sessions.forEach((session, sessionIndex) => {
    ...
    marks.forEach((segment, markIndex) => {
      ...
      const wrongText = normalizeWord(segment.text);
      const correctText = extractCorrectedWord(normalizeWord(segment.fix)) || normalizeWord(segment.fix);
