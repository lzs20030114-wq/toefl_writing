...
export async function deleteSessionCloud(sessionId, userCode) {
  if (!supabase) return { error: "Supabase not configured" };
  // Scope to the owner when known: the sessions RLS policy is permissive
  // (FOR ALL USING true), so an unscoped delete-by-id can cross user
  // boundaries if a wrong id ever reaches this call.
...
export async function deleteSessionsCloud(sessionIds, userCode) {
  if (!supabase) return { error: "Supabase not configured" };
  const ids = (Array.isArray(sessionIds) ? sessionIds : []).filter((n) => Number.isInteger(n));
  if (ids.length === 0) return { error: null };
  let query = supabase.from("sessions").delete().in("id", ids);
  if (userCode) query = query.eq("user_code", userCode);
  const { error } = await query;
  return { error: error?.message || null };
}
