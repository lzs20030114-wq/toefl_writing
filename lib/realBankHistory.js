...
/**
 * 把一条记录里的条目 id 经 resolveItemRef 接到题库当前的 id / 题型。
 * 题型只在「同一科目内」采信（别名只可能把 ap 归位成 rdl 这种，不许把阅读记录挪进听力）；
 * 解析出已下线（retired / id 为空）的条目不进 itemIds —— 覆盖率的分母是当前题库，下线的题不该算进分子。
 */
function resolveEntryItems(session, recordedSubtype, resolveItemRef) {
  const subject = REAL_SUBTYPE_META[recordedSubtype]?.subject;
  let subtype = recordedSubtype;
  const itemIds = [];
  for (const id of realSessionItemIds(session)) {
    let ref = null;
    try {
      ref = resolveItemRef(id, recordedSubtype);
    } catch {
      ref = null;
    }
    if (!ref) {
    ...
    if (ref.type && REAL_SUBTYPE_META[ref.type]?.subject === subject) subtype = ref.type;
    if (ref.retired || !ref.id) continue;
    itemIds.push(String(ref.id));
  }
  return { subtype, itemIds };
}
 ...
 * 与各科历史页的删除接口同一口径）。
 *
 * resolveItemRef（可选）：(条目 id, 记录里的题型) => { id, type, retired }，把题库重建前的旧 id
 * 接到当前 id / 题型（记录页注入 lib/realBankAliases 的 resolveRealReadingRef）。注入后每条 entry 多带：
 *   subtype         —— 当前题型（记录里 subtype=ap、已归位到日常阅读的，这里是 rdl；展示 / 筛选 / 覆盖都按它）
 *   recordedSubtype —— 记录里原本的题型
 *   itemIds         —— 当前 id（已下线的剔除）
 * 不注入时与从前完全一致。为什么是注入而不是本文件直接 import 别名账本：首页入口卡只 import
 * 本文件数条数（countRealBankSessions），不该为此把账本也打进首页包。
 */
export function buildRealBankEntries(sessions, { resolveItemRef } = {}) {
  const list = Array.isArray(sessions) ? sessions : [];
  return list
    .map((session, i) => {
      const subtype = realSessionSubtype(session);
      const entry = {
        session,
        sourceIndex: Number.isFinite(Number(session?.id)) ? Number(session.id) : i,
        subtype,
      };
      if (subtype && typeof resolveItemRef === "function") {
        const resolved = resolveEntryItems(session, subtype, resolveItemRef);
        entry.recordedSubtype = subtype;
        entry.subtype = resolved.subtype;
        entry.itemIds = resolved.itemIds;
      }
      return entry;
    })
 ...
 * entry 带 itemIds（buildRealBankEntries 注入了 resolveItemRef）时按解析后的当前 id 去重 ——
 * 同一篇文章的旧 id 与新 id 只算一篇，归位到别的题型的算进新题型。
 */
export function buildRealBankCoverage(entries, totals = {}) {
  ...
  for (const e of Array.isArray(entries) ? entries : []) {
    ...
    const ids = Array.isArray(e?.itemIds) ? e.itemIds : realSessionItemIds(e.session);
    for (const id of ids) done[sub].add(id);
  }
