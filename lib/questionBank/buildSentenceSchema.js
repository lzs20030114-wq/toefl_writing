...
/**
 * Validate a question set (group of 10 questions).
 * Accepts either:
 * - questions array
 * - set object: { set_id, questions: [...] }
 * Returns { ok: boolean, errors: string[] }
 */
function validateQuestionSet(input) {
  const questions = Array.isArray(input) ? input : input && Array.isArray(input.questions) ? input.questions : null;
  const errors = [];
...
module.exports = {
  ...
  // Backward-compatible alias used by legacy scripts/tests.
