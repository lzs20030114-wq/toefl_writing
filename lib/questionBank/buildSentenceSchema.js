...
function validateQuestion(q) {
  ...
  if (Array.isArray(q.prefilled) && q.prefilled.length > 0) {
    ...
    // TPO uses bare subject pronouns (she/he/they) freely — only ban object pronouns and "not"
    const BANNED_PREFILLED = new Set(["not", "him", "her", "them"]);
    const pfNorm = q.prefilled[0].trim().toLowerCase();
    if (BANNED_PREFILLED.has(pfNorm)) {
      fatal.push(`prefilled "${q.prefilled[0]}" is a banned word — use subject pronoun or subject NP instead`);
    }
  }
