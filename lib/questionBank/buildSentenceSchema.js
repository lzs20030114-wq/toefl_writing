const { ETS_STYLE_TARGETS, isEmbeddedQuestion } = require("./etsProfile");
...
function validateQuestion(q) {
  ...
  if (answerWords.length < 6 || answerWords.length > 14) {
    format.push(`answer word count must be 6-14 (got ${answerWords.length})`);
  }
...
function validateQuestionSet(input) {
  ...
  // Passive voice: ETS only 4% (3/70), not a hard gate requirement
