...
function validateQuestion(q) {
  ...
  // 2026-06-01 DIALOGUE-COHERENCE gate. In the 2-turn dialogue style, the prompt is turn 1
  // (a literal utterance) and the answer the student assembles is turn 2, the REPLY. A
  // question-answer is only coherent there after a STATEMENT/opener prompt
  // ("I'm new here." -> "Where do I sign up?"); it must NOT follow another question (the
  // answer would just re-ask) or a "Tell me…/Describe…" info-request (which expects the
  // answer to SUPPLY info). Two exemptions, both required:
  //   1. task_kind="ask" — the task EXPLICITLY asks the student to pose a question
  //      ("What do you ask the clerk?" -> a question answer is correct by design).
  //   2. meta-instruction prompts ("How do you respond?", "What did X ask?", "What do you
  //      say?") — these TELL the student what to produce, they are not dialogue turns, so a
  //      question answer after them is not "re-asking". Structured (ask/report/respond)
  //      items always use these, so the gate only bites the literal-utterance dialogue style.
  ...
  if (answerIsQuestion && taskKind !== "ask") {
    ...
    if (!isMetaInstruction) {
      ...
      if (promptIsQuestion) {
        fatal.push("incoherent_dialogue: answer is a question but the prompt is also a question — the answer must REPLY, not re-ask. Pair a question-answer with a STATEMENT prompt (or use prompt_task_kind='ask').");
      } else if (promptIsInfoRequest) {
