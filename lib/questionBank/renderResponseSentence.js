...
function renderResponseSentence(question, userChunkOrder, options = {}) {
  ...
  // Reference answer is shown verbatim from q.answer (preserves authored capitalization,
  // proper nouns, and internal commas). answerWords is punctuation-stripped / lowercased and
  // used only for slot indexing + scoring — never for rendering the reference answer.
  const rawAnswer = typeof q.answer === "string" ? q.answer.trim() : "";
  const correctSentence = rawAnswer || finishSentence([...answerWords], hasQMark);
