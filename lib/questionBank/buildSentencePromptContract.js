...
// 2026-06-01: the realExam2026 BS task is a 2-turn dialogue — the prompt is turn 1 (a
// conversational utterance the test-taker replies to). Real direct-question answers are
// paired with STATEMENT openers ("There is a work study position." → "Do you know if it
// requires experience?"). Those statement prompts carry no "task cue", so the legacy-task
// check above would reject them even though the codebase already treats a tagged
// prompt_task_kind="statement" as valid (see isExplicitTaskText). This recognises a bare
// conversational statement opener as a valid dialogue turn — first/second person or an
// existential/demonstrative opener — WITHOUT admitting third-person background narration
// ("Emma's book was on the table." / "A visitor is asking the curator a question."), which
// stays rejected so the background-only guard keeps its teeth.
function isConversationalDialogueTurn(prompt) {
  const trimmed = normalizeText(prompt);
  if (!trimmed) return false;
  if (/\?\s*$/.test(trimmed)) return false; // questions are handled by the task-cue path
  const firstPerson = /\b(i|i'm|i've|i'll|i'd|me|my|mine|we|we're|we've|our|us|let's)\b/i.test(trimmed);
  const secondPerson = /\b(you|your|you're|you've|you'll|you'd)\b/i.test(trimmed);
  const existentialOpener = /^(there's|there is|there are|here's|here is|this |that |these |those )/i.test(trimmed);
  return firstPerson || secondPerson || existentialOpener;
}
