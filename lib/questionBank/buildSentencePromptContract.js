...
function isExplicitTaskText(taskText, taskKind = "") {
  if (taskKind === "statement") return true; // declarative — no cue pattern needed
...
function validateStructuredPromptParts(question, options = {}) {
  ...
  if (
    parts.taskKind &&
    ["ask", "report", "respond", "yesno"].includes(parts.taskKind) &&
    parts.taskText &&
    !/\?$/.test(parts.taskText)
  ) {
    fatal.push("prompt_task_text: ask/report/respond/yesno must end with a question mark");
  }
  if (
