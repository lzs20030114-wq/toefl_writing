 ...
 *
 * QQ Mail SMTP daily send cap. Tencent keeps the exact thresholds confidential
 * ("以上频率限制数值属于腾讯邮箱保密数据，恕不公开"); the figures below are the
 * consistent third-party / agent consensus, NOT an official guarantee:
 *   - Plain personal QQ mailbox:           ~100 emails / 24h
 *   - Personal + QQ会员 / 超级会员 (VIP1-7): ~500 emails / 24h
 *   - Enterprise QQ Mail:                   higher (500-2000+/day by plan)
 * Beyond the daily cap there are also per-minute / hourly frequency limits
 * (exceeding them returns "550 Ip frequency limited"; ~45s spacing is advised)
 * plus a 14-day SMTP-activation wait for newly created mailboxes. For real bulk
 * volume, swap in Resend/SES by changing this file only.
 *
