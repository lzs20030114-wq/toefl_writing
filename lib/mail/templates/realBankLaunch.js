...
/**
 * 题量快照（__tests__/real-bank-launch-email.test.js 锁它与最新一条带题量的公告一致）。
 * 2026-09-14 v1.19.0 更新：写作补录（邮件 27→57、学术讨论 132→162、造句 281→336）+ 阅读多轮补题
 * （CTW 116→155 段、RDL 110→125 篇、AP 99→89 篇；AP 按原卷重录后合并同篇，篇数降而题数 401→417）后
 * 1267 → 1426。2026-09-15 造句补题（跨卷重出还回原卷 + 温度重读补识题面 + 放开重复词块）
 * 336 → 573 题 / 44 → 62 卷，总数 1426 → 1663。2026-09-15 听力/口语跨卷重出还回原卷 + 面试补录
 * 13 套：听力 440 → 516 段、口语 62 → 69 套，总数 1663 → 1746。2026-09-17 v1.20.0 整块录音听力合流 +
 * 口语原声切片/面试 ASR 收题 + 造句 docx 答案页兜底：听力 516 → 1340 段、口语 69 → 130 套、
 * 造句 573 → 629 题 / 62 → 68 卷，总数 1746 → 2687（v1.20.0 发版时漏改这里，CI 红到 09-18 才补上）。
 * data/announcements.json 的历史条目都是发版当时的数字，不回头改。
 *
 * 为什么它要跟着发版涨、而不是冻在首发数字：群发脚本 scripts/ops/send-real-bank-announcement.mjs
 * 受 QQ SMTP 日上限所限是「分几天续发」的，还没收到信的用户拿到的就是这里的当前值。
 * 每个数的出处（发版改公告题量时照着重数一遍）：
 *   writing   = components/home/realExamCounts.js 的 REAL_WRITING_COUNTS
 *               （node scripts/realbank/sync_counts.mjs --dry 现算核对）
 *   reading   = data/realBank/reading/counts.json 的 ap / rdl / ctw
 *   listening = data/realBank/listening/counts.json 的 lcr + lc + la + lat
 *   speaking  = data/realBank/speaking/counts.json 的 repeat + interview
 */
export const REAL_BANK_LAUNCH_COUNTS = {
  total: 2687,
  writing: { total: 848, discussion: 162, email: 57, bs: 629, bsSets: 68 },
  reading: { total: 369, ap: 89, rdl: 125, ctw: 155 },
  listening: 1340,
  speaking: 130,
};
