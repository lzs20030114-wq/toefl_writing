 ...
 *  - Single speaker (professor) delivering a ~200-330 word lecture (realExam2026: mean 258)
...
// RECALIBRATED 2026-05-31 to realExam2026 (113 lectures): real lectures are ~40%
// ARTS + HUMANITIES (art history is the single most common field at 12%, then music,
// architecture, literature, history), only ~33% sciences. The old pool was ~78%
// science/psychology with 0 music, 0 architecture, 0 physics. Reweighted + added them.
const TOPIC_POOL = [
  {
    subject: "art_history",
    weight: 0.12,
    topics: [
      ...
      "how Expressionism represented emotional experience over physical reality",
      "the rediscovery of perspective in early Renaissance painting",
    ],
  },
  {
    subject: "music",
    weight: 0.08,
    topics: [
      "how the sonata form structured Classical-era symphonies",
      "the role of improvisation in early jazz",
      "how folk melodies were absorbed into nationalist compositions",
      "why the pipe organ shaped the acoustics of Baroque churches",
      "how recording technology changed the way music was composed",
    ],
  },
  {
    ...
    weight: 0.07,
    topics: [
      "how Roman concrete allowed the building of large domes",
      "why Gothic cathedrals used flying buttresses",
      "how the skyscraper became possible with steel-frame construction",
      "the influence of climate on traditional vernacular housing",
      "how ancient amphitheaters were designed for natural acoustics",
    ],
  },
  {
    subject: "physics",
    weight: 0.06,
    topics: [
      "why objects in motion resist changes in direction (inertia)",
      "how the Doppler effect changes the pitch of a passing sound",
      "the principle behind how a lever multiplies force",
      "how light refraction produces a rainbow",
      "why a spinning object stays balanced (angular momentum)",
    ],
  },
  {
    subject: "biology",
...
const DIFFICULTY_TIERS = [
  {
    tier: "easy",
    target_pct: 30,
    transcript: "200-240 words, single clear concept, one straightforward example",
    questions: "Q1: main_idea (obvious). Q2-Q3: detail (directly stated). Q4: inference (straightforward)",
  },
  {
    tier: "medium",
    target_pct: 45,
    transcript: "240-280 words, concept with nuance, experiment or study with clear findings",
  ...
  },
  {
    tier: "hard",
    target_pct: 25,
    transcript: "280-330 words, complex concept, multiple examples or layers, broader significance",
...
2. Transcript is CHATTY like a podcast/small-talk -- a real lecture is only LIGHTLY
   conversational (~1 contraction per 100 words). Stacking contractions in every
   sentence, or opening with "Okay, so today we're diving into…", will be REJECTED.
3. (Addressing students with "you" is OPTIONAL -- ~1 in 5 real lectures. Do NOT force it.)
4. A couple of discourse markers ("so", "now", "it turns out") are fine; do NOT cram them in.
...
function buildLATPrompt(count = 2, opts = {}) {
  ...
  // Weighted shuffle (Efraimidis-Spirakis): higher pool.weight => more likely to sort
  // first => more likely selected. The old plain random sort IGNORED weight entirely,
  // so the arts/humanities reweighting (2026-05-31) only took effect once this landed.
  const shuffled = allTopics
    .map(t => ({ t, k: Math.pow(Math.random(), 1 / t.weight) }))
    .sort((a, b) => b.k - a.k)
    .map(x => x.t);
  const selected = [];
  const subjectCount = {};
  ...
  // -- 5. Build item specs --
  // Opening move (2026-07-10): 真题 ~11% 的讲座是"续讲"开场("Last week we talked
  // about…" / "We've been discussing…"),生成库为 0 — 丢失的真题亚型,按 ~1/10 硬指派。
  const recapSlot = Math.random() < Math.min(1, count * 0.11) ? Math.floor(Math.random() * count) : -1;
  const itemSpecs = [];
  for (let i = 0; i < count; i++) {
    const t = selected[i];
    ...
    itemSpecs.push(
      ...
      (recapSlot === i
        ? `  Opening move (REQUIRED): open as a CONTINUING lecture — "Last week we talked about…" / "We've been discussing… Today I want to turn to…" — then move into today's material. (~1 in 10 real lectures opens this way.)\n`
        : "") +
...
- **Aim for 260-300 words (target ~258)** — RECALIBRATED to realExam2026 (107 clean
  lectures: median 250, band 220-258, min 192).
- ⚠️ Models SYSTEMATICALLY UNDERSHOOT: at a stated "200-330 / 258" DeepSeek produced
  only ~196 words (measured 2026-05-31). So AIM HIGH — write toward 280; a lecture
  under 220 words is too short. The old "150-250" was below the real MINIMUM (192).
- Academic register -- SPOKEN, but a lecture, NOT a chat.

### Spoken register (RECALIBRATED 2026-05-31 — real lectures are LIGHTLY conversational,
### far less chatty than the old prompt forced. Real contraction density is only ~1.2
### per 100 words, vs conversations' 4.5; the generator was hitting ~5.0 = a chat, not a lecture):
- Open DECLARATIVELY by naming the concept — like a real professor. Real examples:
  "Expressionism is an influential modernist art movement…", "Migratory birds travel…".
- Contractions: include a FEW (about 3 per lecture, ~1 per 100 words) — NOT zero, but
  NOT in every sentence. "don't / it's / you're" sprinkled, not stacked. A lecture with
  zero contractions reads too written; a few keep it spoken.
- A rhetorical-question or "you"-address hook is OPTIONAL, ~1 in 5 lectures (real
  you-address ~21%, rhetorical opener ~14%). Most lectures do NOT open with "Okay, so
  today we're diving into…". Do not force a "right?" / "does that make sense?".
- A few discourse markers ("So", "Now", "It turns out") are fine; don't overuse.
- Mix sentence lengths: short punchy statements with longer explanatory ones
- DO NOT sound like a textbook -- but DO sound like a prepared lecture, not small talk
...
Return a JSON array. Each element:
{
  ...
  "transcript": "The full lecture text as a single string. Must be 200-330 words (target ~258). A prepared spoken lecture: declarative opening, only occasional contractions (~1 per 100 words), not a chatty monologue.",
  "questions": [
    {
      "type": "main_idea",
