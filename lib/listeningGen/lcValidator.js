...
/**
 * @param {object} item
 * @param {{realExam?: boolean}} [opts] — realExam 走**真题**下限：生成库的下限是照着「一段像样的
 *   对话该有多长」定的，真题不适用。data/realExam2026 的 155 段原卷对话里最短 53 词
 *   （4.6 那三段 53/54/57 词都是完整成对的闲聊），26% 在 80 词以下 —— 拿生成库的尺子量真题
 *   会把真实分布的下四分之一整片切掉。轮次同理（短对话自然少几轮）。
 *   「每人至少 2 轮」不放宽：只有一方开口一次的"对话"多半是转写没切全。
 */
function validateSchema(item, opts = {}) {
  const real = !!opts.realExam;
  const errors = [];
  ...
  const minTurns = real ? 4 : 5;
  if (turnCount < minTurns) {
    errors.push(`too_few_turns: ${turnCount} (min ${minTurns})`);
  ...
  const minWords = real ? 50 : 60;
  if (totalWords < minWords) {
    errors.push(`conversation_too_short: ${totalWords} words (min ${minWords})`);
  ...
  for (let qi = 0; qi < item.questions.length; qi++) {
    ...
    // Option word counts
    // 真题（item.real === true）里 "Yes." / "No." 这种单词选项是真实存在的，放宽到 1 词；
    // 生成库仍按 2 词下限拦，这条是防退化质量闸，不许对生成内容松口。
    const minOptWc = item.real === true ? 1 : 2;
    for (const key of VALID_ANSWERS) {
      if (!q.options[key]) continue;
      const optWc = wc(q.options[key]);
      if (optWc < minOptWc) errors.push(`${prefix}_option_${key}_too_short: ${optWc} words`);
