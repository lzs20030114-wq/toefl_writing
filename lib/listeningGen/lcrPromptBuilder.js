...
// Speaker sentence type distribution — RECALIBRATED 2026-05-31 to realExam2026 (n=178):
// WH-questions dominate at ~49% (bank was only 7%); yes/no+negative ~24%; statements ~26%.
const SPEAKER_TYPES = [
  { type: "wh_question", pct: 49, pattern: "Where/How/What/When/Why/Who...?", example: "Where can I find the class notes?" },
  { type: "yes_no_question", pct: 14, pattern: "Do you/Are you/Can you...?", example: "Did you enjoy the gardening class?" },
  { type: "negative_question", pct: 10, pattern: "Didn't/Isn't/Aren't...?", example: "Didn't I just see you in the library?" },
  { type: "declarative_statement", pct: 27, pattern: "I'm.../I have to.../I'm thinking of...", example: "Maybe the power went out for this whole area." },
];
...
function buildLCRPrompt(count = 10, opts = {}) {
  ...
  // ── 4b. FORCE per-item sentence type (realExam2026: wh 49% / yes-no 24% / stmt 26%).
  // A GLOBAL "~49% wh" instruction is IGNORED by DeepSeek (it produced 0% wh, 2026-05-31),
  // so each item gets an explicit type it MUST follow — this guarantees the proportion.
  const WH_WORDS = ["Where", "Why", "How", "What", "When", "Who"];
  const stPool = [];
  for (let i = 0; i < count; i++) {
    const frac = i / Math.max(1, count);
    if (frac < 0.49) stPool.push(`WH-QUESTION — MUST start with "${WH_WORDS[i % WH_WORDS.length]}" and end with "?"`);
    else if (frac < 0.73) stPool.push(`yes/no question — start with Do/Are/Can/Did/Have, end with "?"`);
    else stPool.push(`declarative statement — NO question mark`);
  }
  for (let i = stPool.length - 1; i > 0; i--) {
    const j = Math.floor(Math.random() * (i + 1));
    [stPool[i], stPool[j]] = [stPool[j], stPool[i]];
  }
  ...
  for (let i = 0; i < count; i++) {
    ...
    itemSpecs.push(
      ...
      `  Sentence type (REQUIRED): ${stPool[i]}\n` +
...
## GENERATION PROTOCOL — DIALOGUE FIRST (MANDATORY ORDER)

For EVERY item, author it in this exact order:

1. **Write a natural two-turn micro-dialogue.** Person A says one spoken sentence; Person B —
   the person A is addressing — replies with the single most natural line. Write it as a real
   overheard exchange, not as a test item.
2. **A's line becomes "speaker"; B's line becomes the KEYED OPTION, verbatim.** The correct
   answer is never authored separately — it can only be the reply B actually gave. Because B is
   by definition the addressee, a speaker-side line (an offer of help, an advisor move) can
   never become the key: B would not say it.
3. **Check the paradigm.** If B's natural reply doesn't embody the assigned answer_paradigm,
   REWRITE THE DIALOGUE until it does — never keep the speaker line and bolt on a "correct
   answer" B wouldn't say. A dialogue you can't make natural is a dialogue to discard.
4. **Only now build the 3 distractors** for the assigned trap types. Distractors are the only
   place where speaker-side lines are allowed (role_inversion trap).
...
- Length: 3-14 words (**target ~8**, realExam2026 median 8 / band 6-11). Short and spoken.
  (The old "target 8-12" ran long; keep prompts tight.)
- Use contractions naturally (I'm, Didn't, Isn't, It's) — at least 60% of items.
- Sentence types (RECALIBRATED 2026-05-31 to realExam2026, n=178): **~49% WH-questions**
  (Who / What / Where / When / Why / How — this is the DOMINANT real form; the bank was
  only ~7% wh, the single biggest short-response gap). ~24% yes/no or negative questions,
  ~26% declarative statements, ~1% imperative. **Open with a WH-word about half the time.**
...
- The key IS Person B's line from your dialogue (see GENERATION PROTOCOL) — these rules
  describe what that line must look like once the dialogue is right.
...
- **ROLE CONSISTENCY (CRITICAL)**: The correct answer is spoken by the person the speaker is
  ADDRESSING — never write a line that only makes sense coming from the speaker's own side.
  Concretely: if the speaker observes/asks about the RESPONDER's state or plans
  ("You look wiped out today." / "Have you decided on a major yet?"), the responder explains
  or answers about THEMSELVES ("I cleaned the whole kitchen last night." / "Not yet — I'm torn
  between two programs."). Offers of help ("Want me to grab you a coffee?", "Let me carry a few.")
  and advisor moves ("Let's go over your options together.") belong to the OBSERVER — they are
  only valid correct answers when the SPEAKER is the one describing their own problem.
  A speaker-side line may be used as a DISTRACTOR (role_inversion trap), never as the key.
...
IMPORTANT: The "explanation" field MUST be written in Chinese (中文). Explain why the correct answer is right and why each distractor is wrong, all in Chinese.
