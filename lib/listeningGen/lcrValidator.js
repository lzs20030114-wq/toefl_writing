...
// ── Role-inversion lint (deterministic, hard error) ──
//
// Catches the "observer's line keyed as the response" bug class found in the
// 2026-08-02 full audit (data/claudeGen/reports/lcr-answer-audit-20260802.md):
// when the speaker puts the RESPONDER in the problem-holder seat ("You look
// wiped out." / "Have you decided on a major yet?"), the key must be spoken
// from the responder's side — an offer of help or advisor move ("Want me to
// grab you a coffee?", "Let's look at your options together.") belongs to the
// speaker's side and can only be a distractor.
//
// Precision-first: patterns are deliberately narrow (hard error must not
// false-positive). Situation-dependent inversions (e.g. a TA-only line under a
// classmate reading) are NOT detectable here — those are the AI auditor's job.

// Speaker lines that put the responder in the problem-holder seat.
const SPEAKER_TARGETS_RESPONDER = /^(you (look|seem|sound)\b|(have|are|were|do|did) you\b|what are you\b|why are you\b|when are you\b|how are you\b)/i;

// …except service/capability questions, where the responder IS the provider
// and helper moves are legitimate keys ("Do you carry…?" → "Let me show you the section.")
const SERVICE_QUESTION = /^(do you (carry|have|sell|stock|offer|serve|accept|take|know)|can you\b|could you\b|would you\b)/i;

// Keyed answers that are observer/helper moves.
// Joint-action ("let's", "why don't we") and reassurance ("don't worry") openers are
// legitimate from EITHER side — they only mark the observer when the clause acts on
// the responder's stuff (contains you/your): "Let's look at your options" is an advisor
// move, "Let's study together this weekend" is a valid responder proposal; "Don't worry,
// I'll walk you through it" is the helper, "Don't worry, I'm back on my feet" is the
// person being asked about.
const OBSERVER_MOVE_KEY = [
  /^(want me to|shall i|why don't i)\b/i,                       // offers to act for the other
  /^here[,—–-]?\s*let me\b/i,                                   // "Here, let me grab a couple."
  /^(let me|let's|why don't we|how about we)\b[^.?!]*\byour?\b/i, // joint/helper move on the responder's stuff
  /^don't worry\b[^.?!]*\bi\s*('ll|will|can)\b[^.?!]*\byour?\b/i, // "Don't worry, I'll walk you through it"
  /^i can \w+ your?\b/i,                                        // "I can lend you some money…"
  /^maybe (sit|take|try|get|go|skip|rest|stay|ask|talk)\b/i,    // subjectless advice to the listener
];

/**
 * Returns a reason string when the keyed answer is a speaker-side line, else null.
 * Exported for reuse (merge gate, L1 audits, tests).
 */
function detectRoleInversion(speaker, keyText) {
  if (!speaker || !keyText) return null;
  const s = speaker.trim();
  if (!SPEAKER_TARGETS_RESPONDER.test(s) || SERVICE_QUESTION.test(s)) return null;
  const k = keyText.trim();
  const hit = OBSERVER_MOVE_KEY.find(re => re.test(k));
  return hit ? `key is a speaker-side move ("${k.slice(0, 40)}…" matches ${hit})` : null;
}

// ── Level 1: Schema validation (hard errors → reject) ──

function validateSchema(item, opts = {}) {
  const errors = [];
  ...
  // 真题下限 3：GT 的 178 条原卷刺激句里最短一条是 "The classroom's cold."（3 词），
  // 完全是这个题型该有的样子。生成库仍要求 4 词，免得模型偷懒产出两三个词的残句。
  const minSpeakerWc = opts.realExam ? 3 : 4;
  if (speakerWc < minSpeakerWc) errors.push(`speaker_too_short: ${speakerWc} words (min ${minSpeakerWc})`);
 ...
 */
function validateLCR(item, opts = {}) {
  // Level 1: Schema
  const errors = validateSchema(item, opts);
  if (errors.length > 0) {
    return { valid: false, errors, warnings: [], flavor: null };
  }

  // Level 1.5: Role-inversion lint (hard error — see detectRoleInversion)
  const inversion = detectRoleInversion(item.speaker, item.options[item.answer]);
  if (inversion) {
    return { valid: false, errors: [`role_inversion_key: ${inversion}`], warnings: [], flavor: null };
  }
...
module.exports = { validateLCR, validateBatch, scoreFlavor, detectRoleInversion };
