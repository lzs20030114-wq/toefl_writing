...
//
// 句级时间戳（docs/listening-sentence-timings.md）：*Timed 渲染要求注入的 synth 返回
// { buffer, words:[{text,start,end}] }（edgeTts.generateSpeechTimed，词时刻相对该段音频开头）。
// 各段 mp3 按字节拼接，播放器按帧顺序解码，所以「前面各段的帧时长之和」就是这一段的起点
// （lib/tts/mp3Frames）；把各段的词平移后，与产线同一把刀切出来的句子做对齐
// （lib/listening/alignSentences）。对不上（词太少 / 定位率不足）就不写时间戳，音频照常。
const { splitSentences } = require("../tts/wavTools");
const { mp3DurationSec } = require("../tts/mp3Frames");
const { alignSentences } = require("../listening/alignSentences");
...
// synthTimed 返回的词（相对该段开头）平移 base 秒，转成 alignSentences 吃的 {w,start,end}。
function shiftedWords(words, base) {
  const out = [];
  for (const w of Array.isArray(words) ? words : []) {
    const text = w && (w.text != null ? w.text : w.w);
    if (!text || !Number.isFinite(w.start) || !Number.isFinite(w.end)) continue;
    out.push({ w: String(text), start: base + w.start, end: base + w.end });
  }
  return out;
}

function timingsOrNull(sentences, words) {
  if (!words.length || !sentences.length) return null;
  return alignSentences(sentences, words).timings;
}

// 带句级时间戳的对话渲染：{ buffer, sentences }。turn 下标按「有台词的轮次」顺序编（与合成顺序一致）。
async function renderConversationAudioTimed(conversation, speakers, synthTimed) {
  ...
  for (let i = 0; i < turns.length; i++) {
    ...
    const r = await synthTimed(text, preset);
    const buf = r && r.buffer;
    if (!buf || !buf.length) throw new Error("empty turn audio");
    buffers.push(buf);
    words.push(...shiftedWords(r.words, base));
    for (const s of splitSentences(text)) sentences.push({ turn: i, ...(turn.speaker != null ? { speaker: String(turn.speaker) } : {}), text: s });
    base += mp3DurationSec(buf);
  }
  return { buffer: Buffer.concat(buffers), sentences: timingsOrNull(sentences, words) };
}

async function renderConversationAudio(conversation, speakers, synth) {
  const adapted = async (text, preset) => ({ buffer: await synth(text, preset), words: [] });
  return (await renderConversationAudioTimed(conversation, speakers, adapted)).buffer;
}
...
// 带句级时间戳的单人渲染：{ buffer, sentences }。句子按整段文本切（与产线同一把刀），
// 各段的词按前面各段的 mp3 帧时长平移后再对齐。
async function renderSpokenAudioTimed(text, synthTimed, { segmented = false } = {}) {
  const whole = String(text || "");
  const segments = segmented ? segmentSpokenText(whole) : [whole];
  if (segments.length === 0) throw new Error("no segments to render");
  const buffers = [];
  const words = [];
  let base = 0;
  for (const seg of segments) {
    const r = await synthTimed(seg);
    const buf = r && r.buffer;
    if (!buf || !buf.length) throw new Error(segmented ? "empty segment audio" : "empty audio");
    buffers.push(buf);
    words.push(...shiftedWords(r.words, base));
    base += mp3DurationSec(buf);
  }
  const sentences = splitSentences(whole).map((s) => ({ text: s }));
  const buffer = buffers.length === 1 ? buffers[0] : Buffer.concat(buffers);
  return { buffer, sentences: timingsOrNull(sentences, words) };
}

async function renderSpokenAudio(text, synth, opts = {}) {
  const adapted = async (seg) => ({ buffer: await synth(seg), words: [] });
  return (await renderSpokenAudioTimed(text, adapted, opts)).buffer;
}
