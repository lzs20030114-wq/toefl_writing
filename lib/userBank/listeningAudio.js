...
// Listening types whose data carries a server-minted audio_url. All four listening subtypes now
// mint audio server-side (lcr/la/lat single-voice, lc multi-voice conversation).
const LISTENING_TYPES = new Set(["lcr", "la", "lat", "lc"]);

function isListeningType(type) {
  return LISTENING_TYPES.has(String(type || "").toLowerCase());
}

// Return `data` with audio_url (and its companion sentence_timings — minted by the same server
// render, meaningless without it) removed IFF this is a listening type carrying them. Non-listening
...
function stripClientAudioUrl(type, data) {
  ...
  if (!("audio_url" in data) && !("sentence_timings" in data)) return data;
  const { audio_url: _dropped, sentence_timings: _droppedTimings, ...rest } = data;
  return rest;
}
