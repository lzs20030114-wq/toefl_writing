 ...
 *    (≥1 saved practice session). Cap: 30 days lifetime via referrals.
 *
 * Anti-abuse:
 *  - Self-ref: inviter_code === invitee_code → reject
 *  - IP flood: same invitee_ip bound a referral in the last 24h → reject
 *  - Cap: inviter already has 10 granted referrals → reject
...
export const INVITER_MAX_GRANTED = 10;            // 10 × 3 = 30 days cap
...
export async function bindReferral({ inviterCode, inviteeCode, inviteeIp, source }) {
  ...
  // 4. Inviter cap: already at max granted referrals?
  const { count: grantedCount } = await supabaseAdmin
    .from("referrals")
    .select("id", { count: "exact", head: true })
    .eq("inviter_code", inviter)
    .eq("status", "granted");
  if ((grantedCount || 0) >= INVITER_MAX_GRANTED) {
    // Still bind it so the activity feels "tracked", but mark rejected so no reward fires.
...
export async function activateReferral({ inviteeCode }) {
  ...
  // 3. Re-check inviter cap right before granting
  const { count: grantedCount } = await supabaseAdmin
    .from("referrals")
    .select("id", { count: "exact", head: true })
    .eq("inviter_code", ref.inviter_code)
    .eq("status", "granted");
  if ((grantedCount || 0) >= INVITER_MAX_GRANTED) {
    await supabaseAdmin
      .from("referrals")
      .update({ status: "rejected", rejection_reason: "cap_exceeded" })
      .eq("id", ref.id);
    return { ok: true, granted: false, reason: "cap_exceeded" };
  }

  // 4. Mark activated (audit trail)
  await supabaseAdmin
    .from("referrals")
    .update({ status: "activated", activated_at: new Date().toISOString() })
    .eq("id", ref.id);

  // 5. Grant Pro to inviter (stacking on existing tier_expires_at)
  ...
  // 7. Notify inviter by email (best-effort, never blocks the grant flow).
...
export async function getReferralStats(inviterCode) {
  ...
  return {
    ...
    daysRemaining: Math.max(0, (INVITER_MAX_GRANTED - grantedCount) * REWARD_DAYS_PER_REFERRAL),
    maxGranted: INVITER_MAX_GRANTED,
