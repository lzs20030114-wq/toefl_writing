...
function inferErrorType({ errorType, level, message, fix, markedText }) {
  ...
  if (!SPELLING_NOTE_RE.test(noteText)) return "";
  if (!SINGLE_WORD_RE.test(wrongText) || !SINGLE_WORD_RE.test(fixText)) return "";
  if (wrongText.toLowerCase() === fixText.toLowerCase()) return "";
  return "spelling";
}
