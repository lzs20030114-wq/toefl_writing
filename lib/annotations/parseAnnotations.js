      ...
      if (/^r\s*$/i.test(tagBody)) {
        ...
        if (nOpen) {
          ...
          if (
            ...
            // 复用上方已算好的 detachedAnchor(同样的 plainText/needle 扫描):
            // 两点之间 plainText 与 needle 在可达路径上都不变(会变的分支均以
            // continue 收尾),无需重复扫描一遍。
            const mappedStart = detachedAnchor;
            if (level && mappedStart >= 0) {
              annotations.push({
                level,
...
// DeepSeek 在中文语境下会把 ASCII 撇号/引号/连字符漂成弯体或长横(与文件头
// 记载的弯引号属性同类现象)。fragment 含 don’t(弯撇)而 source 是 don't(直撇)
// 时,字节级匹配得到零候选,整条批注被静默丢弃。这里做 1:1 单字符折叠,把常见
// 字形变体归一到 ASCII;haystack(buildNormalized)与 needle(collapseWs)两侧
// 复用同一函数才能对得上。因为是单字符替换,buildNormalized 的索引映射不受
// 影响(只有空白折叠是多对一)。
...
function collapseWs(s) {
  return foldGlyphs(String(s || "").trim().replace(/\s+/g, " "));
}

// Normalized view of a string: whitespace runs collapsed to single spaces,
// with a map from each normalized char back to its original index.
function buildNormalized(source) {
  ...
  for (let i = 0; i < src.length; i += 1) {
    const ch = src[i];
    if (/\s/.test(ch)) {
      if (text.length > 0 && pendingWs < 0) pendingWs = i;
      continue;
    }
    if (pendingWs >= 0) {
      text += " ";
    ...
    // 1:1 字形折叠后入 normalized 文本 — needle 侧(collapseWs)做同样折叠。
    text += foldGlyphs(ch);
    map.push(i);
  }
  return { text, map };
...
const REANCHOR_CONTEXT = 24;

// Re-anchor every annotation onto the user's actually submitted text.
// ===ANNOTATION=== asks the AI to echo the original essay and the report
// used to render that echo — but the echo can silently drop or rewrite
// words (observed live: a legitimate word vanished from the displayed
// "original"). The submitted text is the only source of truth for what the
// user wrote, so when the echo differs we remap each mark onto the real
// text — whitespace-insensitively, disambiguating repeated fragments by
// surrounding context — and display the real text instead. Marks whose
// fragment the user never wrote are dropped rather than shown on invented
// text; if nothing at all anchors (echo is alien to the source), we rebuild
// from the source with no marks — never display the AI's echo as "original".
export function reanchorToSource(parsed, sourceText) {
  const src = String(sourceText || "");
  const echo = String(parsed?.plainText || "");
  const annotations = Array.isArray(parsed?.annotations) ? parsed.annotations : [];
  if (!src.trim() || echo === src) return parsed;
  ...
  const valid = annotations
    ...
    .sort((a, b) => a.start - b.start || a.end - b.end);
  if (valid.length === 0) {
    return { ...parsed, plainText: src, annotations: [] };
  }

  const srcN = buildNormalized(src);
  const used = [];
  const occupied = (s, e) => used.some((u) => !(e <= u.start || s >= u.end));

  const remapped = [];
  let cursorN = 0;
  for (const a of valid) {
    const frag = echo.slice(a.start, a.end);
    const fragN = collapseWs(frag);
    if (!fragN) continue;

    // Context around the mark in the echo, normalized, keeping the boundary
    // space between context and fragment so "…a problem |about|" cannot be
    // confused with "…you |about|".
    const leftRaw = echo.slice(Math.max(0, a.start - REANCHOR_CONTEXT * 3), a.start);
    let leftN = collapseWs(leftRaw);
    if (leftN && (/\s$/.test(leftRaw) || /^\s/.test(frag))) leftN += " ";
    leftN = leftN.slice(-REANCHOR_CONTEXT);
    const rightRaw = echo.slice(a.end, a.end + REANCHOR_CONTEXT * 3);
    let rightN = collapseWs(rightRaw);
    if (rightN && (/^\s/.test(rightRaw) || /\s$/.test(frag))) rightN = ` ${rightN}`;
    rightN = rightN.slice(0, REANCHOR_CONTEXT);

    // All candidate positions in the source, word-boundary matches first.
    const candidates = [];
    let idx = boundaryIndexOf(srcN.text, fragN, 0);
    while (idx >= 0) {
      candidates.push(idx);
      idx = boundaryIndexOf(srcN.text, fragN, idx + 1);
    }
    if (candidates.length === 0) {
      idx = srcN.text.indexOf(fragN);
      while (idx >= 0) {
        candidates.push(idx);
        idx = srcN.text.indexOf(fragN, idx + 1);
      }
    }
    ...
    const spans = candidates
      .map((cand) => ({
        cand,
        start: srcN.map[cand],
        end: srcN.map[cand + fragN.length - 1] + 1,
      }))
      .filter((s) => !occupied(s.start, s.end));
    if (spans.length === 0) continue;

    let best = null;
    for (const s of spans) {
      const ctxLeft = srcN.text.slice(Math.max(0, s.cand - REANCHOR_CONTEXT), s.cand);
      const ctxRight = srcN.text.slice(s.cand + fragN.length, s.cand + fragN.length + REANCHOR_CONTEXT);
      const score = commonSuffixLen(ctxLeft, leftN) + commonPrefixLen(ctxRight, rightN);
      const forward = s.cand >= cursorN ? 1 : 0;
      if (!best || score > best.score || (score === best.score && forward > best.forward)) {
        best = { ...s, score, forward };
      }
    }

    remapped.push({ ...a, start: best.start, end: best.end });
    used.push({ start: best.start, end: best.end });
    cursorN = best.cand + fragN.length;
  }

  if (remapped.length === 0) {
    // 与上面 valid.length === 0 分支保持完全同形状的回退:一条批注都锚定不上
    // 时返回用户原文 + 空批注,而不是原样返回 parsed —— parsed.plainText 是
    // AI 复述文,可能已被改写,不能当「原文」展示。
    return { ...parsed, plainText: src, annotations: [] };
  }
  remapped.sort((x, y) => x.start - y.start || x.end - y.end);
  return { ...parsed, plainText: src, annotations: remapped };
}
