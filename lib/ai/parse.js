...
function parseJsonReport(cleaned) {
  ...
  if (!Array.isArray(parsed.key_problems)) {
    parsed.key_problems = [];
  }
  if (!parsed.score_confidence || typeof parsed.score_confidence !== "object") {
    parsed.score_confidence = null;
  }
  if (!parsed.confidence_state || typeof parsed.confidence_state !== "object") {
    parsed.confidence_state = null;
  }
...
function parseRubricSection(text) {
  const src = String(text || "");
  if (!src.trim()) return null;
  const taskMatch = src.match(/TaskFulfillment\s*:\s*([0-5](?:\.\d+)?)\s*\|\s*(.+)$/im);
  const orgMatch = src.match(/OrganizationCoherence\s*:\s*([0-5](?:\.\d+)?)\s*\|\s*(.+)$/im);
  const langMatch = src.match(/LanguageUse\s*:\s*([0-5](?:\.\d+)?)\s*\|\s*(.+)$/im);
  const weightedMatch = src.match(/WeightedScore\s*:\s*([0-5](?:\.\d+)?)/i);
  if (!taskMatch || !orgMatch || !langMatch) return null;

  const weights = { task_fulfillment: 0.4, organization_coherence: 0.3, language_use: 0.3 };
  const dimensions = {
    task_fulfillment: {
      score: Number(taskMatch[1]),
      weight: weights.task_fulfillment,
      definition: "How fully and accurately task requirements are addressed.",
      reason: String(taskMatch[2] || "").trim(),
    },
    organization_coherence: {
      score: Number(orgMatch[1]),
      weight: weights.organization_coherence,
      definition: "Logical flow, progression, and coherence across ideas.",
      reason: String(orgMatch[2] || "").trim(),
    },
    language_use: {
      score: Number(langMatch[1]),
      weight: weights.language_use,
      definition: "Grammar control, vocabulary precision, and sentence clarity.",
      reason: String(langMatch[2] || "").trim(),
    },
  };
  const weightedFromDims =
    dimensions.task_fulfillment.score * weights.task_fulfillment +
    dimensions.organization_coherence.score * weights.organization_coherence +
    dimensions.language_use.score * weights.language_use;

  return {
    dimensions,
    weighted_score: weightedMatch ? Number(weightedMatch[1]) : Number(weightedFromDims.toFixed(2)),
...
function parseScoreSection(text) {
  const src = String(text || "");
  const scoreMatch = src.match(/score\s*[:：]\s*([0-5](?:\.5)?)/i);
...
export function parseReport(rawText) {
  try {
    ...
    const rubric = parseRubricSection(sections.RUBRIC || "");
