...
function parseScoreSection(text) {
  ...
  const scoreMatch = src.match(/(?:分数|score)\s*[:：]\s*([0-5])/i);
  const bandMatch = src.match(/band\s*[:：]\s*([0-9]+(?:\.[0-9]+)?)/i);
  const summaryMatch = src.match(/(?:总评|summary)\s*[:：]\s*(.+)$/im);
  return {
...
function parseActionSection(text) {
  ...
  const actions = blocks
    .map((block) => {
      ...
      const importance =
        (block.match(
          /(?:重要性|Importance)\s*[:：]\s*([\s\S]*?)(?=\n(?:行动|Action|短板\d+|Action\d+)\s*[:：]|$)/im
        ) || [])[1] || "";
      const action =
