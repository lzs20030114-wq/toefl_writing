...
function parseJsonReport(cleaned) {
  ...
  // Only a real number, or a non-empty numeric string, counts as a score.
  // Number(null) / Number("") / Number(false) / Number([]) all coerce to a
  // *finite* 0, so a bare `Number()` would silently turn a missing/blank score
  // into a legitimate-looking 0 (and skip the 0-5 clamp). Reject those shapes so
  // the report falls through to fallbackReport(error) — i.e. "评分失败" the user
  // can retry — instead of a phantom 0.
  ...
  parsed.score = Math.max(0, Math.min(5, Number(rawScore)));
...
function parseDimensionScore(src, label) {
  const re = new RegExp(`${label}\\s*[:：]\\s*([0-5](?:\\.\\d+)?)`, "i");
...
function parseScoreSection(text) {
  ...
  // Extract dimension scores (任务完成/组织连贯/语言使用)
  const taskScore = parseDimensionScore(src, "维度-任务完成");
  const orgScore = parseDimensionScore(src, "维度-组织连贯");
  const langScore = parseDimensionScore(src, "维度-语言使用");
  const hasDimensions = taskScore !== null || orgScore !== null || langScore !== null;
  ...
  return {
    ...
    rubric: hasDimensions ? {
      dimensions: {
        task_fulfillment: { score: taskScore },
        organization_coherence: { score: orgScore },
        language_use: { score: langScore },
      },
    } : null,
  };
}
