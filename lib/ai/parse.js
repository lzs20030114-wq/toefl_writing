...
// 评分报告与讲评共用的段名。段落标记一旦没被认出来，那一段连同它后面的内容会被
// 并进上一段吞掉 —— 表现是分数照常显示、批注/修订版/范文/行动建议全部静默消失、
// 报告还不报错（2026-09-21 线上实测：同一份报告里批注与范文同时不见）。
// **新增段名必须同步这里**，否则那一段会被当成正文吞掉；
// __tests__/ai-parse-section-markers.regression.test.js 会从 prompt 文件反查、卡住遗漏。
const KNOWN_SECTIONS = new Set([
  // 评分报告：lib/ai/prompts/academicWriting.js + emailWriting.js
  "ERRORS", "SCORE", "GOALS", "ANNOTATION", "CORRECTED", "PATTERNS", "COMPARISON", "ACTION", "SIGNALS",
  // 讲评：lib/ai/prompts/writingLesson.js（经 lib/ai/lessonParse.js 复用本函数）
  "VERDICT", "FOCUS", "LANGUAGE", "COMPARE", "NEXT",
]);

// 导出供 lib/ai/lessonParse.js 复用（讲评输出用同一套 ===SECTION=== 约定）。
//
// 旧正则是 /^===([A-Z_]+)===$/gm —— 严格到只认一种写法。模型偶尔会给标记行加装饰
// 或留下空白（`**===SCORE===**`、`===SCORE=== `、`## ===SCORE===`、小写 `===score===`、
// 行首缩进），任何一种都会让整段被吞。这里放宽外围修饰、等号数量与大小写；段名仍
// 限定在 KNOWN_SECTIONS 内，避免把考生正文里偶然出现的 `===xxx===` 误当成段头。
export function extractSections(raw) {
  const text = String(raw || "").replace(/\r\n/g, "\n");
  const re = /^[ \t>*#]*={2,}[ \t]*([A-Za-z_]+)[ \t]*={2,}[ \t*#]*$/gm;
  const markers = [];
  let m = re.exec(text);
  while (m) {
    const name = m[1].toUpperCase();
    if (KNOWN_SECTIONS.has(name)) {
      markers.push({ name, start: m.index, bodyStart: re.lastIndex });
    }
    m = re.exec(text);
  }
  const sections = {};
...
// 段落解析的留痕。这类故障是完全静默的 —— 报告照出、分数照显示，只是批注/范文/
// 修订版/行动建议凭空少了，前端无从分辨「模型没写」和「写了但没解析出来」。
// 关键段缺两个以上就把证据随报告一起存下来（落进 sessions.details.feedback），
// 省得下次还要让用户开 DevTools 现挖。正常报告不写这个字段，不多占一个字节。
//
//   markerLines —— 原始输出里所有像段落标记的行，直接回答「模型到底写成了什么样」
//   rawTail     —— 尾部片段，回答「是不是写到一半被截断了」
const DIAGNOSTIC_SECTIONS = ["ANNOTATION", "CORRECTED", "PATTERNS", "COMPARISON", "ACTION"];

export function buildParseDiagnostics(cleaned, sections) {
  const missing = DIAGNOSTIC_SECTIONS.filter((name) => !sections[name]);
...
export function parseReport(rawText) {
  try {
    ...
    return {
      ...
      sections,
      sectionStates,
      parseDiagnostics: buildParseDiagnostics(cleaned, sections),
