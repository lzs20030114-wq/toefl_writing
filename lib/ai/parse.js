...
function parseScoreSection(text) {
  ...
  const scoreMatch = src.match(/(?:score|分数)\s*[:：]\s*([0-5](?:\.5)?)/i);
...
function parsePatternsSection(text) {
  ...
  try {
    ...
    if (Array.isArray(parsedWhole)) return parsedWhole;
    if (parsedWhole && Array.isArray(parsedWhole.patterns)) return parsedWhole.patterns;
    if (parsedWhole && parsedWhole.tag) return [parsedWhole];
  } catch {
  ...
  const actions = blocks
    .map((block) => {
      ...
      return {
        ...
        importance: String(importance || "").trim(),
...
export function parseReport(rawText) {
  try {
    ...
    const sectionStates = {
      ...
      ANNOTATION: { ok: !annotation.parsed.parseError, raw: sections.ANNOTATION || "" },
      PATTERNS: { ok: !sections.PATTERNS || patterns.length > 0, raw: sections.PATTERNS || "" },
      COMPARISON: {
