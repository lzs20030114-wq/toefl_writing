...
import { buildAnnotationSegments, countAnnotations, reanchorToSource } from "../annotations/parseAnnotations.js";
...
/**
 * Map individual task rubric score (0–5, 0.5 steps) to a band label.
 * Labels align with ETS Writing section performance descriptors.
 */
function scoreToBand(score) {
  ...
  return BAND_LABELS[s] || "Below Basic";
}

export function calibrateDiscussion(result, responseText, baseScore) {
  const signals = result.signals || {};
  const rubric = normalizeRubric(result.rubric, Number(result.score) || 0);
  let finalScore = Number.isFinite(baseScore) ? baseScore : rubric.weighted_score;
  const reasons = [];

  // Purely mechanical rule: word count floor
  if (wc(responseText || "") < MIN_DISCUSSION_WORDS_FOR_GUARDRAIL) {
    finalScore = Math.min(finalScore, 2);
    reasons.push("word_count_floor");
  }

  // Signal-driven rules (consume AI semantic judgement, no regex)
  if (signals.stance_clear === false && finalScore > 3) {
    finalScore = Math.min(finalScore, 3);
    reasons.push("ai_no_stance");
  }
  if (signals.engages_discussion === false && finalScore > 3) {
    finalScore = Math.min(finalScore, 3);
    reasons.push("ai_no_engagement");
  }
  if (signals.has_example === false && finalScore > 4) {
    finalScore = Math.min(finalScore, 4);
    reasons.push("ai_no_example_cap");
  }

  return { finalScore: clampScore(finalScore), reasons, rubric };
}

function getAnnotationState(result, responseText) {
  const parsed = result?.annotationParsed || {};
  const base = {
    plainText: String(parsed.plainText || responseText || ""),
    annotations: Array.isArray(parsed.annotations) ? parsed.annotations : [],
  };
  // The user's submitted text is the source of truth for display — remap
  // marks off the AI's (possibly word-dropping) echo onto the real text.
  const anchored = reanchorToSource(base, responseText);
  return { plainText: anchored.plainText, annotations: [...anchored.annotations] };
}

// 批注一律以模型实际产出为准：曾经的 addBlueRefinements 会在「无批注」或
// 「≥4.5 分且无蓝标」时往原文第一句塞两句硬编码英文（"Can be refined for
// smoother flow…"），落点与句意无关，是报告套话感的直接来源之一。高分报告
// 宁可少而真——这里只做「把模型批注重新锚回用户原文」这一步，不再合成任何批注。
function ensureAnnotationsByScore(result, responseText) {
  return getAnnotationState(result, responseText);
}
...
function detectConfidenceState(type, responseText, confidence, keyProblems) {
  ...
  const hasKeyProblems = Array.isArray(keyProblems) && keyProblems.length > 0;
  ...
  const thinEvidenceThreshold = type === "discussion" ? 70 : 60;
  if (!hasKeyProblems && words < thinEvidenceThreshold) reasons.push("insufficient_qualitative_evidence");
...
export function calibrateScoreReport(type, result, responseText) {
  ...
  let rubric = normalizeRubric(result.rubric, parsedScore);
  const rawScore = rubric.weighted_score;
  // Holistic reconciliation: ETS scoring is holistic-first — the band
  // descriptors describe whole responses, and the official 5-samples are
  // "noisy but excellent" essays whose language DIMENSION reads low even
  // when the errors are timed-writing noise. A pure 40/30/30 weighted mean
  // therefore structurally caps such essays around 4 (metrics ceiling
  // observed in golden-set round 2: ETS-5 essays pinned at 3.5). When the
  // model's holistic band is higher than the weighted mean, let it lift the
  // result by at most half a band; guardrail caps below still win. A
  // holistic score BELOW the weighted mean never drags it down.
  let finalScore = rawScore;
  if (parsedScore > rawScore) {
    finalScore = Math.min(parsedScore, rawScore + 0.5);
  }
  let adjusted = false;
  const reasons = [];
  // holistic_lift 抬升了 finalScore：分数被动过,adjusted 必须同步置真,否则
  // calibration.adjusted 会与实际发生的抬升不一致(前端「已校准」标记失真)。
  if (finalScore !== rawScore) {
    reasons.push("holistic_lift");
    adjusted = true;
  }

  if (type === "email") {
    // 短邮件封顶必须看抬升后的 finalScore,不能看 rawScore:holistic_lift 会把
    // rawScore<4 的短文抬过 4 分线(如 rawScore 3.6 + 0.5 → 4.1),用 rawScore>=4
    // 判断会让这类短文旁路掉本封顶,以 4.0 逃逸。
    if (finalScore >= 4 && wc(responseText || "") < 50) {
      finalScore = Math.min(finalScore, 3);
      adjusted = true;
      reasons.push("email_thin_response_cap");
    }

    // Semantic guardrails driven by the AI's own ===GOALS=== judgement,
    // mirroring the prompt's 硬性规则 (any goal MISSING → ≤3; two PARTIAL
    // → ≤3; one PARTIAL → ≤4). These replace the old hard-coded phrase
    // caps ("really enjoyed", "subscriber of", …) that were overfit to two
    // calibration essays and silently knocked well-written service/leisure
    // emails from 4+ down to 3 — a chronic source of over-harsh scores.
    const goals = Array.isArray(result?.goals) ? result.goals : [];
    const missing = goals.filter((g) => String(g?.status || "").toUpperCase() === "MISSING").length;
    const partial = goals.filter((g) => String(g?.status || "").toUpperCase() === "PARTIAL").length;
    if (goals.length > 0) {
      if (missing >= 1 && finalScore > 3) {
        finalScore = Math.min(finalScore, 3);
        adjusted = true;
        reasons.push("email_goal_missing_cap");
      } else if (partial >= 2 && finalScore > 3) {
        finalScore = Math.min(finalScore, 3);
        adjusted = true;
        reasons.push("email_goals_partial_cap");
      } else if (partial === 1 && finalScore > 4) {
        finalScore = Math.min(finalScore, 4);
        adjusted = true;
        reasons.push("email_goal_partial_cap");
      }
    }
  }

  if (type === "discussion") {
    const disc = calibrateDiscussion(result, responseText, finalScore);
    finalScore = disc.finalScore;
    rubric = disc.rubric;
    disc.reasons.forEach((r) => reasons.push(r));
    // 用 || 合并而不是直接赋值:上游 holistic_lift 已置的 adjusted=true
    // 不能被 discussion 分支平覆盖打回 false。
    adjusted = adjusted || disc.reasons.length > 0;
  }

  finalScore = clampScore(finalScore);
  const finalBand = scoreToBand(finalScore);

  const annotationState = ensureAnnotationsByScore(result, responseText);
