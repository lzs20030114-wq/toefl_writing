import { getSavedCode } from "../AuthContext";

// Stable per-browser id for server-side error attribution. Best-effort:
// falls back to "" when storage/crypto are unavailable (e.g. SSR/tests).
function resolveClientId() {
  try {
    const key = "toefl-client-id";
    const existing = localStorage.getItem(key);
    if (existing) return existing;
    const generated =
      typeof crypto !== "undefined" && typeof crypto.randomUUID === "function"
        ? crypto.randomUUID()
        : `${Date.now()}-${Math.random().toString(36).slice(2, 10)}`;
    localStorage.setItem(key, generated);
    return generated;
  } catch {
    return "";
  }
}

// Turn a non-2xx /api/ai response into an Error the mappers can categorize.
// Reads the body so callers can distinguish a free-tier daily limit
// (DAILY_LIMIT → show an upgrade path) from a transient rate limit instead of
// collapsing every 429 into a futile "server busy" retry. The message stays
// "API error <status>" so status-based categorization (401/403/429) keeps
// working; the daily-limit signal rides along on err.code.
async function readHttpError(r) {
  let respBody = null;
  try { respBody = await r.json(); } catch {}
  const err = new Error("API error " + r.status);
  err.status = r.status;
  err.code = respBody && respBody.code ? String(respBody.code) : "";
  err.serverMessage = respBody && respBody.error ? String(respBody.error) : "";
  return err;
}

// Shared /api/ai request core — one POST wrapped in the outer timeout race.
// Returns the raw response body `d` (has `d.content`, and optionally
// `d.contents` when samples>1). callAI/callAIMulti thin-wrap this so error
// mapping (err.status/err.code/err.serverMessage), AbortController and the
// 150s outer timeout stay byte-identical across both entry points.
async function requestAI(system, message, maxTokens, timeoutMs, temperature, samples) {
  let timeoutId;
  const controller = new AbortController();
  const clientId = resolveClientId();

  try {
    const requestPromise = (async () => {
      const body = {
        system,
        message,
        maxTokens: maxTokens || 2000,
        temperature,
        userCode: getSavedCode() || "",
      };
      // 只在多采样时带 samples,让单采样请求体与旧版逐字一致(不破坏其他 callAI 调用方)。
      if (samples && samples > 1) body.samples = samples;
      const r = await fetch("/api/ai", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          ...(clientId ? { "X-Client-Id": clientId } : {}),
        },
        signal: controller.signal,
        body: JSON.stringify(body),
      });
      if (!r.ok) throw await readHttpError(r);
      const d = await r.json();
      if (d.error) throw new Error(d.error);
      return d;
    })();

    const timeoutPromise = new Promise((_, reject) => {
      timeoutId = setTimeout(() => {
        controller.abort();
        reject(new Error("API timeout"));
      }, timeoutMs);
    });

    return await Promise.race([requestPromise, timeoutPromise]);
  } finally {
    clearTimeout(timeoutId);
  }
}

function isNonEmptyContent(content) {
  return typeof content === "string" && content.trim().length > 0;
}

// AI 辅助类调用(练习记录「AI 解释」、划词讲解、错题分析…)统一的 token 预算。
//
// 这个数字**不是**期望的输出长度——那些讲解实际只有 3-5 句中文(百来 token)。它是给
// 推理留的余量:deepseek-v4-flash 是推理型模型,reasoning_tokens 计入 max_tokens 预算
// (实测记录见 lib/ai/writingEval.js:73 —— 写作评分 4000 预算下约 10% 采样被推理吃光,
// finish_reason=length 且正文为空)。这些辅助调用原本给 260-700,远在推理长度量级以下,
// 于是推理稍长就没正文了,前端表现为「AI 解释点了不出内容也不报错」(2026-09-13 修)。
//
// max_tokens 只是上限:模型用不到就不计费,调大几乎零成本;而一次空响应是 100% 白花钱。
// 所以这里宁可给足。改小之前请先读 app/api/ai/route.js 的 isNonEmptyContent 注释。
export const AI_HELPER_MAX_TOKENS = 2000;

// 讲解预算**分档** —— 2026-09-14。
//
// 上面那条常量是 2026-09-13 为治「空正文」一刀切抬上来的,6 个讲解调用点全用 2000。
// 副作用是把快活儿也拖进了慢档:max_tokens 是含推理的上限,窗口给多大,最坏情况就能
// 想多久。查一个词、看一句造句,本来几秒就该出字的,被迫和「读完整篇阅读讲选项陷阱」
// 共用同一个思考窗口。
//
// 所以按「这次讲解要读多少东西」分档。**但不能只是调小**:700 那一档正是 09-13 出
// 事故的量级(推理吃光预算 → 正文为空)。配套机制是 callAIStream 会带上
// retryMaxTokens,服务端遇到空正文就在**同一次请求、同一次计量**里升到 2000 重来一次
// (app/api/ai/route.js 的 withBudgetEscalation)。于是:常见情况走快档,罕见的长推理
// 自动兜到满档,用户两头都不吃亏。
//
// 要调档先看后台 /admin-api-errors 里 errorType=budget_escalated 的频次:
// 某一档频繁升档就说明它给小了,那是真实数据,不要凭感觉改。
export const AI_EXPLAIN_BUDGET = {
  // 划词讲解:输入是一个词 + 它所在的一句话。
  word: 800,
  // 单句 / 短文级:造句语法点、填词(CTW 原文才 ~120 词)、错题本单题选项陷阱。
  sentence: 1200,
  // 整篇级:阅读/听力讲解要通读原文定位答案句,错题本汇总要横看多道错题。
  passage: AI_HELPER_MAX_TOKENS,
};

export async function callAI(
  system,
  message,
  maxTokens,
  // Default outer timeout — generous enough that long DeepSeek responses
  // (especially writing evaluation, which can stream 2K+ tokens) finish
  // before the user sees a bogus "评分失败". Inner HTTP timeout in
  // deepseekHttp.js is slightly tighter so we surface a network-layer
  // error before the outer race kicks in.
  timeoutMs = 150000,
  temperature = 0.3
) {
  const d = await requestAI(system, message, maxTokens, timeoutMs, temperature, 1);
  // 空正文绝不能当成一次成功的回答返回。服务端(app/api/ai/route.js)现在已把它判成
  // 502,这里是灰度窗口/旧部署的兜底:调用方(AI 解释类 hook)拿到空串会照常写进 state
  // 和缓存,`ex?.text` 是假值于是渲染回按钮、error 又是 null —— 用户看到的就是
  // 「点了没反应、也不报错」。文案沿用 "empty ai response",mapScoringError 已按这个
  // 关键词分类。
  if (!isNonEmptyContent(d.content)) throw new Error("Empty AI response");
  return d.content;
}

// Multi-sample variant for writing evaluation's「三路取中位」. One HTTP request
// (so daily usage is metered once), N parallel DeepSeek calls server-side.
// Returns a non-empty string[] of raw AI outputs. When the server returns a
// `contents` array we filter out empties; if it lacks `contents` (old server
// during a rollout window) we degrade to a single-element [content].
export async function callAIMulti(
  system,
  message,
  maxTokens,
  timeoutMs = 150000,
  temperature = 0.3,
  samples = 3
) {
  const d = await requestAI(system, message, maxTokens, timeoutMs, temperature, samples);
  if (Array.isArray(d.contents)) {
    const filtered = d.contents.filter(isNonEmptyContent);
    if (filtered.length > 0) return filtered;
  }
  // 降级到单采样字段时同样不接受空正文——返回 [""] 只会让 parseReport 报一个
  // 与根因无关的「格式异常」,把「上游没出正文」这件事藏掉。
  if (isNonEmptyContent(d.content)) return [d.content];
  throw new Error("Empty AI response");
}

// AI 辅助讲解的「静默超时」与「总时长上限」。
//
// 为什么不再用 callAI 那个总时长超时:讲解链路以前是 60s **总时长**,而
// deepseek-v4-flash 会先推理再出正文,推理阶段一个 content 字节都不产生。预算从
// 260-700 抬到 2000(见 AI_HELPER_MAX_TOKENS)之后,「思考 50s + 出正文 15s」是完全
// 正常的一次成功调用,却必然撞穿 60s —— 用户看到「AI 响应超时,请重试」,而服务端那
// 次回答其实生成完了,只是没人接,用量还照扣。2026-09-14 修。
//
// 改成流式后判据换成「多久没收到任何字节」:服务端在推理阶段会发 SSE 注释行当心跳,
// 所以只要上游还活着,idle 计时器就会被不断清零;真卡死时 45s 就报错,比旧版更快。
// 总时长上限只是兜底,必须小于 Vercel 的 maxDuration(180s)与服务端预算(165s)。
export const AI_HELPER_IDLE_TIMEOUT_MS = 45000;
export const AI_HELPER_TOTAL_TIMEOUT_MS = 170000;

// SSE 里服务端主动报的错({error, status?, code?})→ 与 readHttpError 同形的 Error,
// 好让 mapAiHelperError 用同一套 status/code 分类,不必为流式再写一份文案表。
function buildStreamError(obj) {
  const status = Number(obj?.status) || 0;
  const err = new Error(status ? "API error " + status : String(obj?.error || "AI stream error"));
  if (status) err.status = status;
  if (obj?.code) err.code = String(obj.code);
  err.serverMessage = String(obj?.error || "");
  return err;
}

/**
 * 流式版的辅助讲解调用。返回完整正文;每收到一段就用累计正文调一次 onDelta,
 * 调用方可以边收边渲染(用户不必对着「分析中...」干等)。
 *
 * 服务端不支持流式时(本地 DEEPSEEK_PROXY_URL 代理路径、测试里的 fetch mock)
 * 会回普通 JSON,这里自动降级成一次性返回 —— 调用方无需分两种写法。
 */
export async function callAIStream(system, message, maxTokens, options = {}) {
  const {
    onDelta,
    temperature = 0.3,
    idleTimeoutMs = AI_HELPER_IDLE_TIMEOUT_MS,
    totalTimeoutMs = AI_HELPER_TOTAL_TIMEOUT_MS,
  } = options;

  const budget = maxTokens || AI_EXPLAIN_BUDGET.passage;
  const controller = new AbortController();
  const clientId = resolveClientId();
  let idleTimer = null;
  let totalTimer = null;
  let timedOut = false;

  // 放弃时既 abort 又 reject 一个哨兵 Promise,每一步都跟它 race。
  // 光靠 abort 是不够的:那要求「abort 一定会让挂起的 reader.read() 报错」,
  // 一旦哪层没照办(旧 polyfill / 中间层包装过的 body),这个 await 就永远挂着,
  // 用户等到的不是错误而是一个转不完的「分析中...」。超时必须由我们自己兜底。
  let rejectGiveUp = null;
  const giveUpPromise = new Promise((_, reject) => {
    rejectGiveUp = () => reject(new Error("API timeout"));
  });
  giveUpPromise.catch(() => {}); // 没人 race 到它时不报 unhandled rejection
  const giveUp = () => {
    timedOut = true;
    controller.abort();
    if (rejectGiveUp) rejectGiveUp();
  };
  const armIdle = () => {
    clearTimeout(idleTimer);
    idleTimer = setTimeout(giveUp, idleTimeoutMs);
  };

  try {
    totalTimer = setTimeout(giveUp, totalTimeoutMs);
    armIdle();

    const r = await Promise.race([giveUpPromise, fetch("/api/ai", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        ...(clientId ? { "X-Client-Id": clientId } : {}),
      },
      signal: controller.signal,
      body: JSON.stringify({
        system,
        message,
        maxTokens: budget,
        temperature,
        stream: true,
        // 空正文时服务端可升到的上限(同一次请求内完成,不额外计量)。
        // 已经在满档的调用不带它,免得服务端做一次无意义的同档重试。
        ...(budget < AI_HELPER_MAX_TOKENS ? { retryMaxTokens: AI_HELPER_MAX_TOKENS } : {}),
        userCode: getSavedCode() || "",
      }),
    })]);
    if (!r.ok) throw await readHttpError(r);

    const contentType = String(r.headers?.get?.("content-type") || "");
    const reader = contentType.includes("text/event-stream") ? r.body?.getReader?.() : null;
    if (!reader) {
      // 非流式响应(代理路径 / 旧部署 / 测试 mock):按老路径一次性取正文。
      const d = await r.json();
      if (d.error) throw new Error(d.error);
      if (!isNonEmptyContent(d.content)) throw new Error("Empty AI response");
      if (onDelta) onDelta(d.content);
      return d.content;
    }

    const decoder = new TextDecoder("utf-8");
    let buffer = "";
    let text = "";
    let streamError = null;
    for (;;) {
      const { value, done } = await Promise.race([giveUpPromise, reader.read()]);
      if (done) break;
      armIdle(); // 任何字节(含心跳注释行)都算「还活着」
      buffer += typeof value === "string" ? value : decoder.decode(value, { stream: true });
      let idx;
      while ((idx = buffer.indexOf("\n")) >= 0) {
        const line = buffer.slice(0, idx).replace(/\r$/, "");
        buffer = buffer.slice(idx + 1);
        // 空行是 SSE 的事件分隔符,":" 开头是心跳注释 —— 都只用来续命,不带数据。
        if (!line || line.startsWith(":") || !line.startsWith("data:")) continue;
        const payload = line.slice(5).trim();
        if (!payload) continue;
        let obj;
        try {
          obj = JSON.parse(payload);
        } catch {
          continue; // 半截/脏行不该毁掉整次讲解
        }
        if (obj.error) {
          streamError = buildStreamError(obj);
          break;
        }
        if (typeof obj.delta === "string" && obj.delta) {
          text += obj.delta;
          if (onDelta) onDelta(text);
        }
      }
      if (streamError) break;
    }
    if (streamError) throw streamError;
    // 与 callAI 同一条铁律:空正文绝不当成功(否则又是「点了没反应也不报错」)。
    if (!isNonEmptyContent(text)) throw new Error("Empty AI response");
    return text;
  } catch (e) {
    // 自己掐断的一律归类成超时,文案走 mapAiHelperError 的 "api timeout" 分支。
    if (timedOut || e?.name === "AbortError") throw new Error("API timeout");
    throw e;
  } finally {
    clearTimeout(idleTimer);
    clearTimeout(totalTimer);
    try { controller.abort(); } catch {}
  }
}

// A free user out of their daily quota — distinct from a transient rate limit.
// The UI should offer 升级 Pro here rather than a retry that can never succeed today.
export function isDailyLimitError(err) {
  if (err && err.code === "DAILY_LIMIT") return true;
  const m = String(err?.message || err || "").toLowerCase();
  return m.includes("daily limit reached");
}

export function mapScoringError(err) {
  const raw = String(err?.message || err || "");
  const m = raw.toLowerCase();
  if (isDailyLimitError(err)) return "今日免费次数已用完，升级 Pro 可无限练习";
  if (m.includes("empty ai response")) return "评分失败，AI服务暂时不可用";
  if (m.includes("api timeout")) return "AI 响应超时，请重试";
  if (m.includes("api error 401") || m.includes("api error 403")) return "认证失败（401/403）";
  if (m.includes("api error 429")) return "AI服务繁忙（429），请稍后重试";
  if (m.includes("unexpected token") || m.includes("json") || m.includes("parse")) return "AI返回格式异常，请重试";
  if (m.includes("api error")) return "评分服务暂时不可用";
  if (m.includes("failed to fetch") || m.includes("network")) return "网络连接异常，请检查后重试";
  return "评分失败，请重试";
}

// AI 解释/辅助类调用(练习记录里的「AI 解释」按钮等)的错误文案。
//
// 与 mapScoringError 分开:那条是写作评分专用的,文案都以「评分」开头,解释场景复用
// 会答非所问。关键是**绝不把 err.message 原样渲染给用户** —— 4 个 explain hook 以前
// 都写「error: e.message」,线上一次 PostgREST 504 就变成用户看不懂、也不知道能重试的
// 红字 "API error 403"(事故说明见 lib/userLookup.js)。
export function mapAiHelperError(err) {
  if (isDailyLimitError(err)) return "今日免费次数已用完，升级 Pro 可无限使用";
  const code = String(err?.code || "");
  const status = Number(err?.status) || 0;
  const m = String(err?.message || err || "").toLowerCase();
  if (code === "PRO_DAILY_CAP") return "服务繁忙，请稍后再试";
  // 402=上游余额不足 / 502=上游挂了 / 503=我们这边查库失败:对用户都是「暂时,可重试」。
  if (code === "USER_LOOKUP_FAILED" || status === 402 || status === 502 || status === 503) {
    return "服务暂时不可用，请重试";
  }
  if (status === 401 || status === 403) return "登录状态异常，请重新登录后重试";
  if (status === 429) return "AI 服务繁忙，请稍后重试";
  if (m.includes("empty ai response")) return "AI 没返回内容，请重试";
  if (m.includes("api timeout")) return "AI 响应超时，请重试";
  if (m.includes("failed to fetch") || m.includes("network")) return "网络连接异常，请检查后重试";
  return "生成失败，请重试";
}

