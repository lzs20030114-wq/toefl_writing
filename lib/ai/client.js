...
// 服务端能接受的 clientTimeoutMs 区间(与 app/api/ai/route.js 的校验一致)。
// 越界就干脆不带这个字段,让服务端用它自己的默认预算 —— 绝不能为了对齐超时
// 反而把请求送成 400。
const MIN_REPORTABLE_TIMEOUT_MS = 5000;
const MAX_REPORTABLE_TIMEOUT_MS = 600000;

// 「发出去就失败」的一次补发。TypeError(Failed to fetch)说明请求根本没走通,
// 服务端不会收到、也就不会计用量,补发一次是安全的。只在**快速**失败时补:
// 慢失败说明请求很可能已经到了服务端,重发就有重复计费的风险。
const FAST_FAIL_RETRY_WINDOW_MS = 5000;
const FAST_FAIL_RETRY_DELAY_MS = 400;
...
async function fetchWithFastRetry(url, init) {
  ...
  try {
    ...
    // 我们自己(超时)或调用方主动中止的,不补发。
    if (init?.signal?.aborted || e?.name === "AbortError") throw e;
    if (Date.now() - startedAt > FAST_FAIL_RETRY_WINDOW_MS) throw e;
    await new Promise((resolve) => setTimeout(resolve, FAST_FAIL_RETRY_DELAY_MS));
    return await fetch(url, init);
  }
}
...
// outer timeout stay byte-identical across both entry points.
//
// opts.signal:调用方的中止信号(例:面试交卷时掐掉在途评分)。它一中止,这次请求
// 立刻收手,不再占着一条到期才结束的连接。
async function requestAI(system, message, maxTokens, timeoutMs, temperature, samples, opts = {}) {
  let timeoutId;
  const controller = new AbortController();
  const clientId = resolveClientId();
  const externalSignal = opts.signal || null;
  const onExternalAbort = () => { try { controller.abort(); } catch { /* 已中止 */ } };
  if (externalSignal) {
  ...
  try {
    const requestPromise = (async () => {
      ...
      // 服务端据此把自己的上游预算收敛到我们放弃之前(见 resolveUpstreamBudget)。
      // 不对齐的后果:客户端 60s 就报错了,服务端还在等 165s —— 白烧 DeepSeek 的
      // token,而且那条失败会记成 internal 500 而不是超时(2026-09-20 事故)。
      const reportable = Math.trunc(Number(timeoutMs));
      if (Number.isFinite(reportable)
        && reportable >= MIN_REPORTABLE_TIMEOUT_MS
        && reportable <= MAX_REPORTABLE_TIMEOUT_MS) {
        body.clientTimeoutMs = reportable;
      }
      // 只在多采样时带 samples,让单采样请求体与旧版逐字一致(不破坏其他 callAI 调用方)。
      if (samples && samples > 1) body.samples = samples;
      const r = await fetchWithFastRetry("/api/ai", {
        method: "POST",
        headers: {
...
export async function callAI(
  ...
  const d = await requestAI(system, message, maxTokens, timeoutMs, temperature, 1, opts);
...
export async function callAIMulti(
  ...
  const d = await requestAI(system, message, maxTokens, timeoutMs, temperature, samples, opts);
...
// 上游(DeepSeek)在排队/变慢,服务端按预算主动掐断 → 504 + UPSTREAM_TIMEOUT。
// 与「网络不通」必须分开:它跟用户的网络毫无关系,让人去查网络只会白折腾
// (2026-09-20 事故里用户看到的就是「网络连接异常，请检查后重试」)。
export function isUpstreamQueuedError(err) {
  if (!err) return false;
  if (String(err.code || "") === "UPSTREAM_TIMEOUT") return true;
  return Number(err.status) === 504;
}
...
export function mapScoringError(err) {
  ...
  if (isUpstreamQueuedError(err)) return "AI 正在排队，请稍后重试";
