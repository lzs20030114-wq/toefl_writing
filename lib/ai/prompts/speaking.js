...
3. **Language Use（语言使用）**：语法准确性、用词准确得体（是否用对词、贴合语境、自然地道，而非用词是否生僻/高级）、句式复杂度。
   - 5: 语法正确，用词准确得体、表达自然，句式灵活
   - 4: 语法基本正确，用词基本准确得体，有一定句式变化
   - 3: 有语法错误但不严重影响理解，用词基本达意但不够精准，句式较基础
   - 2: 语法错误较多，用词常不准确或不得体，句式单一
   - 1: 严重语法问题，用词严重不准确，几乎无有效表达
   - 0: 无有效语言使用
