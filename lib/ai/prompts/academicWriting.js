...
- 5.0 (Advanced)：立场鲜明，论证有力且有具体例证；充分互动讨论语境；逻辑连贯、过渡自然；语言准确多样，几乎无错误
...
/**
 * Real TOEFL Academic Discussion question statistics (from 81 reference questions):
 *
...
// Course list calibrated to 81 real TPO items: these are the ONLY 12 courses
// observed in real TOEFL Academic Discussion items. The previous expanded
...
- Use "Professor" as the name (94% of real TPO uses this exact string).

STUDENT RESPONSES (calibrated to real TPO: mean=429 chars):
- Exactly 2 students with contrasting stances.
- Length: 250–700 characters each. Target ~430 chars.
  Real TPO students average 429 chars. Do NOT cap at 400.
  Maximum 4-5 sentences per student; do NOT pad to fill a length target.
- Student lengths can be similar OR different — see "STUDENT VOICES" above. No mandatory diff.
- Student 2 referencing Student 1 by name: 37% of real TPO does this. Follow the user prompt
  instruction exactly when specified; otherwise default to NOT referencing (independent stance).
...
// Opening style pool calibrated to 81 real TPO items.
// Observed: 36% "today", 4% "this_week", 4% "factual", 2% "as_discussed",
//           2% "over_weeks", 0% "recent" — but 52% used an "other" opener
// that doesn't fit any of these 6 templates. So we keep the templates
// (matching real frequencies) and ADD an explicit "other" bucket weighted
// at 52% so the model has permission to write whatever opener fits naturally.
export const DISC_OPENING_STYLES = [
  { weight: 36, style: "today", instruction: 'Start with "Today, we\'re going to talk about [topic]."' },
  { weight: 52, style: "natural", instruction: 'Open in whatever natural way a professor might begin a discussion post on this topic. Do not force a template. Examples observed in real TOEFL: starting with a question, leading with a brief anecdote, naming a recent event, framing a tension/contradiction, or referencing a course reading. Choose what fits the topic.' },
  { weight: 4, style: "this_week", instruction: 'Start with "For this week\'s discussion, let\'s think about [topic]."' },
  { weight: 4, style: "factual", instruction: 'Start with a factual background statement about the topic (e.g., "Many countries have experienced...", "The number of X has increased...").' },
  { weight: 2, style: "as_discussed", instruction: 'Start with "As we/I discussed/mentioned in class, [context]..."' },
  { weight: 2, style: "over_weeks", instruction: 'Start with "Over the next few weeks, we are going to look at/learn about [topic]."' },
];
