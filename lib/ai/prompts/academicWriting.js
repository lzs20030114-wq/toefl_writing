...
- 4.0 (High-Intermediate)：立场明确，有例证但可更具体/有力；回应了他人但互动可更深入；逻辑通顺；语言有一些局部小错但每一处意思都清楚
- 3.5 (Intermediate+)：立场明确且有一定论证，但例证不够具体或说服力不足；有基本互动；逻辑尚可但过渡偶有生硬；或个别语言错误已造成局部理解障碍。比 3 分强但达不到 4 分的论证质量
- 3.0 (Intermediate)：有立场但论证薄弱，缺乏有力例证；互动浅或缺失；逻辑有断裂；语言错误频繁且多处影响理解
...
- 错误看严重度、不数数量（ETS 校准原则）：语言使用的判断标准是「错误是否妨碍读者理解」，不是数错误个数。介词、时态、单复数、词形这类局部小错即使较多，只要每处意思一目了然，语言使用维度仍应给 3-3.5，总分仍可落在 4-4.5；只有当错误让句意模糊、误读或需要重读时，才据此把总分压到 3.5 以下。
...
- OPENER FORMULA (real 2026改后 is highly formulaic — match it):
