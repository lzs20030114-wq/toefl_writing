...
判错误分两类，分界不是「错误的语法类别」而是「是否体现系统性语法失控 / 是否妨碍理解」：
  ① 不压分的小错（限时噪声）：拼写/打字滑误（airpline、irradicate、imense）、标点后未空格、大小写失误（brazilian）、there/their 替换、冠词或介词滑误（a important、any place of the world）、**偶发的代词指代错、单处时态摇摆、个别搭配不地道**——只要它不妨碍理解、也不反映考生「连基本语法都不会」。这类是 competent writer under timed conditions 的正常产物。
