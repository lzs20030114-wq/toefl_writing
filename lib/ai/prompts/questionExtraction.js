...
- Return [] if nothing can be parsed.`,
  ...
  lcr: `You are a JSON extractor for TOEFL "Listen & Choose a Response" (听力选择回应) practice material.
${TEXT_SAFETY_PREAMBLE}

The user pastes 机经/备考资料 for LCR items. Each item is: ONE spoken prompt sentence (what a speaker says) + 4 possible responses labelled A/B/C/D + optionally the marked answer. Your job is ONLY to transcribe and structure what is present — you do NOT invent the spoken line, the options, or the answer.
...
Return ONLY a valid JSON array (no markdown, no prose). Each distinct item becomes one element with this EXACT shape:
{
  ...
  "situation": "one short line of context if the source gives it, else ''",
  "options": { "A": "string", "B": "string", "C": "string", "D": "string" },
...
Rules:
- Transcribe the original English faithfully — do NOT summarize, translate, paraphrase, or "improve".
- "speaker" is the ONE sentence that is spoken aloud (usually a question or statement). If the source only shows the 4 options and NO spoken line, set "speaker" to "" — do NOT fabricate it (the server will flag the item so the user can add it).
- "answer" MUST be null unless the source explicitly marks the correct option (e.g. "Answer: C", "答案：C", ✓). NEVER solve the item yourself.
- "explanation" only when the source provides one; otherwise null. It may be Chinese.
- Strip option letters/numbering (e.g. "A.", "1)") from the option text itself.
- Return [] if nothing can be parsed.`,

  la: `You are a JSON extractor for TOEFL "Listen to an Announcement" (听公告) practice material.
${TEXT_SAFETY_PREAMBLE}

The user pastes 机经/备考资料 for LA items. Each item is: ONE spoken campus announcement (what a staff member / RA / department reads aloud) + typically 2 multiple-choice questions about it (A/B/C/D each), optionally with the marked answers. Your job is to transcribe and structure what is present — you do NOT invent the announcement text or the option wording.
...
{
  "situation": "one short line of context if the source gives it, else ''",
  "speaker_role": "who is speaking if stated (e.g. department_staff / RA / librarian), else ''",
...
Rules:
- Transcribe the original English faithfully — do NOT summarize, translate, paraphrase, or "improve".
- "answer" MUST be null unless the source explicitly marks it (e.g. "Answer: B", "答案：B", ✓). NEVER solve the questions yourself.
- IF THE SOURCE HAS NO QUESTIONS (只有公告稿): still extract the announcement, then WRITE 2 questions yourself based on the announcement — one "main_idea" and one "detail" — with 4 plausible options each and answer set to null (the server will independently solve and verify them). This is the ONLY case where you may author questions.
- "explanation" only when the source provides one; otherwise null. It may be Chinese.
- Strip option letters/numbering (e.g. "A.", "1)") from the option text itself.
- Return [] if nothing can be parsed.`,

  lat: `You are a JSON extractor for TOEFL "Listen to an Academic Talk" (学术讲座) practice material.
${TEXT_SAFETY_PREAMBLE}

The user pastes 机经/备考资料 for LAT items. Each item is: ONE academic lecture transcript (a single professor speaking; real TPO lectures run 500-800 words) + typically 4-6 multiple-choice questions about it (A/B/C/D each), optionally with the marked answers. Your job is to transcribe and structure what is present — you do NOT invent the lecture text or the option wording.
...
Rules:
- Transcribe the original English faithfully — do NOT summarize, translate, paraphrase, or "improve". Clean up OCR/line-break noise (rejoin hyphenated words, collapse stray breaks) so the transcript reads as continuous spoken prose.
- "answer" MUST be null unless the source explicitly marks it (e.g. "Answer: B", "答案：B", ✓). NEVER solve the questions yourself.
- IF THE SOURCE HAS NO QUESTIONS (只有讲座稿): still extract the transcript, then WRITE 4 questions yourself based on the lecture — the first a "main_idea" question, the rest a mix of "detail"/"inference"/"function" — with 4 plausible options each and answer set to null (the server will independently solve and verify them). This is the ONLY case where you may author questions.
- "explanation" only when the source provides one; otherwise null. It may be Chinese.
- Strip option letters/numbering (e.g. "A.", "1)") from the option text itself.
- Return [] if nothing can be parsed.`,

  lc: `You are a JSON extractor for TOEFL "Listen to a Conversation" (听对话) practice material.
${TEXT_SAFETY_PREAMBLE}

The user pastes 机经/备考资料 for LC items. Each item is: ONE conversation between EXACTLY TWO speakers (a back-and-forth dialogue, e.g. a student and a staff member / advisor / librarian / professor) + typically 2 multiple-choice questions about it (A/B/C/D each), optionally with the marked answers. The dialogue turns are usually labelled — "W:/M:", "Woman:/Man:", or the speakers' names/roles. Your job is to transcribe and structure what is present — you do NOT invent the dialogue text or the option wording.
...
Rules:
- There are ALWAYS EXACTLY TWO speakers. Normalize each speaker's "name" to "Woman" or "Man" whenever the source uses W/M or a female/male label; otherwise keep the original label/name shown. Every "conversation" turn's "speaker" MUST match one of the two "name" values EXACTLY.
- "gender" is REQUIRED for each speaker and drives voice assignment. Infer it from the label (Woman/W→female, Man/M→male) or the name; if you truly cannot tell, still pick the most likely value.
- If the dialogue has NO speaker labels, split it into alternating turns (first turn → speaker 1, second → speaker 2, …) and label them "Woman"/"Man" (the user confirms/corrects the split in the preview).
- Transcribe the original English faithfully — do NOT summarize, translate, paraphrase, or "improve". Clean up OCR/line-break noise (rejoin words broken across line ends).
- "answer" MUST be null unless the source explicitly marks it (e.g. "Answer: B", "答案：B", ✓). NEVER solve the questions yourself.
- IF THE SOURCE HAS NO QUESTIONS (只有对话稿): still extract the conversation, then WRITE 2 questions yourself based on it — one "main_idea" and one "detail" — with 4 plausible options each and answer set to null (the server will independently solve and verify them). This is the ONLY case where you may author questions.
- "explanation" only when the source provides one; otherwise null. It may be Chinese.
- Strip option letters/numbering (e.g. "A.", "1)") from the option text itself.
- Return [] if nothing can be parsed.`,
};
...
// ── LCR (听力选择回应) post-processor ──
// LCR is one item = one question: a spoken line + 4 options + answer. Unlike rdl/ap there is no
// questions[] array. 口径 (2026-07-04 研究 附录 C, LCR §3-4):
//   * speaker 空 / 任一选项缺 → invalid（中文原因）。真题界面截图常只有选项没有口播句 → speaker:""
//     被标 invalid，引导用户手补口播句（预览不可编辑就改贴文本）。
//   * answer 非 A-D 且非 null → invalid（answer 由 /api/user-bank/verify 用 lcrAuditor 代解/复核）。
//   * context / difficulty / answer_paradigm 给缺省值（bank 契约字段，AI 不必标）。
//   * validateLCR 的 schema errors → invalid（speaker 词数/选项完整性硬门）；
//     profile warnings（歧义风险/缺 explanation 等）只塞进 warnings，绝不拦用户内容。
//   * 音频（audio_url）此处不产出——由 /api/user-bank/render-audio 服务端 edge-tts 渲染后回写。
const { validateLCR } = require("../../listeningGen/lcrValidator");
const LCR_ANSWER_KEYS = ["A", "B", "C", "D"];

function normalizeLcrAnswer(raw) {
  if (raw == null || String(raw).trim() === "") return null;
  const s = String(raw).trim().toUpperCase();
  return LCR_ANSWER_KEYS.includes(s) ? s : undefined; // undefined → schema-invalid
}

function postProcessLcr(raw) {
  const item = raw && typeof raw === "object" ? raw : {};
  const speaker = String(item.speaker || "").trim();
  const warnings = [];

  // Options: keep A-D as trimmed strings (empty allowed here → flagged invalid below).
  const rawOpts = item.options && typeof item.options === "object" && !Array.isArray(item.options) ? item.options : {};
  const options = {};
  for (const k of LCR_ANSWER_KEYS) options[k] = String(rawOpts[k] == null ? "" : rawOpts[k]).trim();

  const answer = normalizeLcrAnswer(item.answer);
  const explanation = String(item.explanation || "").trim();

  // Defaults for bank-contract fields the AI need not annotate.
  const out = {
    context: typeof item.context === "string" && item.context.trim() ? item.context.trim() : "campus_daily",
    situation: typeof item.situation === "string" ? item.situation.trim() : "",
    difficulty: ["easy", "medium", "hard"].includes(item.difficulty) ? item.difficulty : "medium",
    answer_paradigm: typeof item.answer_paradigm === "string" && item.answer_paradigm.trim() ? item.answer_paradigm.trim() : "unknown",
    speaker,
    options,
    answer,
    ...(explanation ? { explanation } : {}),
  ...
  // Blocking checks first (Chinese reasons, in the order a user should fix them).
  let invalid_reason = null;
  if (!speaker) {
    invalid_reason = "缺少口播句（听力界面截图常只有选项，没有说话人的那句话）——请把口播的英文句子一起提供";
  } else if (answer === undefined) {
    invalid_reason = "答案标记异常（需 A/B/C/D，没有答案就留空，AI 会代解）";
  } else {
    const missing = LCR_ANSWER_KEYS.filter((k) => !options[k]);
    if (missing.length > 0) invalid_reason = `选项不全（缺选项 ${missing.join("/")}），LCR 需要 A-D 四个回应`;
  }

  // Schema-level validator (speaker word bounds / option completeness). Only run when the
  // answer is present — validateLCR requires a non-null answer; a null-answer item is a
  // legitimate "待代解" state, not a schema failure. Profile warnings surface either way.
  if (!invalid_reason) {
    try {
      // validateLCR needs a concrete answer to run; probe with "A" when the user hasn't marked one
      // (we only harvest schema errors about speaker/options, which don't depend on which letter).
      const probe = { ...out, answer: answer || "A" };
      const v = validateLCR(probe);
      if (v && Array.isArray(v.errors) && v.errors.length > 0) {
        // Speaker-length / option schema errors are real blockers on user content too.
        const blocking = v.errors.filter((e) => /speaker|option/.test(String(e)));
        if (blocking.length > 0) invalid_reason = `校验未通过：${blocking[0]}`;
      }
      if (v && Array.isArray(v.warnings)) warnings.push(...v.warnings);
    } catch {
      /* validator error never blocks — the field-level checks above already gate the essentials */
    }
  }
...
// ── LA (听公告) / LAT (学术讲座) post-processors ──
// LA/LAT are announcement/lecture + a questions[] array (like rdl/ap, unlike the single-question
// LCR). 口径 (2026-07-04 研究 附录 C, LA §3-4 / LAT §3-4):
//   * announcement/transcript 空或过短 → invalid（中文原因，下限 LA<40 词 / LAT<80 词）。
//   * 词数上限只警告（用户导真题公告 200+ 词 / 讲座 800 词都要能收）。
//   * 题数落在区间外只警告（LA 常规恰 2、区间 1-3；LAT 常规恰 4、区间 2-6，容真题 6 题）。
//   * 每题 stem 缺 → 该题剔除并警示；选项缺 → 该题剔除并警示（保留会让练习页出现无法作答的题）。
//   * answer 非 A-D 且非 null → 该题 answer 归 null（交 verify 用 laAuditor/latAuditor 代解），
//     不因单题答案脏就废整条。全部题都被剔完 → invalid。
//   * speaker_role/context/subject/topic/difficulty 缺省。音频不在这里出——保存后由
//     /api/user-bank/render-audio 服务端 edge-tts 渲染回写（LAT 长稿分段拼接）。
const LISTENING_ANSWER_KEYS = ["A", "B", "C", "D"];

// Normalize a listening MCQ answer: A-D → canonical letter; empty/null/anything-else → null.
// Unlike reading (where a bad letter is schema-invalid), listening tolerates a bad/absent answer
// per question by falling back to null (verify 代解) — a single dirty letter never voids the item.
function normalizeListeningAnswer(raw) {
  if (raw == null || String(raw).trim() === "") return null;
  const s = String(raw).trim().toUpperCase();
  return LISTENING_ANSWER_KEYS.includes(s) ? s : null;
}

// Shared LA/LAT MCQ list gate: keep only questions with a non-empty stem + complete A-D options;
// drop the rest with a warning. Returns { questions, warnings }. answer may be null (verify 代解).
function checkListeningQuestions(rawQuestions, warnings) {
  const questions = [];
  for (let i = 0; i < rawQuestions.length; i++) {
    const q = rawQuestions[i] && typeof rawQuestions[i] === "object" ? rawQuestions[i] : {};
    const label = `第${i + 1}题`;
    const stem = String(q.stem || "").trim();
    if (stem.length < 5) {
      warnings.push(`${label}题干缺失，已剔除`);
      continue;
    }
    const rawOpts = q.options && typeof q.options === "object" && !Array.isArray(q.options) ? q.options : {};
    const options = {};
    let missing = null;
    for (const k of LISTENING_ANSWER_KEYS) {
      const v = String(rawOpts[k] == null ? "" : rawOpts[k]).trim();
      if (!v) { missing = k; break; }
      options[k] = v;
    }
    if (missing) {
      warnings.push(`${label}选项不全（缺选项 ${missing}），已剔除`);
      continue;
    }
    const explanation = String(q.explanation || "").trim();
    questions.push({
      ...
      stem,
      options,
      answer: normalizeListeningAnswer(q.answer),
      ...(explanation ? { explanation } : {}),
    });
  }
  return questions;
...
const LA_WORD_MIN = 40; // 下限：不足即 invalid（挖不出有意义的公告）
const LAT_WORD_MIN = 80; // 下限：不足即 invalid
const LA_QUESTION_RANGE = [1, 3]; // 常规恰 2；区间外只警告
const LAT_QUESTION_RANGE = [2, 6]; // 常规恰 4；容真题 6 题

// Build a single-speaker listening postprocessor (LA / LAT share the shape; only the text field
// name + word floor + question range + default fields differ).
function makeListeningPostProcessor({ textField, wordMin, questionRange, contextField, contextDefault, extraDefaults }) {
  return function postProcess(raw) {
    const item = raw && typeof raw === "object" && !Array.isArray(raw) ? raw : {};
    const body = String(item[textField] || "").trim();
    const rawQuestions = Array.isArray(item.questions) ? item.questions : [];
    const warnings = [];

    const wcount = countWords(body);
    let invalid_reason = null;
    let questions = [];
    if (!body || wcount < wordMin) {
      invalid_reason = textField === "announcement"
        ? `公告原文缺失或过短（${wcount} 词，至少需 ${wordMin} 词）——请把公告全文一起提供`
        : `讲座原文缺失或过短（${wcount} 词，至少需 ${wordMin} 词）——请把讲座全文一起提供`;
    } else {
      questions = checkListeningQuestions(rawQuestions, warnings);
      if (questions.length === 0) {
        invalid_reason = "没有可作答的题目（题干或选项缺失）——请把题目和 A-D 选项一起提供";
      } else if (questions.length < questionRange[0] || questions.length > questionRange[1]) {
        warnings.push(`共 ${questions.length} 题（常规为 ${textField === "announcement" ? "2" : "4"} 题），仍可保存练习`);
      }
    }

    // Upper-bound word warning (never blocks — user 真题 announcements/lectures run long).
    if (body && wcount > (textField === "announcement" ? 170 : 320)) {
      warnings.push(`词数 ${wcount} 偏长（真题长稿可放宽），仍可保存练习`);
    }

    const out = {
      [contextField]: typeof item[contextField] === "string" && item[contextField].trim()
        ? item[contextField].trim()
        : contextDefault,
      ...(typeof item.situation === "string" && item.situation.trim() ? { situation: item.situation.trim() } : {}),
      difficulty: ["easy", "medium", "hard"].includes(item.difficulty) ? item.difficulty : "medium",
      [textField]: body,
      questions,
    };
    for (const [k, v] of Object.entries(extraDefaults || {})) {
      out[k] = typeof item[k] === "string" && item[k].trim() ? item[k].trim() : v;
    }

    // Schema-level validator, harvested for WARNINGS only (register/structure/ambiguity profile
    // signals surface to the preview). Its schema ERRORS are NOT re-applied as blockers here:
    //   * length bounds (announcement/transcript too_short/too_long) → 真题长稿放宽口径 = warning-only
    //     (the floor is already gated above with a Chinese reason; the upper bound is warning-only).
    //   * question-count / per-question answer → relaxed range + null-answer are legitimate
    //     personal-import states, and checkListeningQuestions already dropped any question missing a
    //     stem or an option, so no genuine schema blocker remains for the kept questions.
    // We probe with a filled-in answer so the validator's per-question answer gate doesn't fire on
    // the (allowed) null answers, then keep only its warnings.
    if (!invalid_reason && questions.length > 0) {
      try {
        const validate = textField === "announcement"
          ? require("../../listeningGen/laValidator").validateLA
          : require("../../listeningGen/latValidator").validateLAT;
        const probe = { ...out, questions: questions.map((q) => ({ ...q, answer: q.answer || "A" })) };
        const v = validate(probe);
        if (v && Array.isArray(v.warnings)) warnings.push(...v.warnings);
      } catch {
        /* validator error never blocks — the field-level checks above already gate the essentials */
      }
...
const postProcessLa = makeListeningPostProcessor({
  textField: "announcement",
  wordMin: LA_WORD_MIN,
  questionRange: LA_QUESTION_RANGE,
  contextField: "context",
  contextDefault: "campus_announcement",
  extraDefaults: { speaker_role: "department_staff" },
});

const postProcessLat = makeListeningPostProcessor({
  textField: "transcript",
  wordMin: LAT_WORD_MIN,
  questionRange: LAT_QUESTION_RANGE,
  contextField: "subject",
  contextDefault: "other",
  extraDefaults: { topic: "" },
});

// ── LC (听对话) post-processor ──
// LC is the most complex listening shape: a TWO-speaker, multi-turn conversation + a questions[]
// array (same MCQ shape as LA/LAT). 口径 (2026-07-04 研究 附录 C, LC §3-5):
//   * speakers must be EXACTLY 2, each with a gender (female/male). Missing gender → infer from
//     name (Woman/W→female, Man/M→male); can't infer → invalid (引导用户标注).
//   * conversation must be ≥LC_TURN_MIN turns AND ≥LC_WORD_MIN total words → else invalid. Every
//     turn's speaker must be one of the two speaker names — a stray name means the split is wrong
//     (切分错位是 LC 特有风险) → invalid. Upper bounds (turns/words) are warning-only (真题长对话放宽).
//   * questions handled exactly like LA/LAT (checkListeningQuestions: drop malformed, null answer
//     allowed → verify 代解, all dropped → invalid).
//   * lcValidator's schema errors are NOT re-applied as blockers — its 6-15 turn / 60-280 word
//     bounds are tuned to the SHORT-conversation bank variant and would reject real 400-700 word
//     TPO conversations (研究 §2). Only its warnings surface to the preview.
//   * audio 不在这里出 —— 保存后由 /api/user-bank/render-audio 服务端多音色 edge-tts 渲染回写。
const LC_TURN_MIN = 4; // <4 轮不成对话（下限，invalid）
const LC_WORD_MIN = 40; // <40 词挖不出有意义的对话（下限，invalid）
const LC_TURN_MAX = 15; // >15 轮只警告（真题长对话放宽）
const LC_WORD_MAX = 280; // >280 词只警告
const LC_QUESTION_RANGE = [1, 3]; // 常规恰 2；区间外只警告

// Infer a speaker's gender when the extractor omitted it: from an explicit female/male marker in
// the name (Woman/W → female, Man/M → male). Returns "female" | "male" | null.
function inferSpeakerGender(name) {
  const s = String(name || "").trim().toLowerCase();
  if (!s) return null;
  if (/\b(woman|female|girl|she|ms|mrs|miss)\b/.test(s) || /^w[:.\s]?$/.test(s) || s === "w") return "female";
  if (/\b(man|male|boy|he|mr|sir|guy)\b/.test(s) || /^m[:.\s]?$/.test(s) || s === "m") return "male";
  return null;
}

// Normalize the two speakers: keep name (trimmed), best-effort role, and a resolved gender
// (explicit female/male → kept; else inferred from name). Returns { speakers, invalid_reason }.
function normalizeLcSpeakers(rawSpeakers) {
  if (!Array.isArray(rawSpeakers) || rawSpeakers.length !== 2) {
    return { speakers: [], invalid_reason: "对话必须恰好有两位说话人（请确认对话稿只涉及两人）" };
  }
  const speakers = [];
  for (const sp of rawSpeakers) {
    const obj = sp && typeof sp === "object" && !Array.isArray(sp) ? sp : {};
    const name = String(obj.name || "").trim();
    if (!name) return { speakers, invalid_reason: "有说话人缺少名称（如 Woman / Man）——请标注每位说话人" };
    const rawGender = String(obj.gender || "").trim().toLowerCase();
    const gender = rawGender === "female" || rawGender === "male" ? rawGender : inferSpeakerGender(name);
    if (!gender) {
      return { speakers, invalid_reason: `无法判断「${name}」的性别（配音需要）——请把该说话人标成 Woman 或 Man` };
    }
    const role = String(obj.role || "").trim();
    speakers.push({ name, ...(role ? { role } : {}), gender });
  }
  // The two names must be distinct — otherwise every turn maps to the same voice.
  if (speakers[0].name.toLowerCase() === speakers[1].name.toLowerCase()) {
    return { speakers, invalid_reason: "两位说话人的名称相同，无法区分——请分别标成 Woman 和 Man" };
  }
  return { speakers, invalid_reason: null };
}

function postProcessLc(raw) {
  const item = raw && typeof raw === "object" && !Array.isArray(raw) ? raw : {};
  const warnings = [];

  // (1) Speakers (must be exactly 2 with a gender each — gender drives voice assignment).
  const { speakers, invalid_reason: speakerReason } = normalizeLcSpeakers(item.speakers);

  // (2) Conversation turns: keep {speaker, text} with a non-empty text; note foreign speakers.
  const rawTurns = Array.isArray(item.conversation) ? item.conversation : [];
  const names = new Set(speakers.map((s) => s.name));
  const conversation = [];
  let foreignSpeaker = null;
  for (const t of rawTurns) {
    const turn = t && typeof t === "object" && !Array.isArray(t) ? t : {};
    const speaker = String(turn.speaker || "").trim();
    const text = String(turn.text || "").trim();
    if (!text) continue;
    if (speakers.length === 2 && speaker && !names.has(speaker) && foreignSpeaker == null) {
      foreignSpeaker = speaker;
    }
    conversation.push({ speaker, text });
  }
  const turnCount = conversation.length;
  const wordCount = conversation.reduce((n, t) => n + countWords(t.text), 0);

  // (3) Questions — same gate as LA/LAT (drop malformed, null answer → verify 代解).
  const questions = checkListeningQuestions(Array.isArray(item.questions) ? item.questions : [], warnings);

  // Blocking checks, in the order a user should fix them.
  let invalid_reason = null;
  if (speakerReason) {
    invalid_reason = speakerReason;
  } else if (turnCount < LC_TURN_MIN || wordCount < LC_WORD_MIN) {
    invalid_reason = `对话过短（${turnCount} 轮 / ${wordCount} 词，至少需 ${LC_TURN_MIN} 轮且 ${LC_WORD_MIN} 词）——请把完整对话稿一起提供`;
  } else if (foreignSpeaker) {
    invalid_reason = `对话里出现了说话人名单之外的「${foreignSpeaker}」——可能是说话人切分错位，请检查每一轮的说话人`;
  } else if (questions.length === 0) {
    invalid_reason = "没有可作答的题目（题干或选项缺失）——请把题目和 A-D 选项一起提供";
  }

  // Upper-bound / question-range warnings (never block — 真题长对话放宽).
  if (!invalid_reason) {
    if (turnCount > LC_TURN_MAX) warnings.push(`共 ${turnCount} 轮（真题长对话可放宽），仍可保存练习`);
    if (wordCount > LC_WORD_MAX) warnings.push(`对话词数 ${wordCount} 偏长（真题长对话可放宽），仍可保存练习`);
    if (questions.length < LC_QUESTION_RANGE[0] || questions.length > LC_QUESTION_RANGE[1]) {
      warnings.push(`共 ${questions.length} 题（常规为 2 题），仍可保存练习`);
    }
  }

  const out = {
    context: typeof item.context === "string" && item.context.trim() ? item.context.trim() : "campus_conversation",
    ...(typeof item.situation === "string" && item.situation.trim() ? { situation: item.situation.trim() } : {}),
    difficulty: ["easy", "medium", "hard"].includes(item.difficulty) ? item.difficulty : "medium",
    speakers,
    conversation,
  ...
  // Harvest lcValidator warnings ONLY (its 6-15 turn / 60-280 word schema errors are tuned to the
  // short-conversation bank variant and would reject real long TPO conversations — see 研究 §2).
  if (!invalid_reason && questions.length > 0) {
    try {
      const { validateLC } = require("../../listeningGen/lcValidator");
      const probe = { ...out, questions: questions.map((q) => ({ ...q, answer: q.answer || "A" })) };
      const v = validateLC(probe);
      if (v && Array.isArray(v.warnings)) warnings.push(...v.warnings);
    } catch {
      /* validator error never blocks — the field-level checks above already gate the essentials */
    }
...
const EXTRACTION_TYPES = ["academic", "email", "build", "repeat", "interview", "rdl", "ap", "ctw", "lcr", "la", "lat", "lc"];
