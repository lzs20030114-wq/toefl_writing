...
- 4.5 (High-Intermediate+)：三个目标充分完成且有细节；语域恰当；语言错误少且全部轻微；组织好。接近满分但有细微瑕疵
- 4.0 (High-Intermediate)：三个目标均完成（允许某个细节稍薄）；语域基本恰当；语言有一些局部小错（介词/时态/词形/主谓一致等），但每一处意思都清楚；组织良好
...
- 错误看严重度、不数数量（ETS 校准原则）：语言使用的判断标准是「错误是否妨碍读者理解」，不是数错误个数。介词、时态、单复数、词形这类局部小错即使较多（十余处），只要每处意思一目了然、收件人无需猜测，语言使用维度仍应给 3-3.5，总分仍可落在 4-4.5；只有当错误让句意模糊、误读或需要重读时，才据此把总分压到 3.5 以下。
