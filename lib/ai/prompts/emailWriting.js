...
判错误分两类，分界不是「语法类别」而是「是否体现系统性语法失控 / 是否妨碍理解」：
  ① 不压分的小错（限时噪声）：拼写/打字滑误、标点后未空格、大小写失误、there/their 替换、冠词或介词滑误、代词指代错、时态摇摆、个别搭配不地道——只要不妨碍理解、也不反映「连基本语法都不会」。
  ...
  ①/② 重复判定线（照此执行，不得凭感觉）：**拼写、代词指代、冠词介词这三类小错即使重复出现也全部仍是 ① 类**（重复的手滑还是手滑，官方 5 分样文正是这种画像）。时态摇摆或搭配不地道各自不超过 2-3 处也是 ① 类。只有「基本动词形态 / 词类误用 / 主谓一致」这类语法根基错出现 2 处以上（锚 B 画像——暖气报修邮件正是 has stop working + has starting + radiators is 多处根基错叠加大量搭配错），或任何错误真正妨碍理解，才构成 ② 类。
...
第 2 步 · 定分：语言使用维度只由 ② 类决定——② 类为零/极少且不妨碍理解 → 4.5–5；② 类反复出现、系统性失控（锚 B 画像，如公寓暖气报修邮件反复动词形态错）→ 3–3.5、整体落 4 档；有错误妨碍理解 → 才压 3.5 以下。若三目标齐全、语域得体、无系统性失控，整体落 5 档。
  5 与 4.5 的分界（防变宽过冲）：5.0 要求**每个目标都有细节展开**（官方口径「全面且有细节地完成」）。若三个目标都完成、但其中某个只有一句话带过（如 appreciation 只有一句 "I really enjoy your magazine"、询问只有一句），整体分给 4–4.5，不得给 5——目标齐 ≠ 目标满。
