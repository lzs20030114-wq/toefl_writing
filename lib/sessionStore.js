...
async function syncCloudHistory() {
  if (!currentUserCode || !isSupabaseConfigured || cloudSyncInFlight) return;
  cloudSyncInFlight = true;
  try {
    const { sessions, error } = await loadSessionsCloud(currentUserCode);
    if (!error) {
...
export function loadHist() {
  if (currentUserCode && isSupabaseConfigured) {
    if (!cloudSyncInFlight && (!cloudHistCache || !Array.isArray(cloudHistCache.sessions))) {
      cloudHistCache = { sessions: [] };
      syncCloudHistory();
    }
