...
import { consumeUsage } from "./dailyUsage";
...
export function saveSess(s) {
  ...
  // Consume daily usage (server enforces limits for all tiers)
  if (currentUserCode) {
    consumeUsage(currentUserCode).catch(() => {});
  }
}
...
export function upsertMockSess(s, mockSessionId) {
  ...
  // Consume daily usage only for new mock sessions (not updates) — mock exam costs 3
  if (isNewSession && currentUserCode) {
    consumeUsage(currentUserCode, 3).catch(() => {});
  }
}
