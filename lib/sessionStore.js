...
import { getSavedTier } from "./AuthContext";
...
export function saveSess(s) {
  ...
  // Consume daily usage for free-tier users
  const tier = getSavedTier();
  if (tier === "free" && currentUserCode) {
    consumeUsage(currentUserCode, tier).catch(() => {});
  }
}
...
export function upsertMockSess(s, mockSessionId) {
  ...
  // Consume daily usage only for new mock sessions (not updates)
  if (isNewSession) {
    const tier = getSavedTier();
    if (tier === "free" && currentUserCode) {
      consumeUsage(currentUserCode, tier).catch(() => {});
