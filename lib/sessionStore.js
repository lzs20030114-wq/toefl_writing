...
// Replace an existing session in place (used when a stored session is re-scored
// from history). Cloud mode updates the row by its numeric id; local mode
// replaces by index. Never inserts a new row — a re-score must not duplicate.
export function updateSess(updatedSession, fallbackIndex = null) {
  if (!currentUserCode || !isSupabaseConfigured) {
    if (!isBrowser()) return;
    const h = loadFromLocalStorage();
    const idx = Number(fallbackIndex);
    if (Number.isInteger(idx) && idx >= 0 && idx < h.sessions.length) {
      h.sessions[idx] = normalizeSession({ ...h.sessions[idx], ...updatedSession });
      writeHistoryLocalStorage(h);
    }
    return;
  }
  const id = Number(updatedSession?.id);
  if (!Number.isInteger(id)) return;
  const sessions = Array.isArray(cloudHistCache?.sessions) ? [...cloudHistCache.sessions] : [];
  const cacheIdx = sessions.findIndex((s) => Number(s?.id) === id);
  if (cacheIdx >= 0) sessions[cacheIdx] = { ...sessions[cacheIdx], ...updatedSession };
  cloudHistCache = { sessions };
  emitHistoryUpdated();
  updateSessionCloud(id, updatedSession).then(({ error }) => {
    if (error) console.error(error);
    syncCloudHistory();
  });
}
