...
export function deleteSession(identifier) {
  ...
  // Refuse ids that don't match a synced row. A just-saved optimistic entry
  // has no row id yet, and callers that fall back to its array index must
  // never have that index treated as a row id (sessions.id is a global
  // SERIAL — a collision would delete an unrelated record).
  if (!sessions.some((s) => Number(s?.id) === id)) return loadHist();
  cloudHistCache = { sessions: sessions.filter((s) => Number(s?.id) !== id) };
  emitHistoryUpdated();
  deleteSessionCloud(id, currentUserCode).then(({ error }) => {
    if (error) console.error(error);
    syncCloudHistory();
  });
  return loadHist();
...
// Batch variant for "clear this subject's records": one cloud round trip and
// one resync instead of N of each. Local identifiers are array indexes, so
// they're deleted in descending order — ascending splices would shift later
// targets onto the wrong (possibly other-subject) records.
export function deleteSessions(identifiers) {
  const ids = (Array.isArray(identifiers) ? identifiers : [])
    .map(Number)
    .filter((n) => Number.isInteger(n));
  if (!currentUserCode || !isSupabaseConfigured) {
    [...ids].sort((a, b) => b - a).forEach((idx) => deleteFromLocalStorage(idx));
    return loadFromLocalStorage();
  }
  const sessions = Array.isArray(cloudHistCache?.sessions) ? [...cloudHistCache.sessions] : [];
  // Same synced-row guard as deleteSession, applied per id.
  const rowIds = ids.filter((id) => sessions.some((s) => Number(s?.id) === id));
  if (rowIds.length === 0) return loadHist();
  cloudHistCache = { sessions: sessions.filter((s) => !rowIds.includes(Number(s?.id))) };
  emitHistoryUpdated();
  deleteSessionsCloud(rowIds, currentUserCode).then(({ error }) => {
    if (error) console.error(error);
    syncCloudHistory();
  });
  return loadHist();
