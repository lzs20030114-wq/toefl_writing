 ...
 *   Permissions-Policy                  禁用不需要的浏览器功能（摄像头等），允许本站口语练习使用麦克风
 ...
 *   media-src 'self' data: blob: *.supabase.co — 音频播放 (TTS + 用户录音 blob + Supabase 听力 mp3
 *                               + 考试音频手势解锁的 data: 静音 WAV)
...
const CSP_DIRECTIVES = [
  ...
  // data: 不能删 — lib/audio/examAudioController.js 的 SILENT_WAV(data:audio/wav) 靠它
  // 在「开始」手势内解锁共享 audio 元素；拦掉它 = iOS 全场自动播放失效（曾真实发生）。
  // 有 jest 用例锁定（__tests__/middleware-csp-media.test.js）。
