...
const { auditLCItem } = require("../../../../lib/listeningGen/lcAuditor");
...
// LC verify: a TWO-speaker conversation + multi-question MCQ. Unlike LA/LAT (single text field),
// auditLCItem reads item.conversation + item.questions, so LC needs its own request shaping —
// but its result envelope (audit.details[] with questionIndex/aiAnswer/reasoning) is IDENTICAL, so
// the results mapping is the same as verifyListening's. answer may be null per question (verify 代解).
async function verifyLcConversation(rawItem) {
  const rawTurns = Array.isArray(rawItem.conversation) ? rawItem.conversation : [];
  const conversation = [];
  for (const t of rawTurns) {
    const speaker = String(t?.speaker || "").trim();
    const text = String(t?.text || "").trim();
    if (text) conversation.push({ speaker, text });
  }
  const rawQuestions = Array.isArray(rawItem.questions) ? rawItem.questions : [];
  if (conversation.length === 0 || rawQuestions.length === 0) {
    return jsonError(400, "item must include the conversation turns and at least one question");
  }
  if (rawQuestions.length > MAX_QUESTIONS) return jsonError(400, `Too many questions (max ${MAX_QUESTIONS})`);
  ...
  const audit = await auditLCItem({ conversation, questions }, (prompt) => verifyCallAI(prompt));
  if (audit.error || !Array.isArray(audit.details)) return jsonError(502, "复核失败，请稍后重试");
...
export async function POST(request) {
  try {
    ...
    if (!["rdl", "ap", "lcr", "la", "lat", "lc"].includes(type)) return jsonError(400, "Invalid type");
    ...
    // ── LC (听对话): two-speaker conversation + multi-question MCQ. Uses lcAuditor (reads
    // conversation + questions), mapped to the same rdl/ap results envelope.
    if (type === "lc") return await verifyLcConversation(rawItem);
