  ...
  const inner = (
    <Link
      ...
      style={{
        ...
        boxShadow: hasFooter ? "none" : (isHover ? (isChallenge ? "0 6px 20px rgba(255,30,30,0.2)" : T.shadowHover) : T.shadow),
    ...
    >
      ...
      <div style={{ padding: "14px 16px 14px 18px", flex: 1, minWidth: 0, display: "flex", flexDirection: "column", justifyContent: "center" }}>
        <div style={{ fontSize: 11, color: isChallenge ? CH.accent : T.t3, fontWeight: 700, marginBottom: 3, letterSpacing: 0.3 }}>{n}</div>
      ...
      <div style={{ padding: "14px 14px", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "flex-end", gap: 6 }}>
        <div style={{ fontSize: 11, fontWeight: 600, whiteSpace: "nowrap", color: isChallenge ? (isMock ? CH.accent : CH.t2) : T.t2, background: isChallenge ? (isMock ? "rgba(255,30,30,0.1)" : "rgba(255,255,255,0.05)") : "transparent", borderRadius: 6, padding: "3px 7px", border: `1px solid ${isChallenge ? (isMock ? "rgba(255,30,30,0.2)" : "rgba(255,255,255,0.08)") : T.bdr}` }}>{it}</div>
        ...
        <Arrow color={isChallenge ? CH.accent : T.t3} />
      </div>
    </Link>
  );
  ...
  return (
    <Link
      ...
      style={{
        ...
        boxShadow: isHover ? T.shadowHover : T.shadow,
        transition: "transform 150ms ease, box-shadow 150ms ease, border-color 150ms ease",
      }}
    >
      ...
      <div style={{ padding: "14px 16px 14px 18px", flex: 1, minWidth: 0, display: "flex", flexDirection: "column", justifyContent: "center" }}>
        <div style={{ fontSize: 11, color: isChallenge ? color : T.t3, fontWeight: 700, marginBottom: 3 }}>{eyebrow}</div>
      ...
      <div style={{ padding: "14px 16px", display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "flex-end", gap: 6 }}>
        {badge ? <div style={{ fontSize: 11, fontWeight: 600, whiteSpace: "nowrap", color: isChallenge ? color : T.t2, background: isChallenge ? "rgba(255,255,255,0.05)" : "transparent", borderRadius: 6, padding: "3px 8px", border: `1px solid ${isChallenge ? `${color}33` : T.bdr}` }}>{badge}</div> : null}
        <Arrow color={isChallenge ? color : T.t3} />
      </div>
    </Link>
  );
}
