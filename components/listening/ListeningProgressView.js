    ...
    <div style={{ transition: "all 0.2s" }}>
      ...
      {expanded && s.details?.results && (
        <div style={{ padding: "0 16px 14px 62px", animation: "fadeUp 0.2s ease" }}>
          {subtype === "lcr" ? <LCRDetail session={s} /> : subtype === "lc" ? <LCDetail session={s} /> : <LADetail session={s} />}
        </div>
      )}
    </div>
  );
...
export function ListeningProgressView({ onBack }) {
  ...
  const sessions = useMemo(() => {
    ...
    return hist.sessions.filter(s => s.type === "listening").sort((a, b) => new Date(b.date) - new Date(a.date));
  }, [hist]);
