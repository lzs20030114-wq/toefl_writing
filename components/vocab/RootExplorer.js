                  ...
                  <div role="group" aria-label={`${item.word} 复习类型`} style={{ display: "flex", gap: 5, marginTop: 5 }}>
                      {[["reading", "阅读词"], ["listening", "听力词"]].map(([mode, label]) => (
                        <button key={mode} type="button" aria-pressed={(reviewModes[item.word] || getCard(item.word)?.reviewMode || "reading") === mode}
                          ...
                          style={{ border: `1px solid ${(reviewModes[item.word] || getCard(item.word)?.reviewMode || "reading") === mode ? ACCENT : C.bdr}`, background: (reviewModes[item.word] || getCard(item.word)?.reviewMode || "reading") === mode ? "#ECFEFF" : "#fff", color: C.t2, borderRadius: 999, padding: "2px 8px", fontSize: 11, cursor: "pointer" }}>
                          {label}
                        </button>
                      ))}
                  </div>
