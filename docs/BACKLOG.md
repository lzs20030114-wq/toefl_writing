...
- [中] **造句真题：跨卷重出的题已放出来（2026-09-14/15），剩下 157 槽是真缺内容、都要本机跑**：
  ...
  **剩余 157 槽（都要本机的 .codex-tmp 与源料）**：
  ① 管线丢题 68：`node scripts/realbank/loss_ledger.mjs --plan` 出作业单，主力是
  5.11 / 5.18 / 5.23 / 5.29 / rf0826 与 5.6_v2 / 5.10_v2（后两套是同日重跑卷，补进来大概率又是重复题）。
  ② 落库丢弃 42：structured 里只有答案句、没有题面模板 / 词块 —— 跑 `extract_bs_pages.py`
  补那几页识图（写作 PDF 无文字层，题面只存在于考试界面截图里）。
  ③ 源料缺陷 47：3.10（原卷「写作.pdf」实为听力文件，无写作题）+ 2.23 / 3.29 / 2.8 等卷里
  GT 也没记到的那些题。**GT 明确记了、但库里连答案句都没有的还有 53 道** —— 这 53 道是
