...
- [中] 写作评分三路全空正文时的**自救**（出处：2026-09-21 上面那条的后续）。**已做一半（2026-09-21）**：三路全空时先降级单发自救一次（剩余预算 >45s 才开始，救回来就照常出分并留一条 `deepseek_rescue / empty_content_rescued` 痕迹，救不回来才回 502 且详情带 `rescue:` 段）。降到 1 路同时针对两种成因：躲开三路并发（09-09 网关掐流同款诱因），以及对「推理吃光预算」这种重尾随机失败做独立重试。**仍待定**：根治方案要看线上 `empty_content_rescued` 的出现频率和诊断行分布再拍——若自救命中率高说明就是并发/重尾问题，可考虑直接把 samples 常态降到 1；若自救也普遍失败，说明是账号级或网关级故障，得往别处查。原始约束如下：`lib/ai/writingEval.js` 请求 `maxTokens=8000` 而 `/api/ai` 的 `MAX_TOKENS` 上限是 8192，**已无余量可抬**。待线上攒到几条带诊断行的真实记录后再定方案：若详情普遍是 `finish=length` + reasoning 很长 → 是推理吃光预算，考虑降级重试（关推理或压低预算）或抬 MAX_TOKENS；若普遍是 `chunks=0 / stream-cut` → 是上游回 200 就掐流，那就该按 09-09 原计划把 samples=3 在重试时降为 1 路。**先看数据再动手，不要盲目加重试烧钱。**
