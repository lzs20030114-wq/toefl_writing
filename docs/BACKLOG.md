...
- [中] **真题阅读缺题 483 → 313（2026-09-14，零成本部分已修，报告 REALBANK-LOSS-SYSTEMIC-2026-09-14.md §12）**：
  已修 = 填词分词（括号 / 破折号连写整篇构建失败）+ 前端挖空前后标点渲染、4.29 只剩填词的卷不再整卷跳过、
  `ctw_verify` 补充判据（答案页错字 / 残片糊字母，要词典 + 屏幕 OCR 两份证据）、`--reverify-*` 看图正文失效与合流快照被灌回、
  阅读库按上一版顺序落盘（「第 N 套」不再漂）；CTW 146→153 段（另 10 段是已上线篇目的副本，记跨套重复）。
  **剩 313**：同篇库里已有 79（账本还没按内容认别名）/ 复核下架 50 / 小额重扫 57（JSON 失败、路由错，≈¥0.5，先 --dry）/
  1.21B 重扫 + 1.28A M2 第 6 空答案页重复待人定 20 / 插入句题 19（insert_promote，别重扫）/ 盲审两票不一致 9（人工核答案）/
  对齐层无块 74（4.29 选择题答案页缺、3.2B·3.24·4.20 找不到题面屏、rf0826 没对齐）/ 其它 5。
