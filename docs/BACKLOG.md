...
- [高] 真题丢题全科口径（2026-09-14，报告 data/claudeGen/reports/REALBANK-LOSS-SYSTEMIC-2026-09-14.md）：
  ...
  **先跑写作的邮件/学术讨论**（108 题：email 54 + disc 54）——学术讨论全库仅 7 题、是整卷拼齐的
  唯一瓶颈，且写作不走盲审闸，重扫完直接 build_bank 就进库，链路最短。
  **造句 bs 的 341 题不能靠重扫**：题面来自 extract_bs_pages.py 看图产出的 `<卷>.bs.json`
  （写作 PDF 无文字层），且还卡着「词块边界被 OCR 糊掉、只能做成真题句子+本站切块」的来源分档未决项。
...
- [✅部分完成 2026-09-14] 阅读「选项残 = 整题作废」的死循环（2026-09-14，报告 REALBANK-INGEST-AUDIT-2026-09-14.md §7）：
  ...
  **待做**：①把落库丢弃计数按 (卷,科,题号,原因) 落账、纳入 loss-ledger（现在只 console.log 到终端，
  跑完即散，事后无从查证）；②`real_ap_21b_2_12` 插入题选项是正文碎片而非 [A]~[D]（26 道里 1 道）。
  **背景数据**：AP 86 篇里 32 篇不足 5 题（28 篇 4 题 + 4 篇 3 题），共缺 36 题；
  选项残只是成因之一，其余是盲审不一致/复核扣下/插入题无标记/源缺屏，要本机 build_bank --dry 看日志拆。
