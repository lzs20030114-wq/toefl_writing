...
- [✅完成 2026-09-09] **真题按题型组套（同源不借）**：assemble_sets 默认 `--borrow` 关，`type_sets` 按场按题型出套并如实标 got/need；跨套重复下架的 212 条按 `dup_of` 原位还回（别名）。结果与补题清单见 docs/realbank-set-blueprint.md §三/§四。**待做**：前端题型套入口；补题清单本机执行（#2a 插入题 ■ 找回工具链已就绪、#2b 重排版插入题合成选项已改待重跑、#3 对话标性别、#4 4/5 月 16 套约 ¥26，命令顺序见 blueprint §四）；#5 填词词首还原需先看原始残片。
