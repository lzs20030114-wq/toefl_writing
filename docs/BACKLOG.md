...
- [中] **真题阅读缺题 483 → 313 → 241（2026-09-14，报告 REALBANK-LOSS-SYSTEMIC-2026-09-14.md §12 / §12.3）**：
  零成本已修 = 填词分词 + 前端标点渲染、4.29 只剩填词、ctw_verify 补充判据、阅读库保序；用户拍板花钱的三项（≈¥0.77）已做 =
  Q35 误路由成填词（route_type.js）+ 5 块填词重扫 + rf0808 答案表解析 + 插入句题转正 11 道（过审 7）+ Dinosaur Feathers 改留原卷版。
  顺手堵的坑：writeStructured 不传 freshKeys 会把听力合流产出灌回快照；audit_answers 没有 --dry。
  **剩 241**：同篇库里已有 79 / 复核下架 ~47 / 盲审两票不一致 11（含本轮 4 道插入题，人工核答案）/ 插入题源料缺屏 9 + 过不了标记校验 4 /
  1.28A Stoicism 整篇下架（看图正文是全的，换正文解除下架待拍板）/ 1.28A M2 填词第 6 空答案页重复待人定 /
  对齐层无块 74（4.29 选择题答案页缺、3.2B·3.24·4.20 找不到题面屏、rf0826 没对齐）/ 点选句子 1 / 计数口径 4。
