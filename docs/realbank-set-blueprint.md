...
> 2026-09-14 起，这张手工清单的「还缺什么」由 `node scripts/realbank/loss_ledger.mjs` 自动算：
> 读 sets.json + 复核清单 + 源料体检 + 合并账本，逐槽给出缺口与归因，写 `data/realBank/loss-ledger.json`。
> 下面这张表保留作为历史记录（每一项的**做法**仍然有效）；要知道当前缺口，跑账本，别照表猜。
> 注意账本的分母是 69 套 × 蓝图全科，包含 sets.json 里整科缺席的科目 —— 本节旧数字是不含它们的口径。
