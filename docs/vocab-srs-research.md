...
> **产品规则更新（2026-09-24）：**本报告的卡型比较保留为当时的研究依据；当前产品按用户选择，所有词（含旧卡和新卡）默认要求会写，复习时走中译英拼写方向。用户可在单词本逐词剔除会写要求，剔除后只考英译中。一个词仍只有一张卡，调度和二档评分不变。
...
上述第 2 条记录的是 2026-09-13 的初版实现；2026-09-24 起卡型默认改为中译英拼写，逐词关闭后走认词方向。原句挖空保留为拼写卡的语境线索。
