...
## 10. 1.28A Module 1 用新源音频补录（2026-09-11）

新源 `1月/1.28/1.28 套一听力音频/part1.m4a` 是真正的 1.28A M1 音频（旧源里的 mp3 是 1.21A 的）。做法：不写桌面源目录，在 `.codex-tmp/realbank/src-staged/1.28新托福真题A卷/` 搭临时源（part1 转成 `ListeningModule1.mp3`、拷 PDF 与口语音频、**不放 Module2**），把错配音频的 ASR 缓存改名备份，`REALBANK_SRC_ROOT` 指向临时源跑 `asr_cache.py` → `merge_first_source_asr.py --set`。先摘掉了 lc-speaker-overrides 里 1.28A M1 Q17 那条（当时是听错配音频标的，不可信）。

结果：对齐 **0/27 → 20/27**（M1 全对上；M2 7 组 no_audio_span 属预期）。但盲审只有 **13/23 = 56.5%**——短应答 11/12 一致，通知 4/6、讲座 5/8 **不一致**。按题面关键词对每段材料算重合度：对话题组 Q15-16（汽车保养）对的是逐字稿第 1 段，而第 1 段配的是 Q13-14；通知 Q19-20（女生明天去泳池/博物馆/看房东）和三段通知一段都不沾——材料在逐字稿里根本没有；讲座配对看着对但 Q29-32 四题全不一致，也不是整体错位一格。**结论：1.28A 这份卷的题目 PDF / 逐字稿 PDF / 答案页内部就不齐（不止音频装错）**，只有短应答（逐条按刺激句相似度对上 + 盲审一致）可信，对话/通知/讲座即使个别题碰巧一致也不放。

落库：短应答 9 条进库，其中 4 条与已在线条目口播逐字相同（128b_1_06 / 128b_1_10 / rf0620_1_06 / rf0622_1_01）被跨卷去重挤掉了老的——老的已绑原声，应保留 → 复核清单加 4 条 `dup_of` 指定保留老的，重建后老的回来、原声回挂 385 不掉。**净新增 5 条短应答**（128a_1_04/05/09/11/12）：3 条绑上原声（过闸），2 条尾巴不干净/词速异常 → TTS 配音 ¥0.01。对话/通知/讲座 0 条（盲审闸拦下）。

顺带暴露一个 bind_original_audio 的 bug：按 `--set` 增量跑时「TTS 兜底补旁白」没按范围过滤，且本轮报告里没有其它卷的原句就退化成通用句并覆盖线上文件（8 条通知/讲座）——已修根因（沿用台账原句、范围过滤、一致则跳过）并从 HEAD 台账重做 8 条。

商家补录清单再加一条：1.28A 需要**成套一致**的题目/逐字稿/答案/音频，现有 A 卷包是拼的。
