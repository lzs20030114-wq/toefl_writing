...
**修正后的立项建议 + 执行状态（2026-07-09）**：
1. ✅ **唯一解校验器已建并全跑**：`lib/readingGen/ctwUniqueness.js` + `scripts/audit/ctw-uniqueness.mjs`
   （挂 `full-audit-l1` script=ctw-uniqueness）。复核 L1 嫌疑 114 题，87 题有多解空，第二解按类：
   **屈折 37 / 功能词 39 / 内容近义 51**（明细 `ctw-uniqueness-report.md`）。
2. ✅ **屈折变体（37 空/34 题）判分宽容已落**：`lib/reading/ctwScoring.js` 统一判分，空加 `accepted_words`，
   练习+模考都接受同词根等价形式（sugar/sugars 等）。严格只认已知屈折后缀，杜绝 the/these 误纳
   （首版 heuristic 有此 bug，已修+加回归测试）。**残余风险**：极少数变体在具体句里时态/单复数其实不合
   （如某句需 cooled 却也纳了 cool）；只惠及填了该变体的用户，可接受。想零残余就把这 37 个改成"重挖空"——说一声即可回退。
3. ⏳ **功能词短碎片（39 空）**：真题靠上下文锁死、我们这些锁不住 → **建议生成器换词重挖**（放宽判分会失去区分度）。清单在报告里标 `function`。
4. ⏳ **内容近义（51 空）**：逐个看语境能否锁定，锁不住重挖或补 accepted_words。清单标 `content`。
5. 生成期门禁：`checkItemUniqueness` 已可接进合库 auditor（新题多解即拒/换词），待接线夜间 routine。
6. 重挖修复后重跑 `full-audit-l1`（script=ctw-uniqueness）验证多解归零。
