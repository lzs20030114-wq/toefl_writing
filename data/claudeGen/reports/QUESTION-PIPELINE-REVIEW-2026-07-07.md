...
P0-2「重复入库」已在分支 `claude/question-pipeline-review-6p2bpt` 全链路处置完毕：
1. **合库层内容去重**（commit `10c8c4d`）：`lib/gen/contentDedup.js` 全题型指纹+近重复，接入 `merge-staging.mjs`（含批内自重复）与 `mergeClaude.mjs`（BS 0.75 / Disc·Email 0.8）；隔离实测同批双合第二次 38 条全拦。
2. **生成端排除补齐**（commit `80f017a`）：12 题型排除列表改 bank∪staging 并集（staging 优先）；修 LAT 排除恒空 bug；RDL 首次获得 excludeSubjects；`lib/gen/promptExcludes.js` 共用辅助。
3. **存量清理**（本 commit）：`scripts/ops/dedup-banks.mjs` 清除 8 库 1472 条重复（保留最老原件），删除清单在 `dedup-removed-ids-2026-07-07.json`；8 库复测重复率归零（3083→1611）；重建 `answer_hashes.json`（590→612，旧台账与现库零重叠、早已失效）。

**仍开放**：P0-1 盲审 routine trigger 重建与 388 条积压补合（需用户拍板执行方式）；860 条孤儿音频的 Supabase 存储清理（清单已备）；报告 §7 其余 P1/P2 项。
