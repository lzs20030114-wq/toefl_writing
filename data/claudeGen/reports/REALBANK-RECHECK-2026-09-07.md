...
| 题干与逐字稿不配 | LAT rf0629_1_29#1 | 问 Bollywood，稿里讲的是 bossa nova |
...
| RDL | 60 组 / 139 题 | **49 / 114** | LC | 36 / 72 | **29 / 56** |
...
1. **音频未核**：Supabase 存储从这个环境访问不到（代理拒 CONNECT）。LAT 4 条截掉的「Module 2」尾巴和 `rf0708_2_12` 的开头串音，如果音频是商家原声切片，音频里可能还在——本机听一下，有就重切/重配。
2. `real_ap_511_1_26#2` 答案键 D vs 独立作答 C，对原截图核。
3. LC 6 条「说话人对调」如本机听音频能确认是题干错，改题干后可放行。
4. 保留但带小瑕疵（不影响作答，未动）：AP `310_1_25` 柱状图只剩坐标轴文字；RDL `rf0615_1_125` 一处对话答非所问；LC `rf0716_1_15` 一句 ASR 乱码 + 一处 my/your；LA `rf0615_1_21#0` A/D 都说得通（键 A）；LCR `rf0615_1_10` C/D 竞争；7 套面试无 intro（UI 有兜底文案）。
5. 阅读题干里「paragraph N」引用：题池来源 OCR 常把段落拍平，本次只扣了引用无从定位的题；以后录入题池阅读时应在 parse 阶段保住段落分隔。
