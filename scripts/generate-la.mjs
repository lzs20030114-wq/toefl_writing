...
/**
 * Generate Listen to an Announcement (LA) questions using DeepSeek.
 *
 * Usage: node scripts/generate-la.mjs [--count 10] [--with-tts] [--dry-run] [--skip-audit]
 *
 * Pipeline:
 *   1. Build prompt for announcement listening questions
 *   2. Call DeepSeek to generate items
 *   3. Validate structure and quality
 *   4. (Optional) Generate TTS audio for announcement text
 *   5. Save accepted items to staging
 *
 * 2026 TOEFL Listening Task:
 *   - Hear a campus/classroom announcement (50-120 words)
 *   - Answer 2-3 MCQ about details, inference, main idea
 */
...
const { validateLA, validateBatchDistribution } = require("../lib/listeningGen/laValidator.js");
...
async function generateTTS(item, id) {
  ...
  try {
    ...
    const est = estimateCost(item.announcement);
    console.log(`   TTS for ${id}: ${est.chars} chars, ~$${est.cost.toFixed(4)}`);

    const buffer = await generateSpeech(item.announcement, {
      voice: "nova",
      model: "tts-1-hd",
      format: "mp3",
    });
...
async function main() {
  console.log("========================================================");
  console.log("   Listen to an Announcement (LA) — Generation Pipeline");
  console.log("========================================================\n");
  ...
  const prompt = buildLAPrompt(COUNT, { excludeIds: [...existingIds] });
  ...
  for (let i = 0; i < parsed.length; i++) {
    const item = parsed[i];
    const id = `la_gen_${timestamp}_${String(i + 1).padStart(3, "0")}`;
    item.id = id;
  ...
  if (accepted.length > 0) {
    ...
    console.log(`\n-- Sample --`);
    console.log(`Context: ${sample.context} | Speaker: ${sample.speaker_role || "unknown"}`);
    console.log(`Announcement: "${sample.announcement?.slice(0, 100)}..."`);
