...
| `credits-schema.sql` | 2026-07-13 | 已跑(2026-07-18 用户确认) | AI 点数钱包+双余额(订阅/购买)+幂等原子刷新/发放/扣点/退款 RPC；仅建基础设施，不接现有流程，需保持三个 credits feature flags=false |
| `daily-usage-quota.sql` | 2026-06-04 | 历史迁移,状态未知(建库早期) | `/api/ai` 服务端每日 AI 用量计数 |
| `feature-engagement-subtype.sql` | 2026-07-16 | 已跑 | 留存后台「按小题型」粒度:给 `feature_sessions` 追加 `subtype` 列(并把遗留 adaptive-*/speaking-exam 归一进所属科目) + `feature_subtype_totals`/`feature_subtype_stickiness` 视图。需在 feature-engagement.sql 之后跑 |
