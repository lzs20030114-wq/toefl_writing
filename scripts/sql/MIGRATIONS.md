...
| `feature-engagement-subtype.sql` | 2026-07-16 | **未跑** | 留存后台「按小题型」粒度:给 `feature_sessions` 追加 `subtype` 列(并把遗留 adaptive-*/speaking-exam 归一进所属科目) + `feature_subtype_totals`/`feature_subtype_stickiness` 视图。需在 feature-engagement.sql 之后跑;未跑时后台小题型档显示迁移提示(优雅降级,不影响整科统计) |
