...
/**
 * Generate Listen to a Conversation (LC) questions using DeepSeek.
 *
 * Usage: node scripts/generate-lc.mjs [--count 10] [--with-tts] [--dry-run] [--skip-audit]
 *
 * Pipeline:
 *   1. Build prompt for conversation listening questions
 *   2. Call DeepSeek to generate items
 *   3. Validate structure and quality
 *   4. (Optional) Generate TTS audio for conversation
 *   5. Save accepted items to staging
 *
 * 2026 TOEFL Listening Task:
 *   - Hear a short conversation between 2 people (8-12 turns, ~100-200 words)
 *   - Answer 2 MCQ about details and inference
 */
...
const { validateLC, validateBatchDistribution } = require("../lib/listeningGen/lcValidator.js");
...
async function generateTTS(item, id) {
  ...
  try {
    ...
    // Concatenate conversation turns for TTS
    const fullText = item.conversation
      .map(t => `${t.speaker}: ${t.text}`)
      .join(" ");

    const est = estimateCost(fullText);
    console.log(`   TTS for ${id}: ${est.chars} chars, ~$${est.cost.toFixed(4)}`);

    const buffer = await generateSpeech(fullText, {
      voice: "nova",
      model: "tts-1-hd",
      format: "mp3",
    });
...
async function main() {
  console.log("========================================================");
  console.log("   Listen to a Conversation (LC) — Generation Pipeline");
  console.log("========================================================\n");
  ...
  const prompt = buildLCPrompt(COUNT, { excludeIds: [...existingIds] });
  ...
  for (let i = 0; i < parsed.length; i++) {
    const item = parsed[i];
    const id = `lc_gen_${timestamp}_${String(i + 1).padStart(3, "0")}`;
    item.id = id;
  ...
  if (accepted.length > 0) {
    ...
    console.log(`\n-- Sample --`);
    console.log(`Context: ${sample.context} | Speakers: ${sample.speakers?.map(s => `${s.name} (${s.role})`).join(", ")}`);
    console.log(`Turns: ${sample.conversation?.length || 0} | Questions: ${sample.questions?.length || 0}`);
    if (sample.conversation?.[0]) {
      console.log(`  ${sample.conversation[0].speaker}: "${sample.conversation[0].text}"`);
      if (sample.conversation[1]) {
        console.log(`  ${sample.conversation[1].speaker}: "${sample.conversation[1].text}"`);
      }
      console.log(`  ... (${(sample.conversation.length - 2)} more turns)`);
    }
