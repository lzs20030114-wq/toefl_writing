...
// 总结页那条要冲掉「STT 落地 → 等待态收口 → 总结页」一整串 effect，两拍不够。
...
describe("RepeatTask 总结页", () => {
  test("重录不覆盖原始成绩，也不会二次上报 onComplete", async () => {
    ...
    // 真考节奏（repeat-auto-record-flow）：「开始」先在手势内预授权麦克风（放行一次），
    // 进录音阶段自动开麦（再放行一次），点停止 = 最后一句录完 → 等识别 → STT 落地自动进总结页，
    // 中间没有 Recorded / Finish 那一屏。
    ...
    const stopBtn = screen.getByText("录音中…点击停止").parentElement.querySelector("button");
    await act(async () => { fireEvent.click(stopBtn); await flush(); });
