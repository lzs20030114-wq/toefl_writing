...
test("watchdog: extends while genuinely buffering (data seen), then times out at 15s", () => {
  jest.useFakeTimers();
  const { c, events } = makeController();
  c.play(SRC_A);
  setNetworkState(2); // NETWORK_LOADING — still fetching
  setReadyState(1);   // 已收到过数据(HAVE_METADATA) → 真在缓冲,给足 15s
  jest.advanceTimersByTime(4000);
...
test("watchdog: unblessed element stuck at 0 bytes fails fast at 4s (iOS 无手势不加载)", () => {
  // iOS 真机实锤:无手势时 networkState 显示 2 但永远 readyState=0——再等到
  // 15s 也是死等,4s 即判负让恢复弹窗尽快出现。
  jest.useFakeTimers();
  const { c, events } = makeController();
  c.play(SRC_A);
  setNetworkState(2);
...
test("retry() spends the whole gesture on the real clip — no silent-WAV prelude (继续考试恢复)", async () => {
  // iOS 一次手势只祝福一次播放:静音片抢跑会耗光手势,正片沦为无手势加载。
  // retry 必须同步、直接地播正片(与用户点直链同级),不许中途碰 SILENT_WAV。
  playMock.mockRejectedValueOnce(new DOMException("denied", "NotAllowedError"));
  const { c, events } = makeController();
  ...
  c.retry(); // 「继续考试」点击
  expect(audioEl.src).toBe(SRC_A); // 同步换回正片 — 手势内完成,从未经过 data: 静音片
  expect(playMock.mock.calls.length).toBe(callsBefore + 1); // 只有正片这一次 play()
  await flush();
  audioEl.dispatchEvent(new Event("playing"));
  expect(c.isUnlocked()).toBe(true); // 正片出声即视为解锁完成
  expect(events.some((e) => e.type === "loading" && e.src === SRC_A)).toBe(true);
  c.destroy();
});
