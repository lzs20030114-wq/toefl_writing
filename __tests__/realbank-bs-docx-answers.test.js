/**
 * 5 月第二来源（docx 卷）的造句答案句兜底（scripts/realbank/extract_bs_pages.py 的 docx_writing_answers）。
 *
...
make("卷好", ["阅读", "1a 2b 3c", "加试", "1d", "", "听力", "1a 2c", "", "写作"] + ten + ["", "口语", "all our books are on the shelves"])
make("卷少", ["写作"] + ten[:9] + ["口语", "repeat me please"])
make("卷无", ["阅读", "1a", "口语", "a b c"])
E.CONVERTED_DIR = tmp
print(json.dumps({k: E.docx_writing_answers(k) for k in ("卷好", "卷少", "卷无", "卷不存在")}, ensure_ascii=False))
`;
...
maybe("docx 卷造句答案句兜底", () => {
  ...
  test("写作表头之后、下一个科目表头之前的 10 段，按顺序编号；不吃进口语复述句", () => {
    expect(Object.keys(result["卷好"])).toHaveLength(10);
    expect(result["卷好"]["1"]).toBe("sentence number 1 is here");
    expect(result["卷好"]["10"]).toBe("sentence number 10 is here");
    expect(Object.values(result["卷好"])).not.toContain("all our books are on the shelves");
  });

  test("不是恰好 10 句 / 没有写作段 / 没有转换记录 → 一条都不给（不猜）", () => {
    expect(result["卷少"]).toEqual({});
    expect(result["卷无"]).toEqual({});
    expect(result["卷不存在"]).toEqual({});
  });
});
